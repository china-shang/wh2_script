	...
	if cm:model():campaign_name("wh2_main_great_vortex") then
		...
		if cm:is_new_game() == true and player:is_human() == true then
			...
			for i = 0, faction_list:num_items() - 1 do
				...
				if faction:is_human() == false and faction:has_pooled_resource("wh2_main_ritual_currency") == true then
		...
		core:add_listener(
			...
			function(context)
				return context:faction():name() == "wh2_main_def_hag_graef";
			end,
			function(context)
		...
		if not effect.get_advice_history_string_seen("malus_vortex_reminder") then
			effect.set_advice_history_string_seen("malus_vortex_reminder")
...
function shadow_FactionTurnStart(malus_faction)
	...
	if malekith_faction:is_null_interface() == false then
		if malekith_faction:has_pooled_resource("wh2_main_ritual_currency") == true then
			malekith_amount = malekith_faction:pooled_resource("wh2_main_ritual_currency"):value();
		end
	end

	local malus_amount = malus_faction:pooled_resource("wh2_main_ritual_currency"):value();
...
function shadow_UpdateMalusUI()
	...
	if malus_vortex_bar then
		...
		"wh2_dlc14_qb_def_final_battle_malus");
	end
end
