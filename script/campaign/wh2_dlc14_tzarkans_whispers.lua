...
function add_tzarkans_whispers_listeners()
	...
	core:add_listener(
		...
		function(context)
			return context:faction():name() == tzarkans_whispers.faction;
		end,
		function(context)
	...
	core:add_listener(
		"tzarkan_MissionSucceeded",
		"MissionSucceeded",
		function(context)
			return context:mission():mission_record_key() == "wh2_dlc14_qb_skv_final_battle_snikch";
		end,
		function(context)
			tzarkans_whispers.faction = context:faction():name();
		end,
		true
	);
	...
	if turn_number == tzarkan_generate_start_mission_turn then
		if tzarkan_generate_start_mission_count == -1 then
			...
			if mission_generated_1 == true or mission_generated_2 == true or mission_generated_3 == true then
				...
				tzarkans_whispers.last_turn = turn_number + tzarkan_mission_cooldown_turns;
		...
		elseif tzarkan_generate_start_mission_count > 0 then
			...
			for i = 1, tzarkan_generate_start_mission_count do
				local generated = tzarkan_generate_mission(faction);
...
function tzarkan_generate_mission(faction)
	if faction == nil then
		faction = cm:model():world():faction_by_key(tzarkans_whispers.faction);
	end
	...
	while mission_generated == false and #mission_types.items > 0 do
		local mission, index = mission_types:weighted_select();
...
function tzarkan_generate_war_mission(tzarkan_faction)
	...
	if #weighted_factions.items > 0 then
		...
		mm:add_payload("faction_pooled_resource_transaction{resource def_malus_sanity;factor wh2_dlc14_resource_factor_sanity_tzarkans_whispers;amount "..tzarkan_possession_reward..";}");
		mm:trigger();
...
function tzarkan_generate_raze_mission(tzarkan_faction)
	...
	if tazrkan_raze_mission_max_distance > 0 then
		region_list = cm:get_regions_within_distance_of_character(faction_leader, tazrkan_raze_mission_max_distance, true, false);
	else
		region_list = cm:get_border_regions_of_faction(tzarkan_faction, true, false);
	end
