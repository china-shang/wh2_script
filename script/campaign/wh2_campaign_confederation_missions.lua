...
confed_missions_data = {
	...
	campaign = "all", --- campaign key or 'all'
	...
	custom_callback = 
		function()
			return confed_missions:is_settlement_primary_building_at_level("wh_main_yn_edri_eternos_the_oak_of_ages", 3)
		end,
	...
	custom_callback =
		function()
			return confed_missions:is_settlement_primary_building_at_level("wh_main_yn_edri_eternos_the_oak_of_ages", 2)
		end,
	...
	custom_callback = 
		function()
			if not cm:is_dlc_flag_enabled("TW_WH2_DLC16_TWILIGHT") then
				return false
			end
			return confed_missions:is_settlement_primary_building_at_level("wh_main_yn_edri_eternos_the_oak_of_ages", 3)
		end,
	...
	factions = {"wh_dlc05_wef_wood_elves","wh_dlc05_wef_argwylon","wh2_dlc16_wef_sisters_of_twilight"}, 
	campaign = "main_warhammer",
	target_faction = "wh2_dlc13_wef_laurelorn_forest",
	disable_diplomatic_confed = false,
	disable_cai_targeting = false,
	turn_number = 0,
	require_diplomatic_knowledge = true,
	...
	----THOREK/SPINE OF SOTEK---
	["thorek_spine_of_sotek"] = 
	{
	factions = "wh2_dlc17_dwf_thorek_ironbrow",
	campaign = "wh2_main_great_vortex",
	target_faction = "wh2_main_dwf_spine_of_sotek_dwarfs",
	disable_diplomatic_confed = true,
	disable_cai_targeting = true,
	turn_number = 2,
	mission_key = "wh2_dlc17_dwf_thorek_early_game_confederate",
	dilemma_key = "wh2_dlc17_dwf_thorek_confederation_spine_of_sotek",
	confed_choices = {true,true,true,false},
	mission_generated = {},
	dilemma_completed = false
	},
...
function confed_missions:setup()
	...
	for mission_ref, mission_info in pairs(confed_missions_data) do
		...
		if not is_string(mission_info.campaign) then
			script_error("ERROR: trying to generate a confederation mission for [" .. tostring(mission_info.factions) .. "] but campaign is not a string");
		end
		...
		---15/09/2020 - T&T: convert saved data from the older system to the new format. Can be removed once T&T is out in the wild.
		if is_boolean(mission_info.mission_generated) then
			local human_factions = cm:get_human_factions()

			local mission_generated = mission_info.mission_generated

			mission_info.mission_generated = {}

			for i = 1, #human_factions do
				mission_info.mission_generated[human_factions[i]] = mission_generated
			end
		end
	end
...
function confed_missions:setup_listeners()
	...
	core:add_listener(
		"ConfedMissionsFactionTurnStart",
		"ScriptEventHumanFactionTurnStart",
		true,
		function(context)
			for mission_ref, mission in pairs(confed_missions_data) do 
				if mission.mission_generated[context:faction():name()] and cm:get_faction(mission.target_faction):is_dead() then 
					for i = 1, #mission.factions do
						cm:cancel_custom_mission(mission.factions[i],mission.mission_key,true)
					end
				end

				if confed_missions:is_mission_valid_for_faction(context:faction():name(), mission_ref) then
					confed_missions:trigger_mission(context:faction():name(), mission_ref)
				end
			end
		end,
		true)
	...
	core:add_listener(
		...
		function(context)
			...
			if confed_mission.confed_choices[choice]  == true then 
				cm:force_confederation(context:faction():name(), confed_mission.target_faction)
			end
...
function confed_missions:is_mission_valid_for_faction(faction_key, mission_key)
	...
	if cm:get_campaign_name() ~= mission.campaign and mission.campaign ~= "all" then 
		return false
	end
	...
	if is_function(mission.custom_callback) and mission:custom_callback() == false then
		return false
	end
...
function confed_missions:is_settlement_primary_building_at_level(region_key, building_level)
	...
	if region_interface:settlement():primary_slot():building():building_level() >= building_level then
		return true
	else
		return false
	end
