...
function add_nakai_hunters_listeners()
	...
	core:add_listener(
		...
		function(context)
			...
			cm:complete_scripted_mission_objective("wh_main_long_victory", "final_battle_condition", true);
		end,
		true
	);
	core:add_listener(
		...
		true,
		function(context)
			local nakai = context:faction();

			if #hunter_force_cqis > 0 and nakai:is_human() == true and nakai:name() == "wh2_dlc13_lzd_spirits_of_the_jungle" then
				local force_list = nakai:military_force_list();
						...
						for j = 1, #hunter_force_cqis do
							...
							if cm:model():has_military_force_command_queue_index(f_cqi) == true then
								...
								if force:is_null_interface() == false and force:has_general() == true then
									...
									if general:is_visible_to_faction("wh2_dlc13_lzd_spirits_of_the_jungle") == true then
...
function nakai_generate_targets()
	local target_faction = cm:get_faction("wh2_dlc13_emp_the_huntmarshals_expedition");
	...
	for i = 1, #target_subtypes do
		...
		for j = 0, char_list:num_items() - 1 do			
			if char_list:item_at(j):character_subtype(target_subtypes[i][1]) then
				...
				-- trigger mission
				local mm = mission_manager:new("wh2_dlc13_lzd_spirits_of_the_jungle", "wh2_dlc13_nakai_hunters_" .. i);
				...
				mm:add_payload("faction_pooled_resource_transaction{resource lzd_old_ones_favour;factor wh2_main_resource_factor_missions;amount 250;}");
...
function KillHunterMissionSucceeded(mission_key)
	...
	if cm:get_saved_value("hunter_jorek_killed") and cm:get_saved_value("hunter_hertwig_killed") and cm:get_saved_value("hunter_rodrik_killed") and cm:get_saved_value("hunter_kalara_killed") then
		cm:trigger_mission("wh2_dlc13_lzd_spirits_of_the_jungle", "wh2_dlc13_qb_lzd_final_battle_nakai", true);
		...
		cm:complete_scripted_mission_objective("wh_main_long_victory", "kill_four_hunters", true);
	end
end
