...
-- for testing
bool_start_global_interventions = true;
...
INTERVENTION_CLOSE_DISTANCE = 50;


-- shows infotext alongside the startup attack-enemy advice if it's appropriate
function show_attacking_enemy_infotext()
	if effect.get_advice_history_string_seen("prelude_attacking_enemy_advice") then
		out("show_attacking_enemy_infotext() called but is not showing attacking enemy infotext as it's already been seen");
		return;
	end;
	
	out("show_attacking_enemy_infotext() called, showing attacking enemy infotext");
	
	effect.set_advice_history_string_seen("prelude_attacking_enemy_advice")
...
function start_global_interventions(include_scripted_tours)
	...
	--
	if include_scripted_tours then
	...
	-- can be started separately by first-turn scripts
	if not in_post_normal_battle_victory_options.is_started then
		in_post_normal_battle_victory_options:start();
	end;
	if not in_post_battle_defeat_options.is_started then
	...
	in_help_pages:start();
	in_non_player_waaagh:start();
	in_non_player_brayherd:start();
	in_strategic_overview:start();
	...
	-- Ogre Camps
	in_ogre_camps_introduction:start();
	...
	if subculture ~= "wh_main_sc_dwf_dwarfs" then
		...
		if local_faction == "wh2_dlc17_dwf_thorek_ironbrow" then
			...
			if cm:get_campaign_name() == "wh2_main_great_vortex" then
				in_forge_first_part_recovered:start();
				in_forge_vortex_final_battle:start();
				in_thorek_vortex_skaven_attack:start();
			else
				in_eg_thorek_confederate:start();
	...
	if subculture ~= "wh_main_sc_emp_empire" then
		...
		--anyone-but-wulfhart
		if local_faction ~= "wh2_dlc13_emp_the_huntmarshals_expedition" then
			--Presige
			...
			--Wulfhart Vortex Progress
			in_wulfhart_campaign_gorrok_defeated:start();
			in_wulfhart_campaign_final_battle:start();
		end;
	end;
	...
	if subculture ~= "wh_main_sc_chs_chaos" then
		-- not norsca
		if subculture ~= "wh_main_sc_nor_norsca" then		
			in_chaos:start();
		end;
	...
	if subculture == "wh_main_sc_chs_chaos" then
		-- beastmen and chaos only
		in_player_creates_chaos_rift:start();
	else
		-- anyone else
		in_enemy_creates_chaos_rift:start();
	...
	if subculture == "wh_dlc05_sc_wef_wood_elves" then
		if cm:get_campaign_name() == "main_warhammer" then		
			--in_oak_of_ages:start();
			--in_oak_of_ages_construction_2:start();
			--in_oak_of_ages_construction_3:start();
			--in_oak_of_ages_construction_4:start();
			--in_oak_of_ages_construction_5:start();
			--in_worldroots_oak_of_ages:start();
		end
		in_worldroots:start();
		...
		in_spites:start();
		--in_asrai_lookout_constructed:start();
		--in_asrai_lookout_lost:start();
		--in_amber:start();
		--in_negative_amber:start();
	...
	if subculture == "wh2_main_sc_hef_high_elves" then
		...
		if local_faction == "wh2_main_hef_nagarythe" then
		...
		elseif local_faction == "wh2_main_hef_avelorn" then
	...
	if subculture == "wh2_main_sc_def_dark_elves" then
		...
		elseif local_faction == "wh2_main_def_hag_graef" then
			...
			in_malus_vortex_one:start();	
			in_malus_vortex_two:start();	
			in_malus_vortex_three:start();	
			in_malus_vortex_four:start();	
			in_malus_vortex_five:start();	
			in_malus_vortex_reminder:start();
	...
	if subculture == "wh2_main_sc_lzd_lizardmen" then
		...
		--anyone-but-Nakai
		if local_faction ~= "wh2_dlc13_lzd_spirits_of_the_jungle" then
		...
		if local_faction == "wh2_dlc12_lzd_cult_of_sotek" then
	...
	if subculture == "wh2_main_sc_skv_skaven" then
		...
		if local_faction == "wh2_main_skv_clan_skyre" then
		...
		elseif local_faction == "wh2_main_skv_clan_eshin" then
			...
			in_shadowy_dealings_vortex_action:start();
			in_shadowy_dealings_clan_contracts:start();
			in_shadowy_dealings_action_completed:start();
			in_snikch_vortex_one:start();
			in_snikch_vortex_two:start();
			in_snikch_vortex_three:start();
			in_snikch_vortex_reminder:start();
		elseif local_faction == "wh2_main_skv_clan_moulder" then
			if cm:get_campaign_name() == "wh2_main_great_vortex" then
				in_throt_story_introduction:start();
				in_throt_story_province_completed:start();
				in_throt_story_waystone_one:start();
				in_throt_story_waystone_three:start();
			end
	...
	if subculture == "wh2_dlc11_sc_cst_vampire_coast" then
		...
		in_ulthuan_monster_first_attack:start();
		in_ulthuan_monster_second_attack:start();
		in_ulthuan_monster_attacks_player_settlement:start();
		in_ulthuan_monster_under_player_control:start();
		in_player_infamy_grows:start();
		in_player_climbs_infamy_list:start();
		in_player_tops_infamy_list:start();
		in_first_shanty_pirate_attacks:start();
		in_first_shanty_pirate_defeated:start();
		in_second_shanty_pirate_attacks:start();
		in_second_shanty_pirate_defeated:start();
		in_third_shanty_pirate_attacks:start();
		in_third_shanty_pirate_defeated:start();
...
in_mission_advice:add_advice_key_precondition("war.camp.advice.missions.001");
in_mission_advice:set_min_advice_level(1);
in_mission_advice:set_wait_for_event_dismissed(false);
...
function in_mission_advice_trigger()
	...
	in_mission_advice:play_advice_for_intervention(
		-- Quests may be issued by parties interested in your progress. These can be reviewed at any time, my Lord.
		"war.camp.advice.missions.001",
		{
			"war.camp.advice.missions.info_001",
			"war.camp.advice.missions.info_002",
			"war.camp.advice.missions.info_003",
			"war.camp.advice.missions.info_004"
		}
	);
end;
...
in_pre_field_battle_options:set_min_advice_level(1);
...
in_pre_field_battle_options_victory_unsure:set_min_advice_level(1);
...
in_pre_field_battle_options_victory_unlikely:set_min_advice_level(1);
...
in_autoresolving:set_min_advice_level(2);
in_autoresolving:set_player_turn_only(false);
in_autoresolving:set_wait_for_battle_complete(false);
in_autoresolving:set_suppress_pause_before_triggering(true);		-- do this so we get no grace period
...
in_pre_minor_settlement_battle_options:set_min_advice_level(1);
...
in_post_normal_battle_victory_options:set_min_advice_level(1);
...
function in_post_normal_battle_victory_options_advice_trigger()	
	if cm:model():world():whose_turn_is_it():subculture() == "wh_main_sc_nor_norsca" then
...
in_post_battle_defeat_options:set_min_advice_level(1);
...
in_post_siege_battle_victory_options:set_min_advice_level(1);
...
in_ambush_defence:set_min_advice_level(1);
in_ambush_defence:set_player_turn_only(false);
in_ambush_defence:set_wait_for_battle_complete(false);
in_ambush_defence:set_disregard_cost_when_triggering(true);
...
in_commandment_warning:set_min_advice_level(1);
in_commandment_warning:set_suppress_pause_before_triggering(true);
...
in_war_warning:set_min_advice_level(2);
in_war_warning:set_suppress_pause_before_triggering(true);
...
in_technology_warning:set_min_advice_level(1);
in_technology_warning:set_suppress_pause_before_triggering(true);
...
in_public_order:set_min_advice_level(1);
...
function in_public_order_advice_play()
	...
	if not effect.get_advice_history_string_seen("prelude_public_order_advice") then		
	...
	effect.set_advice_history_string_seen("prelude_public_order_advice");
...
in_revolt_imminent:set_min_advice_level(1);
...
in_buildings_damaged:set_min_advice_level(1);
...
in_buildings_damaged:add_trigger_condition(
	...
	function(context)
		...
		if cm:model():world():whose_turn_is_it():name() ~= local_faction then 
			return false;
		end;
...
in_diplomacy_screen:set_min_advice_level(2);
...
in_diplomacy:set_min_advice_level(1);
...
function in_diplomacy_advice_process_context(diplomacy_context)
	...
	-- disable the override when the panel closes
	core:add_listener(
		"in_diplomacy_advice",
		"PanelClosedCampaign",
...
in_alliances:set_min_advice_level(1);
...
in_trade_agreement_sought:set_min_advice_level(1);
...
in_non_aggression_pacts:set_min_advice_level(1);
...
-- intervention declaration
--[[
in_character_skill_points = intervention:new(
	"character_skill_points",	 											-- string name
	0, 																		-- cost
	function() in_character_skill_points_trigger() end,						-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 											-- show debug output
);

in_character_skill_points:add_trigger_condition(
	"ScriptEventPlayerCharacterSkillPointAvailable",
	function(context)	
		-- return true, but store the character involved
		in_character_skill_points.char_cqi = context:character():cqi();
		
		return true;
	end
);

in_character_skill_points:set_min_advice_level(1);
in_character_skill_points:add_advice_key_precondition("war.camp.advice.character_skills.001");

in_character_skill_points:add_whitelist_event_type("character_rank_gainedevent_feed_target_character_faction");


function in_character_skill_points_trigger()
	
	-- work out if we have a character to zoom to
	local char_cqi = in_character_skill_points.char_cqi;
	local character = cm:get_character_by_cqi(char_cqi);
	
	-- don't do anything and reset this intervention if the character can't be found
	if not character then
		in_character_skill_points:cancel();
		return;
	end;
	
	local show_cutscene = true;
	
	if not character or not character:character_type("general") or not character:has_military_force() or character:military_force():is_armed_citizenry() then
		show_cutscene = false;
	end;
	
	-- Your subordinates gain experience and skills as they serve your cause, my Lord. Be sure to guide their development to your advantage.
	local advice_key = "war.camp.advice.character_skills.001";
	local infotext = {
		"war.camp.advice.character_skills.info_001", 
		"war.camp.advice.character_skills.info_002", 
		"war.camp.advice.character_skills.info_003",
		"war.camp.advice.character_skills.info_004"
	};
	
	if show_cutscene then
		in_character_skill_points:scroll_camera_to_character_for_intervention( 
			char_cqi,
			advice_key, 
			infotext
		);
	else
		in_character_skill_points:play_advice_for_intervention( 
			advice_key, 
			infotext
		);
	end;
...
in_region_rebels:set_min_advice_level(1);
...
in_hero_creation:set_min_advice_level(1);
...
in_movement_points_exhausted:add_trigger_condition(
	...
	function(context)
		...
		return character:faction():name() == cm:get_local_faction_name() and character:has_military_force() and character:action_points_remaining_percent() < 8;
	end
);

in_movement_points_exhausted:set_min_advice_level(1);
...
in_greenskins:set_min_advice_level(2);
...
in_empire:set_min_advice_level(2);
...
in_dwarfs:set_min_advice_level(2);
...
in_vampire_counts:set_min_advice_level(2);
...
in_vampire_counts:set_min_turn(3);
...
in_beastmen:set_min_advice_level(2);
...
in_money:set_min_advice_level(2);
...
in_near_bankruptcy:set_min_advice_level(1);
...
in_bankruptcy:set_min_advice_level(1);
...
in_hero_action_suffered:set_min_advice_level(1);
...
in_raise_forces:set_min_advice_level(1);
...
in_unit_recruitment:set_min_advice_level(2);
...
in_building_construction:set_min_advice_level(2);
...
in_stances:add_precondition(function() return not effect.get_advice_history_string_seen("has_adopted_stance") end);
in_stances:set_min_advice_level(1);
in_stances:set_min_turn(7);
...
in_ambush_stance:set_min_advice_level(2);
in_ambush_stance:add_precondition(function() return not effect.get_advice_history_string_seen("ambush_stance") end);
...
-- intervention declaration
in_growth = intervention:new(
	"growth",	 												-- string name
	35, 														-- cost
	function() in_growth_advice_trigger() end,					-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 								-- show debug output
);

in_growth:add_advice_key_precondition("war.camp.advice.growth.001");
in_growth:set_min_advice_level(1);

in_growth:add_trigger_condition(
	"ScriptEventPlayerRegionGainedDevelopmentPoint",
	function(context)
		if context:region():has_development_points_to_upgrade() then
			in_growth.region_name = context:region():name();
			return true;
		end;
		return false;
	end
...
function in_growth_advice_trigger()
	local region = cm:get_region(in_growth.region_name);
	
	if not region or region:owning_faction():name() ~= cm:get_local_faction_name() then
		-- failsafe
		in_growth:complete();
	--  No growth for Oak of Ages
	elseif region:name() ~= "wh_main_yn_edri_eternos_the_oak_of_ages" and region:name() ~= "wh_dlc05_oak_of_ages" then
		in_growth:scroll_camera_to_settlement_for_intervention(
			in_growth.region_name,
			-- Population growth within a province necessarily leads to an expansion of facilities, my lord. The cities here are ready for development.
			"war.camp.advice.growth.001",
			{
				"war.camp.advice.growth.info_001",
				"war.camp.advice.growth.info_002",
				"war.camp.advice.growth.info_003",
				"war.camp.advice.growth.info_004",
				"war.camp.advice.growth.info_005"
			}
		);
	end;
end;
...
-- intervention declaration
in_lord_dies = intervention:new(
	"lord_dies",					 															-- string name
	30, 																						-- cost
	function() in_lord_dies_trigger() end,														-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 																-- show debug output
);

in_lord_dies:set_min_advice_level(2);
in_lord_dies:add_advice_key_precondition("war.camp.advice.lords.001");
in_lord_dies:set_player_turn_only(false);
in_lord_dies:set_wait_for_battle_complete(false);
in_lord_dies:set_wait_for_fullscreen_panel_dismissed(false);

in_lord_dies:add_trigger_condition(
...
function in_lord_dies_trigger()
	in_lord_dies:play_advice_for_intervention(
		-- Your appointed Lord has fallen in battle, Sire. A replacement must be chosen to lead what remains of his army. Let us hope they don't meet with the same fate.
		"war.camp.advice.lords.001", 
		{
			"war.camp.advice.lords.info_001",
			"war.camp.advice.lords.info_002",
			"war.camp.advice.lords.info_003"
		}
	);
end;
...
-- intervention declaration
in_non_player_waaagh = intervention:new(
	"non_player_waaagh",					 												-- string name
	30, 																					-- cost
	function() in_non_player_waaagh_trigger() end,											-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 															-- show debug output
);

in_non_player_waaagh:set_min_advice_level(1);
in_non_player_waaagh:add_advice_key_precondition("war.camp.advice.waaagh.002");

in_non_player_waaagh:add_trigger_condition(
	"ScriptEventNonPlayerCharacterWaaaghOccurred",
	function(context)
		local waaagh_char = context:character();
		if waaagh_char:faction():culture() == "wh_main_grn_greenskins" and waaagh_char:is_visible_to_faction(cm:get_local_faction_name()) then	
			in_non_player_waaagh.char_cqi = waaagh_char:cqi();
			return true;
		end;
		return false;
	end
...
function in_non_player_waaagh_trigger()
	local char_cqi = in_non_player_waaagh.char_cqi;
		
	-- A Waaagh! my lord! These Greenskins have worked themselves into such a frenzy that their foul kin travel from far and wide to join their banner.
	local advice_key = "war.camp.advice.waaagh.002";
	local infotext = {
	...
	in_non_player_waaagh:scroll_camera_to_character_for_intervention( 
		char_cqi,
		advice_key,
		infotext
	);
...
-- intervention declaration
in_non_player_brayherd = intervention:new(
	"non_player_brayherd",					 												-- string name
	30, 																					-- cost
	function() in_non_player_brayherd_trigger() end,										-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 															-- show debug output
);

in_non_player_brayherd:set_min_advice_level(1);
in_non_player_brayherd:add_advice_key_precondition("war.camp.advice.brayherds.001");

in_non_player_brayherd:add_trigger_condition(
	"ScriptEventNonPlayerCharacterWaaaghOccurred",
	function(context)
		local brayherd_char = context:character();
		if brayherd_char:faction():culture() == "wh_dlc03_bst_beastmen" and brayherd_char:is_visible_to_faction(cm:get_local_faction_name()) then	
			in_non_player_brayherd.char_cqi = brayherd_char:cqi();
			return true;
		end;
		return false;
	end
...
function in_non_player_brayherd_trigger()
	local char_cqi = in_non_player_brayherd.char_cqi;
		
	-- My Lord, a brayherd has been sighted! These Beastmen have caught the scent of blood and begun to stampede in great numbers - be wary of crossing their path!
	local advice_key = "war.camp.advice.brayherds.001";
	local infotext = {
		"war.camp.advice.brayherds.info_001",
		"war.camp.advice.brayherds.info_002", 
		"war.camp.advice.brayherds.info_003"
	};
	
	in_non_player_brayherd:scroll_camera_to_character_for_intervention( 
		char_cqi,
		advice_key,
		infotext
	);
...
-- intervention declaration
in_strategic_overview = intervention:new(
	"strategic_overview",					 											-- string name
	60, 																				-- cost
	function() in_strategic_overview_trigger() end,										-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 														-- show debug output
);

in_strategic_overview:set_min_advice_level(2);
in_strategic_overview:add_precondition(function() return not uim:get_interaction_monitor_state("strategic_overview_opened") end);
in_strategic_overview:add_advice_key_precondition("war.camp.advice.strategic_map.001");
in_strategic_overview:set_min_turn(11);

in_strategic_overview:add_trigger_condition(
	"ScriptEventPlayerFactionTurnStart",
	function(context)
		return true;
	end
...
function in_strategic_overview_trigger()
	
	-- A Waaagh! my lord! These Greenskins have worked themselves into such a frenzy that their foul kin travel from far and wide to join their banner.
	local advice_key = "war.camp.advice.strategic_map.001";
	local infotext = {
		"war.camp.advice.strategic_map.info_001",
		"war.camp.advice.strategic_map.info_002", 
		"war.camp.advice.strategic_map.info_003"
	};
	
	in_strategic_overview:play_advice_for_intervention( 
		advice_key, 
		infotext
	);
end;
...
-- intervention declaration
in_lord_creation = intervention:new(
	"lord_creation",	 													-- string name
	0, 																		-- cost
	function() in_lord_creation_trigger() end,								-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 											-- show debug output
);


in_lord_creation:add_trigger_condition(
...
in_lord_creation:set_min_advice_level(2);
in_lord_creation:add_advice_key_precondition("war.camp.advice.lords.002");
in_lord_creation:add_precondition(function() return not uim:get_interaction_monitor_state("force_raised") end);

function in_lord_creation_trigger()
	-- A new army has been raised under the command of your appointed Lord. It only remains for you to issue them orders, Sire.
	local advice_key = "war.camp.advice.lords.002";
	local infotext = {
		"war.camp.advice.lords.info_001",
		"war.camp.advice.lords.info_002", 
		"war.camp.advice.lords.info_003"
	...
	in_lord_creation:play_advice_for_intervention( 
		advice_key, 
		infotext
	);
end;
...
in_enemy_reinforcements:set_min_advice_level(1);
...
in_player_reinforcements:set_min_advice_level(2);
...
in_enemy_raiding:set_min_advice_level(1);
...
in_unit_exchange:set_min_advice_level(2);
...
in_unit_exchange:set_min_turn(12);
...
in_raiding_stance_possible:set_min_advice_level(2);
...
in_archaon_encountered:set_min_advice_level(1);
...
in_archaon_defeated:set_min_advice_level(1);
...
in_archaon_defeated:set_suppress_pause_before_triggering(true);		-- do this so we get no grace period
...
in_player_besieged:set_min_advice_level(1);
...
in_enemy_port:set_min_advice_level(2);
in_enemy_port:add_advice_key_precondition("war.camp.advice.ports.001");
in_enemy_port:set_min_turn(10);

in_enemy_port:add_trigger_condition(
	"ScriptEventPlayerFactionTurnStart",
	function(context)	
		-- return true if the garrison commander of any port that is controlled by an enemy of the player is visible to the player, false otherwise	
		local player_faction = cm:get_faction(cm:get_local_faction_name());
		...
		for i = 0, war_list:num_items() - 1 do
			...
			for j = 0, region_list:num_items() - 1 do
				...
				if not current_region:is_abandoned() then
					...
					if is_settlement(settlement) and settlement:is_port() then
						...
						if gr:has_army() and gr:army():has_general() then
							if gr:can_be_occupied_by_faction(cm:get_local_faction_name()) and gr:army():general_character():is_visible_to_faction(cm:get_local_faction_name()) then
								in_enemy_port.region_key = current_region:name();
								return true;
							end;
						end;
					end;
...
in_player_army_near_coast:set_min_advice_level(2);
...
in_resources:set_min_advice_level(2);
...
in_settlement_climate:set_min_advice_level(1);
...
in_winds_of_magic_changed:set_min_advice_level(2);
...
in_enemy_province_capital:set_min_advice_level(2);
...
in_enemy_province_capital:add_trigger_condition(
	"ScriptEventPlayerFactionTurnStart",
	function(context)		
		-- return true if an enemy holds a province capital
		local player_faction = cm:get_faction(cm:get_local_faction_name());
		local war_list = player_faction:factions_at_war_with();	
		for i = 0, war_list:num_items() - 1 do
			local current_faction = war_list:item_at(i);
			...
			for j = 0, region_list:num_items() - 1 do
				...
				if current_region:is_province_capital() then
					...
					if current_gr:has_army() and current_gr:army():has_general() and current_gr:army():general_character():is_visible_to_faction(cm:get_local_faction_name()) then
						in_enemy_province_capital.region_key = current_region:name();
						return true;
					end;
				end;
			end;
...
in_zones_of_control:set_min_advice_level(2);
...
in_hordes:set_min_advice_level(1);
...
in_attrition_chaos:set_min_advice_level(1);
-- in_attrition_chaos:set_suppress_pause_before_triggering(true);		-- do this so we get no grace period
-- in_attrition_chaos:add_advice_key_precondition("war.camp.advice.corruption.005");

in_attrition_chaos:add_precondition(
	function()
		return not effect.get_advice_history_string_seen("chaos_attrition");
	end
);
...
in_attrition_chaos:add_trigger_condition(
	...
	function(context)
		...
		if character:faction():name() == cm:get_local_faction_name() and character:has_region() and character:action_points_remaining_percent() > 0 then			
			if character:region():majority_religion() == chaos_corruption_string then
...
function in_attrition_chaos_trigger()
	...
	effect.set_advice_history_string_seen("chaos_attrition");
...
in_attrition_vampire:set_min_advice_level(1);
-- in_attrition_vampire:set_suppress_pause_before_triggering(true);		-- do this so we get no grace period
in_attrition_vampire:add_advice_key_precondition("war.camp.advice.corruption.005");

in_attrition_vampire:add_precondition(
	function()
		return not effect.get_advice_history_string_seen("vampire_attrition");
	end
);
...
in_attrition_vampire:add_trigger_condition(
	...
	function(context)
		local character = context:character();
		if character:faction():name() == cm:get_local_faction_name() and character:has_region() and character:action_points_remaining_percent() > 0 then			
			if character:region():majority_religion() == vampiric_corruption_string then
...
function in_attrition_vampire_trigger()
	...
	effect.set_advice_history_string_seen("vampire_attrition");
...
in_attrition_general:set_min_advice_level(1);
--in_attrition_general:set_suppress_pause_before_triggering(true);		-- do this so we get no grace period
...
in_attrition_general:add_trigger_condition(
	...
	function(context)
		...
		if character:faction():name() == cm:get_local_faction_name() and character:action_points_remaining_percent() > 0 and character:faction():treasury() > 0 then
			...
			local region_corruption = region:majority_religion();
			...
			elseif faction:culture() == "wh_main_chs_chaos" or faction:culture() == "wh_dlc03_bst_beastmen" then
				if region_corruption == chaos_corruption_string then
					in_attrition_general.char_cqi = character:cqi();
					return true;
				end;
			
			-- otherwise return true if the char is in untainted territory
			else
				if region_corruption == untainted_corruption_string then
					in_attrition_general.char_cqi = character:cqi();
					return true;
				end;
			end;
...
in_vmp_attrition_untainted:set_min_advice_level(1);
-- in_vmp_attrition_untainted:set_suppress_pause_before_triggering(true);		-- do this so we get no grace period
...
in_vmp_attrition_untainted:add_trigger_condition(
	...
	function(context)
		...
		if character:faction():name() == cm:get_local_faction_name() and character:has_region() and character:region():majority_religion() == untainted_corruption_string then
...
in_enemy_threaten_settlement:set_min_advice_level(2);
...
in_foreign_hero:set_min_advice_level(2);
...
in_global_recruitment:set_min_advice_level(2);
...
in_horde_growth:set_min_advice_level(1);
...
---------------------------------------------------------------
--
--	player creates chaos rift
--
---------------------------------------------------------------

-- intervention declaration
in_player_creates_chaos_rift = intervention:new(
	"player_creates_chaos_rift",	 											-- string name
	30, 																		-- cost
	function() in_player_creates_chaos_rift_trigger() end,						-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 												-- show debug output
);

in_player_creates_chaos_rift:set_min_advice_level(1);
in_player_creates_chaos_rift:add_advice_key_precondition("war.camp.advice.chaos_rifts.002");


in_player_creates_chaos_rift:add_trigger_condition(
	"RegionAbandonedWithBuildingEvent",
	function(context)
		local retval = (context:abandoning_faction():name() == cm:get_local_faction_name());
		if retval then
			in_player_creates_chaos_rift.region_name = context:building():region():name();			
			return true;
		end;
		return false;
	end
...
function in_player_creates_chaos_rift_trigger()
	local region_name = in_player_creates_chaos_rift.region_name;
	
	-- Your warriors have raised an artefact of worship to the Dark Gods in the rubble of this wretched place. It spreads the taint of Chaos across the land, mighty Lord, weakening the resolve of those that might stand against you.
	local advice_key = "war.camp.advice.chaos_rifts.002";
	local infotext = {
		"war.camp.advice.chaos_artefacts.info_001",
		"war.camp.advice.chaos_artefacts.info_002",
		"war.camp.advice.chaos_artefacts.info_003"
	};

	if region_name then
		in_player_creates_chaos_rift:scroll_camera_to_settlement_for_intervention(
			region_name,			
			advice_key, 
			infotext
		);
	else
		in_player_creates_chaos_rift:play_advice_for_intervention(
			advice_key,
			infotext
		);
	end;
...
---------------------------------------------------------------
--
--	enemy creates chaos rift
--
---------------------------------------------------------------

-- intervention declaration
in_enemy_creates_chaos_rift = intervention:new(
	"enemy_creates_chaos_rift",	 												-- string name
	30, 																		-- cost
	function() in_enemy_creates_chaos_rift_trigger() end,						-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 												-- show debug output
);

in_enemy_creates_chaos_rift:set_min_advice_level(1);
in_enemy_creates_chaos_rift:add_advice_key_precondition("war.camp.advice.chaos_rifts.002");
in_enemy_creates_chaos_rift:add_advice_key_precondition("war.camp.advice.chaos_rifts.001");


in_enemy_creates_chaos_rift:add_trigger_condition(
	"RegionAbandonedWithBuildingEvent",
	function(context)
		-- return true if the artefact was created by someone other than the player in a settlement visible to the player
		if context:abandoning_faction():name() ~= cm:get_local_faction_name() then
			local player_faction = cm:get_faction(cm:get_local_faction_name());
			local region = context:building():region();
			local garrison_commander = cm:get_garrison_commander_of_region(region);
			
			if garrison_commander and garrison_commander:is_visible_to_faction(player_faction) then
				in_enemy_creates_chaos_rift.region_name = region:name();
				return true;
			end;
		end;
...
function in_enemy_creates_chaos_rift_trigger()
	local region_name = in_enemy_creates_chaos_rift.region_name;
	
	-- The despicable armies of darkness have assaulted this place, slaughtering all they can find and razing the town to dust. They have left an artefact to spread the foul taint of Chaos across the land. Destroy it!
	local advice_key = "war.camp.advice.chaos_rifts.001";
	local infotext = {
		"war.camp.advice.chaos_artefacts.info_001",
		"war.camp.advice.chaos_artefacts.info_002",
		"war.camp.advice.chaos_artefacts.info_003"
	...
	if region_name then
		in_enemy_creates_chaos_rift:scroll_camera_to_settlement_for_intervention(
			region_name,			
			advice_key, 
			infotext
		);
	else
		in_enemy_creates_chaos_rift:play_advice_for_intervention(
			advice_key,
			infotext
		);
	end;
...
in_elven_council:set_min_advice_level(1);
...
function in_elven_council_trigger()
	local faction_name = cm:model():world():whose_turn_is_it():name()
...
in_amber:set_min_advice_level(2);
...
in_negative_amber:set_min_advice_level(1);
...
in_oak_of_ages:set_min_advice_level(1);
...
in_oak_of_ages:add_trigger_condition(
	"ScriptEventPlayerFactionTurnStart",
	function()
		local oak_of_ages_region_name = "wh_main_yn_edri_eternos_the_oak_of_ages";
...
in_oak_of_ages:add_trigger_condition(
	...
	function(context)
		...
		return (region_name == "wh_main_yn_edri_eternos_the_oak_of_ages" or region_name == "wh_dlc05_oak_of_ages") and region:owning_faction() == player_faction and cm:model():world():whose_turn_is_it() == player_faction;
	end
);
...
in_oak_of_ages_construction_2:set_min_advice_level(1);
...
in_oak_of_ages_construction_3:set_min_advice_level(1);
...
in_oak_of_ages_construction_4:set_min_advice_level(1);
...
in_oak_of_ages_construction_5:set_min_advice_level(1);
...
in_elven_halls:set_min_advice_level(1);
...
in_asrai_lookouts:set_min_advice_level(1);
...
in_asrai_lookout_constructed:set_min_advice_level(1);
...
in_asrai_lookout_lost:set_min_advice_level(1);
...
in_worldroots:set_min_advice_level(1);
...
in_wef_final_battle:set_min_advice_level(2);
...
in_wood_elves_racial_advice:set_min_advice_level(1);
...
in_peasant_recruited:set_min_advice_level(1);
...
in_peasant_negative:set_min_advice_level(1);
...
in_peasant_positive:set_min_advice_level(1);
...
in_add_blessing_lady:set_min_advice_level(1);
...
in_remove_blessing_lady:set_min_advice_level(1);
...
in_first_chivalry_gained:set_min_advice_level(1);
...
in_first_chivalry_lost:set_min_advice_level(1);
...
in_chivalry_negative:set_min_advice_level(1);
...
in_chivalry_negative:add_trigger_condition(
	"ScriptEventFoodValueDown",
	function()
		if cm:model():world():whose_turn_is_it():is_human() and  cm:model():world():whose_turn_is_it():total_food() < 0 then
			return true;
		end
	end
);
...
in_chivalry_level_up:set_min_advice_level(1);
...
in_chivalry_level_up_repanse:set_min_advice_level(1);
...
in_chivalry_level_down:set_min_advice_level(1);
...
in_green_knight_intro:set_min_advice_level(1);
...
in_green_knight_intro:add_trigger_condition(
	...
	function()	
		if cm:model():world():whose_turn_is_it():is_human() and  cm:model():world():whose_turn_is_it():total_food() >= 100 then
			return true;
		end
	end
);
...
in_green_knight_available:set_min_advice_level(1);
...
in_green_knight_available:add_trigger_condition(
	"ScriptEventFoodLevelUp",
	function()
		if cm:model():world():whose_turn_is_it():is_human() and  cm:model():world():whose_turn_is_it():total_food() >= 400 then
			return true;
		end
	end
);
...
in_green_knight_summon:set_min_advice_level(1);
...
in_green_knight_fade:set_min_advice_level(1);
...
in_errantry_war_dilemma:set_min_advice_level(1);
...
in_errantry_war_dilemma_repanse:set_min_advice_level(1);
...
in_bretonnia:set_min_advice_level(2);
...
in_brt_racial_advice:set_min_advice_level(1);
...
in_hef_racial_advice:set_min_advice_level(1);
...
in_high_elves:set_min_advice_level(2);
...
in_def_racial_advice:set_min_advice_level(1);
...
in_dark_elves:set_min_advice_level(2);
...
in_lzd_racial_advice:set_min_advice_level(1);
...
in_lizardmen:set_min_advice_level(2);
...
in_skv_racial_advice:set_min_advice_level(1);
...
in_skaven:set_min_advice_level(2);
...
in_tmb_racial_advice:set_min_advice_level(1);
...
in_tomb_kings:set_min_advice_level(2);
...
in_hef_influence_advice:set_min_advice_level(1);
...
in_hef_intrigue_at_the_court_advice:set_min_advice_level(1);
...
in_skv_skaven_underworld_advice:set_min_advice_level(1);
...
in_skv_skaven_food_advice:set_min_advice_level(1);
...
in_skv_stalking_advice:set_min_advice_level(1);
...
in_lzd_astromancy_advice:set_min_advice_level(1);
...
in_lzd_geomantic_web_advice:set_min_advice_level(1);
...
in_rites:set_min_advice_level(1);
...
in_rite_pestilent_scheme:set_min_advice_level(1);
...
in_rite_doom:set_min_advice_level(1);
...
in_def_black_arks:set_min_advice_level(1);
...
in_loyalty:set_min_advice_level(1);
...
in_low_loyalty:set_min_advice_level(1);
...
in_civil_war:set_min_advice_level(1);
...
in_corruption_chaos:set_min_advice_level(1);
...
in_corruption_vampires:set_min_advice_level(1);
...
in_corruption_untainted:set_min_advice_level(1);
...
in_corruption_skaven:set_min_advice_level(1);
...
in_skv_skaven_corruption:set_min_advice_level(1);
...
in_skv_skaven_corruption:add_trigger_condition(
	...
	function(context)
		...
		for i = 0, region_list:num_items() - 1 do
			local current_region = region_list:item_at(i);
			local current_region_home_religion_amount = current_region:religion_proportion("wh2_main_religion_skaven");
			
			if current_region_home_religion_amount > 0.35 then
...
function in_skv_skaven_corruption_advice_trigger()
	local region = cm:get_region(in_skv_skaven_corruption.region_name);
	
	if not region or region:owning_faction():name() ~= cm:get_local_faction_name() then	
		in_skv_skaven_corruption:play_advice_for_intervention(
			advice,
			infotext
		);
	else
...
in_multi_turn_recruitment:set_min_advice_level(1);
...
in_black_arks:set_min_advice_level(1);
...
in_skv_menace_below:set_min_advice_level(1);
...
in_skaven_underworld:set_min_advice_level(1);
...
in_plague:set_min_advice_level(1);
in_plague:add_advice_key_precondition("wh2.camp.advice.plagues.001");

in_plague:add_trigger_condition(
	"ForcePlagueStateChanged",
	function(context)	
		-- return true if this is a military force belonging to the player
		
		if context:is_infected() and context:military_force():faction():name() == cm:get_local_faction_name() then
			if context:military_force():has_general() then
				in_plague.char_cqi = context:military_force():general_character():cqi();
			end;
			return true;
		end;
		return false;
...
in_plague:add_trigger_condition(
	...
	function(context)	
		...
		if context:is_infected() and context:region():owning_faction():name() == cm:get_local_faction_name() then
			in_plague.region_name = context:region():name();
			return true;
		end;
		return false;
	end
...
in_death_night_turn_one:set_min_advice_level(1);
...
in_death_night_respected:set_min_advice_level(1);
in_death_night_respected:set_disregard_cost_when_triggering(true);
...
in_death_night_depleted:set_min_advice_level(1);
in_death_night_depleted:set_disregard_cost_when_triggering(true);
...
in_death_night_ready:set_min_advice_level(1);
in_death_night_ready:set_disregard_cost_when_triggering(true);
...
in_death_night_triggered:set_min_advice_level(1);
in_death_night_triggered:set_disregard_cost_when_triggering(true);
...
in_death_night_morathi:set_min_advice_level(1);
in_death_night_morathi:set_disregard_cost_when_triggering(true);
...
in_death_night_alarielle:set_min_advice_level(1);
in_death_night_alarielle:set_disregard_cost_when_triggering(true);
...
in_blood_voyage_spawned:set_min_advice_level(1);
in_blood_voyage_spawned:set_disregard_cost_when_triggering(true);
...
in_blood_voyage_destroyed:set_min_advice_level(1);
in_blood_voyage_destroyed:set_disregard_cost_when_triggering(true);
...
in_sword_of_khaine_available:set_min_advice_level(1);
in_sword_of_khaine_available:set_disregard_cost_when_triggering(true);
...
in_sword_of_khaine_player_claimed:set_min_advice_level(1);
in_sword_of_khaine_player_claimed:set_disregard_cost_when_triggering(true);
...
in_sword_of_khaine_ai_claimed:set_min_advice_level(1);
in_sword_of_khaine_ai_claimed:set_disregard_cost_when_triggering(true);
...
in_sword_of_khaine_stuck:set_min_advice_level(1);
in_sword_of_khaine_stuck:set_disregard_cost_when_triggering(true);
...
in_sword_of_khaine_returned:set_min_advice_level(1);
in_sword_of_khaine_returned:set_disregard_cost_when_triggering(true);
...
in_sword_of_khaine_dilemma:set_min_advice_level(1);
in_sword_of_khaine_dilemma:set_disregard_cost_when_triggering(true);
...
in_assassination_targets_first_targets:set_min_advice_level(1);
in_assassination_targets_first_targets:set_disregard_cost_when_triggering(true);
...
in_assassination_targets_first_targets:add_trigger_condition(
	"ScriptEventAssassinationFirstTargets",
	function()
		return true;
	end
);
...
in_assassination_targets_first_kill:set_min_advice_level(1);
in_assassination_targets_first_kill:set_disregard_cost_when_triggering(true);
...
in_assassination_targets_kill_all_targets:set_min_advice_level(1);
in_assassination_targets_kill_all_targets:set_disregard_cost_when_triggering(true);
...
in_assassination_targets_new_targets:set_min_advice_level(1);
in_assassination_targets_new_targets:set_disregard_cost_when_triggering(true);
...
in_protector_of_ulthuan_inner_gained:set_min_advice_level(1);
in_protector_of_ulthuan_inner_gained:set_disregard_cost_when_triggering(true);
...
in_protector_of_ulthuan_inner_lost:set_min_advice_level(1);
in_protector_of_ulthuan_inner_lost:set_disregard_cost_when_triggering(true);
...
in_protector_of_ulthuan_unity:set_min_advice_level(1);
in_protector_of_ulthuan_unity:set_disregard_cost_when_triggering(true);
...
in_protector_of_ulthuan_outer_lost:set_min_advice_level(1);
in_protector_of_ulthuan_outer_lost:set_disregard_cost_when_triggering(true);
...
in_power_of_nature:set_min_advice_level(1);
in_power_of_nature:set_disregard_cost_when_triggering(true);
...
in_mortal_worlds_torment_stage_one:set_min_advice_level(1);
in_mortal_worlds_torment_stage_one:set_disregard_cost_when_triggering(true);
...
in_mortal_worlds_torment_stage_two:set_min_advice_level(1);
in_mortal_worlds_torment_stage_two:set_disregard_cost_when_triggering(true);
...
in_mortal_worlds_torment_stage_three:set_min_advice_level(1);
in_mortal_worlds_torment_stage_three:set_disregard_cost_when_triggering(true);
...
in_treasure_map_mission:set_min_advice_level(1);
in_treasure_map_mission:set_disregard_cost_when_triggering(true);
...
in_full_treasure_map_log:set_min_advice_level(2);
in_full_treasure_map_log:set_disregard_cost_when_triggering(true);
...
in_first_treasure_map_mission_succeeded:set_min_advice_level(2);
in_first_treasure_map_mission_succeeded:set_disregard_cost_when_triggering(true);
...
in_fifth_treasure_map_mission_succeeded:set_min_advice_level(2);
in_fifth_treasure_map_mission_succeeded:set_disregard_cost_when_triggering(true);
...
in_treasure_not_found:set_min_advice_level(2);
in_treasure_not_found:set_disregard_cost_when_triggering(true);
...
in_ulthuan_monster_first_attack:set_min_advice_level(1);
in_ulthuan_monster_first_attack:set_disregard_cost_when_triggering(true);
...
in_ulthuan_monster_second_attack:set_min_advice_level(2);
in_ulthuan_monster_second_attack:set_disregard_cost_when_triggering(true);
...
in_ulthuan_monster_attacks_player_settlement:set_min_advice_level(2);
in_ulthuan_monster_attacks_player_settlement:set_disregard_cost_when_triggering(true);
...
in_ulthuan_monster_under_player_control:set_min_advice_level(1);
in_ulthuan_monster_under_player_control:set_disregard_cost_when_triggering(true);
...
in_player_infamy_grows:set_min_advice_level(1);
...
in_player_climbs_infamy_list:set_min_advice_level(1);
...
in_player_tops_infamy_list:set_min_advice_level(1);
...
---------------------------------------------------------------
--
--	First Shanty Pirate Attacks
--
---------------------------------------------------------------


-- intervention declaration
in_first_shanty_pirate_attacks = intervention:new(
	"in_first_shanty_pirate_attacks",					 									-- string name
	10,	 																					-- cost
	function() trigger_in_first_shanty_pirate_attacks() end,								-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 															-- show debug output
);

in_first_shanty_pirate_attacks:set_min_advice_level(1);
in_first_shanty_pirate_attacks:set_disregard_cost_when_triggering(true);
in_first_shanty_pirate_attacks:add_advice_key_precondition("wh2_dlc11.camp.advice.cst.shanty.001");
in_first_shanty_pirate_attacks:add_whitelist_event_type("faction_event_mission_issuedevent_feed_target_mission_faction");

in_first_shanty_pirate_attacks:add_trigger_condition(
	"ScriptEventFirstShantyPirateAttacks",
	function(context)
		return true;
	end
);

function trigger_in_first_shanty_pirate_attacks()
	
	in_first_shanty_pirate_attacks:play_advice_for_intervention(
		-- Another has called you out. Admiral Houghhoff looks to silence your cannons and ambitions, for he has the power of song and the sea at his very disposal!
		"wh2_dlc11.camp.advice.cst.shanty.001",
		{
			"dlc11.camp.advice.sea_shanties.info_001",
			"dlc11.camp.advice.sea_shanties.info_002",
			"dlc11.camp.advice.sea_shanties.info_004",
			"dlc11.camp.advice.sea_shanties.info_005"
		}
	);
end;
...
---------------------------------------------------------------
--
--	First Shanty Pirate Defeated
--
---------------------------------------------------------------


-- intervention declaration
in_first_shanty_pirate_defeated = intervention:new(
	"in_first_shanty_pirate_defeated",					 									-- string name
	10,	 																					-- cost
	function() trigger_in_first_shanty_pirate_defeated() end,								-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 															-- show debug output
);

in_first_shanty_pirate_defeated:set_min_advice_level(2);
in_first_shanty_pirate_defeated:set_disregard_cost_when_triggering(true);
in_first_shanty_pirate_defeated:add_advice_key_precondition("wh2_dlc11.camp.advice.cst.shanty.002");
in_first_shanty_pirate_defeated:add_whitelist_event_type("faction_event_mission_successevent_feed_target_mission_faction");

in_first_shanty_pirate_defeated:add_trigger_condition(
	"ScriptEventFirstShantyPirateDefeated",
	function(context)
		return true;
	end
);

function trigger_in_first_shanty_pirate_defeated()
	
	in_first_shanty_pirate_defeated:play_advice_for_intervention(
		-- To the bottom of the sea they go! Their Sea Shanty is now yours to wield and you are a step closer to facing Amanar.
		"wh2_dlc11.camp.advice.cst.shanty.002",
		{
			"dlc11.camp.advice.sea_shanties.info_001",
			"dlc11.camp.advice.sea_shanties.info_002",
			"dlc11.camp.advice.sea_shanties.info_003"
		}
	);
end;
...
---------------------------------------------------------------
--
--	Second Shanty Pirate Attacks
--
---------------------------------------------------------------


-- intervention declaration
in_second_shanty_pirate_attacks = intervention:new(
	"in_second_shanty_pirate_attacks",					 									-- string name
	10,	 																					-- cost
	function() trigger_in_second_shanty_pirate_attacks() end,								-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 															-- show debug output
);

in_second_shanty_pirate_attacks:set_min_advice_level(1);
in_second_shanty_pirate_attacks:set_disregard_cost_when_triggering(true);
in_second_shanty_pirate_attacks:add_advice_key_precondition("wh2_dlc11.camp.advice.cst.shanty.003");
in_second_shanty_pirate_attacks:add_whitelist_event_type("faction_event_mission_issuedevent_feed_target_mission_faction");

in_second_shanty_pirate_attacks:add_trigger_condition(
	"ScriptEventSecondShantyPirateAttacks",
	function(context)
		return true;
	end
);

function trigger_in_second_shanty_pirate_attacks()
	
	in_second_shanty_pirate_attacks:play_advice_for_intervention(
		-- These seas harbour many infamous pirates. The mighty Wavebreaker has been spotted, admiral, its captain looks to sink you with haste.
		"wh2_dlc11.camp.advice.cst.shanty.003",
		{
			"dlc11.camp.advice.sea_shanties.info_001",
			"dlc11.camp.advice.sea_shanties.info_002",
			"dlc11.camp.advice.sea_shanties.info_004",
...
---------------------------------------------------------------
--
--	Second Shanty Pirate Defeated
--
---------------------------------------------------------------


-- intervention declaration
in_second_shanty_pirate_defeated = intervention:new(
	"in_second_shanty_pirate_defeated",					 									-- string name
	10,	 																					-- cost
	function() trigger_in_second_shanty_pirate_defeated() end,								-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 															-- show debug output
);

in_second_shanty_pirate_defeated:set_min_advice_level(2);
in_second_shanty_pirate_defeated:set_disregard_cost_when_triggering(true);
in_second_shanty_pirate_defeated:add_advice_key_precondition("wh2_dlc11.camp.advice.cst.shanty.004");
in_second_shanty_pirate_defeated:add_whitelist_event_type("faction_event_mission_successevent_feed_target_mission_faction");

in_second_shanty_pirate_defeated:add_trigger_condition(
	"ScriptEventSecondShantyPirateDefeated",
	function(context)
		return true;
	end
);

function trigger_in_second_shanty_pirate_defeated()
	
	in_second_shanty_pirate_defeated:play_advice_for_intervention(
		-- The second Shanty is now yours to command – its strength coupled with the first will strike a mighty blow! Yet to summon the Merwyrm a third must be first discovered...
		"wh2_dlc11.camp.advice.cst.shanty.004",
		{
			"dlc11.camp.advice.sea_shanties.info_001",
			"dlc11.camp.advice.sea_shanties.info_002",
			"dlc11.camp.advice.sea_shanties.info_003"
...
---------------------------------------------------------------
--
--	Third Shanty Pirate Attacks
--
---------------------------------------------------------------


-- intervention declaration
in_third_shanty_pirate_attacks = intervention:new(
	"in_third_shanty_pirate_attacks",					 									-- string name
	10,	 																					-- cost
	function() trigger_in_third_shanty_pirate_attacks() end,								-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 															-- show debug output
);

in_third_shanty_pirate_attacks:set_min_advice_level(1);
in_third_shanty_pirate_attacks:set_disregard_cost_when_triggering(true);
in_third_shanty_pirate_attacks:add_advice_key_precondition("wh2_dlc11.camp.advice.cst.shanty.005");
in_third_shanty_pirate_attacks:add_whitelist_event_type("faction_event_mission_issuedevent_feed_target_mission_faction");

in_third_shanty_pirate_attacks:add_trigger_condition(
	"ScriptEventThirdShantyPirateAttacks",
	function(context)
		return true;
	end
);

function trigger_in_third_shanty_pirate_attacks()
	
	in_third_shanty_pirate_attacks:play_advice_for_intervention(
		-- Barron von Heastie, admiral of the Red Dragon, beckons you to combat. He will test your seamanship and arsenal, but the prize that awaits holds the key to defeating Amanar.
		"wh2_dlc11.camp.advice.cst.shanty.005",
		{
			"dlc11.camp.advice.sea_shanties.info_001",
			"dlc11.camp.advice.sea_shanties.info_002",
			"dlc11.camp.advice.sea_shanties.info_004",
...
---------------------------------------------------------------
--
--	Third Shanty Pirate Defeated
--
---------------------------------------------------------------


-- intervention declaration
in_third_shanty_pirate_defeated = intervention:new(
	"in_third_shanty_pirate_defeated",					 									-- string name
	10,	 																					-- cost
	function() trigger_in_third_shanty_pirate_defeated() end,								-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 															-- show debug output
);

in_third_shanty_pirate_defeated:set_min_advice_level(2);
in_third_shanty_pirate_defeated:set_disregard_cost_when_triggering(true);
in_third_shanty_pirate_defeated:add_advice_key_precondition("wh2_dlc11.camp.advice.cst.shanty.006");
in_third_shanty_pirate_defeated:add_whitelist_event_type("faction_event_mission_successevent_feed_target_mission_faction");

in_third_shanty_pirate_defeated:add_trigger_condition(
	"ScriptEventThirdShantyPirateDefeated",
	function(context)
		return true;
	end
);

function trigger_in_third_shanty_pirate_defeated()
	
	in_third_shanty_pirate_defeated:play_advice_for_intervention(
		-- The power of the seas can now unite as one and curse Wulfhart's harpoon with the strength needed to fell Amanar.
		"wh2_dlc11.camp.advice.cst.shanty.006",
		{
			"dlc11.camp.advice.sea_shanties.info_001",
			"dlc11.camp.advice.sea_shanties.info_002",
			"dlc11.camp.advice.sea_shanties.info_003"
...
in_rogue_army_sighted:set_min_advice_level(1);
...
in_pieces_of_eight_tab_opened:set_min_advice_level(1);
...
in_first_piece_of_eight_collected:set_min_advice_level(1);
...
in_fourth_piece_of_eight_collected:set_min_advice_level(1);
...
in_all_pieces_of_eight_collected:set_min_advice_level(1);
...
in_post_normal_battle_pirate_loyalty:set_min_advice_level(1);
...
in_low_pirate_loyalty:set_min_advice_level(1);
...
in_pirate_civil_war:set_min_advice_level(1);
...
in_fleet_office:set_min_advice_level(1);
...
in_post_siege_battle_pirate_cove:set_min_advice_level(1);
...
in_pre_island_battle_options:set_min_advice_level(2);
...
in_puzzle_dice:set_min_advice_level(2);
...
in_puzzle_clock:set_min_advice_level(2);
...
in_puzzle_numbers:set_min_advice_level(2);
...
in_puzzle_odd_one_out:set_min_advice_level(2);
...
in_sacrifices_to_sotek_post_battle_option:set_min_advice_level(1);
...
in_sacrifices_to_sotek_pooled_increase:set_min_advice_level(1);
...
in_sacrifices_to_sotek_pooled_increase:add_trigger_condition(
	...
	function(context)
		...
		for i = 0, context:faction():pooled_resources():num_items() - 1 do
			
			if context:faction():pooled_resources():item_at(i):key() == "lzd_sacrificial_offerings" then
			
				if context:faction():pooled_resources():item_at(i):value() > 0 then
					return true;
				end;
			end;
		end;
...
in_sacrifices_to_sotek_sacrifice_ready:set_min_advice_level(1);
	...
	function(context)
		for i = 0, context:faction():pooled_resources():num_items() - 1 do
			...
			if context:faction():pooled_resources():item_at(i):key() == "lzd_sacrificial_offerings" then
				...
				if context:faction():pooled_resources():item_at(i):value() >= 100 then
					return true;
				end;
			end;
		end;
...
in_sacrifices_to_sotek_sotek_available:set_min_advice_level(1);
	...
	function(context)
		...
		for i = 0, context:faction():pooled_resources():num_items() - 1 do
			...
			if context:faction():pooled_resources():item_at(i):key() == "lzd_sacrificial_offerings" then
				...
				if (context:faction():pooled_resources():item_at(i):value() >= 1000) and (cm:get_saved_value("pos_3_1")) then
					return true;
				end
			end;
		end;		
...
in_sacrifices_to_sotek_unlocked_tier_2:set_min_advice_level(1);
...
in_sacrifices_to_sotek_unlocked_tier_3:set_min_advice_level(1);
...
in_prophecy_turn_one:set_min_advice_level(1);
...
in_prophecy_stage_1_completed:set_min_advice_level(1);
...
in_prophecy_stage_2_completed:set_min_advice_level(1);
...
in_prophecy_stage_3_completed:set_min_advice_level(1);
...
in_bretonnian_vows_button_clicked:set_min_advice_level(0);
...
in_bretonnian_vows_knights_vow_completed:set_min_advice_level(1);
...
in_bretonnian_vows_questing_vow_completed:set_min_advice_level(1);
...
in_bretonnian_vows_grail_vow_completed:set_min_advice_level(1);
...
in_ikit_workshop_turn_one:set_min_advice_level(1);
...
in_ikit_workshop_button_clicked:set_min_advice_level(0);
...
in_ikit_workshop_panel_opened:set_min_advice_level(0);
...
in_ikit_workshop_upgrade_ready:set_min_advice_level(1);
...
in_ikit_workshop_upgrade_ready:add_trigger_condition(
	...
	function(context)
		for i = 0, context:faction():pooled_resources():num_items() - 1 do
			...
			if context:faction():pooled_resources():item_at(i):key() == "skv_reactor_core" then
			
				if context:faction():pooled_resources():item_at(i):value() >= 2 then
					return true;
				end;
			end;
		end;
...
in_ikit_workshop_completed_2_upgrades:set_min_advice_level(1);
in_ikit_workshop_completed_2_upgrades:add_advice_key_precondition("wh2_dlc12.camp.advice.skv.ikit_workshop.005");
--in_ikit_workshop_completed_2_upgrades:add_whitelist_event_type("scripted_persistent_eventevent_feed_target_faction");
...
in_ikit_workshop_first_victory:set_min_advice_level(1);
...
in_ikit_workshop_nuke_purchase_available:set_min_advice_level(1);
...
in_ikit_workshop_completed_5_upgrades:set_min_advice_level(1);
...
in_ikit_workshop_upgraded_workshop:set_min_advice_level(1);
...
in_ikit_workshop_prebattle_with_nuke:set_min_advice_level(1);
...
in_ikit_workshop_prebattle_with_nuke:add_trigger_condition(
	...
	function(context)
		...
		if faction_key then
			...
			for i = 0, faction:pooled_resources():num_items() - 1 do
				
				if faction:pooled_resources():item_at(i):key() == "skv_nuke" then
				
					if faction:pooled_resources():item_at(i):value() >= 1 then
						return true;
					end;
				end;
			end;
...
in_ikit_workshop_10_warp_fuel:set_min_advice_level(1);
	...
	function(context)
		for i = 0, context:faction():pooled_resources():num_items() - 1 do
			...
			if context:faction():pooled_resources():item_at(i):key() == "skv_reactor_core" then
				...
				if context:faction():pooled_resources():item_at(i):value() >= 10 then
					return true;
				end;
			end;
		end;
...
in_under_empire_post_settlement_battle:set_min_advice_level(1);
...
in_under_empire_player_establishes:set_min_advice_level(1);
...
in_under_empire_building_completed:set_min_advice_level(1);
...
in_under_empire_player_discovered:set_min_advice_level(1);
...
in_under_empire_ai_discovered:set_min_advice_level(1);
...
in_under_empire_player_removes:set_min_advice_level(1);
...
in_under_empire_doomsphere_started:set_min_advice_level(1);
...
in_under_empire_doomsphere_completed:set_min_advice_level(1);
...
in_under_empire_player_war_camp_completed:set_min_advice_level(1);
...
in_under_empire_ai_war_camp_completed:set_min_advice_level(1);
...
in_under_empire_agent_in_region:set_min_advice_level(1);
...
in_under_empire_ai_doomsphere_completed:set_min_advice_level(1);
...
in_under_empire_ai_doomsphere_completed_followup:set_min_advice_level(1);
...
in_elector_counts_open_panel:set_min_advice_level(1);
...
in_elector_counts_occupy_capital:set_min_advice_level(1);
...
in_elector_counts_appoint_elector:set_min_advice_level(1);
...
in_imperial_authority_low_negative_authority:set_min_advice_level(1);
...
in_imperial_authority_high_negative_authority:set_min_advice_level(1);
...
in_imperial_authority_civil_war:set_min_advice_level(1);
...
in_imperial_authority_political_event:set_min_advice_level(1);
...
in_imperial_authority_invasion_event:set_min_advice_level(1);
...
in_imperial_authority_confederation_event:set_min_advice_level(1);
...
in_imperial_authority_declare_war_on_elector:set_min_advice_level(1);
...
in_imperial_authority_elector_killed:set_min_advice_level(1);
...
in_prestige_gained:set_min_advice_level(1);
...
in_gotrek_and_felix_pub_built:set_min_advice_level(1);
...
in_gotrek_and_felix_dilemma_event:set_min_advice_level(1);
...
in_gotrek_and_felix_departed:set_min_advice_level(1);
...
in_mandate_gained:set_min_advice_level(1);
...
in_mandate_gained:add_trigger_condition(
	...
	function(context)
		local faction = context:faction();
		local resource = faction:pooled_resource("emp_progress");
...
in_mandate_level_2:set_min_advice_level(1);
...
in_mandate_level_3:set_min_advice_level(1);
...
in_mandate_losing_settlement:set_min_advice_level(1);
...
in_mandate_supplies_dilemma:set_min_advice_level(1);
...
in_hostility_gained:set_min_advice_level(1);
...
in_hostility_level_1:set_min_advice_level(1);
...
in_hostility_level_3:set_min_advice_level(1);
...
in_hostility_level_5:set_min_advice_level(1);
...
in_hostility_defeat_spawned_army:set_min_advice_level(1);
...
in_hostility_resource_degrades:set_min_advice_level(1);
...
in_hostility_level_degrades:set_min_advice_level(1);
		...
		if resource:key() == "emp_wanted" and faction:name() == cm:get_local_faction_name() then
			...
			if old_effect ~= "" then
				...
				if tonumber(old_level) > 1 then
					...
					local new_level = string.sub(new_effect, string.len(new_effect));
...
in_hunters_open_panel:set_min_advice_level(1);
...
in_hunters_first_hunter_unlocked:set_min_advice_level(1);
...
in_hunters_first_hunter_story_complete:set_min_advice_level(1);
...
in_hunters_all_hunters_unlocked:set_min_advice_level(1);
...
in_hunters_all_hunters_stories_completed:set_min_advice_level(1);
...
in_wulfhart_campaign_final_battle:set_min_advice_level(1);
...
in_defence_of_the_great_plan_first_settlement_victory = intervention:new(
	...
	90, 																						-- cost
	function() in_in_defence_of_the_great_plan_first_settlement_victory_trigger() end,			-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 																-- show debug output
);
...
in_defence_of_the_great_plan_first_settlement_victory:set_min_advice_level(1);
...
function trigger_in_defence_of_the_great_plan_first_settlement_victory_advice()
...
in_defence_of_the_great_plan_first_time_gifting:set_min_advice_level(1);
...
in_defence_of_the_great_plan_open_panel:set_min_advice_level(1);
...
in_defence_of_the_great_plan_5_temples:set_min_advice_level(1);
...
in_defence_of_the_great_plan_temple_unit_available:set_min_advice_level(1);
...
in_defence_of_the_great_plan_temple_unit_available:add_trigger_condition(
	...
	function(context)
		local faction = context:faction();
		local resource = faction:pooled_resource("lzd_old_ones_favour");
...
in_defence_of_the_great_plan_1_god_complete:set_min_advice_level(1);
...
in_defence_of_the_great_plan_2_gods_complete:set_min_advice_level(1);
...
in_defence_of_the_great_plan_3_gods_complete:set_min_advice_level(1);
...
in_jungle_nexus:set_min_advice_level(1);
...
in_warm_blood_invaders_turn_2:set_min_advice_level(1);
...
in_warm_blood_invaders_hunter_killed:set_min_advice_level(1);
...
in_warm_blood_invaders_all_hunters_killed:set_min_advice_level(1);
...
in_nakai_campaign_final_battle:set_min_advice_level(1);
...
in_desert_thirst:set_min_advice_level(1);
...
in_shadowy_dealings_open_panel:set_min_advice_level(1);
...
---------------------------------------------------------------
--
--	Shadowy Dealings (Vortex Action Available)
--
---------------------------------------------------------------

-- intervention declaration
in_shadowy_dealings_vortex_action = intervention:new(
	"in_shadowy_dealings_vortex_action",									-- string name
	80,	 																	-- cost
	function() trigger_in_shadowy_dealings_vortex_action() end,				-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 											-- show debug output
);

in_shadowy_dealings_vortex_action:set_min_advice_level(1);
in_shadowy_dealings_vortex_action:add_advice_key_precondition("wh2_dlc14.camp.advice.skv.shadowry_dealings.002");

in_shadowy_dealings_vortex_action:add_trigger_condition(
	"ScriptEventPlayerFactionTurnStart",
	function(context)
		local faction = context:faction();
		if faction:name() == cm:get_local_faction_name() then
			...
			for i = 0, mf_list:num_items() -1 do
				local mf_gen = mf_list:item_at(i):general_character();
				if mf_gen:character_subtype_key() == "wh2_dlc14_skv_deathmaster_snikch" then
...
function trigger_in_shadowy_dealings_vortex_action()
	in_shadowy_dealings_vortex_action:play_advice_for_intervention(
		-- To discover more about the daemonic power, Eshin must dispatch its best agent. 
		-- That means you, my lord, for this is a crucial mission that only the Deathmaster can undertake.
		"wh2_dlc14.camp.advice.skv.shadowry_dealings.002"
	);
end;
...
in_shadowy_dealings_clan_contracts:set_min_advice_level(1);
...
in_shadowy_dealings_action_completed:set_min_advice_level(1);
...
---------------------------------------------------------------
--
--	Snikch Campaign (First Mission Completed)
--
---------------------------------------------------------------

-- intervention declaration
in_snikch_vortex_one = intervention:new(
	"in_snikch_vortex_one",										-- string name
	0,	 														-- cost
	function() trigger_in_snikch_vortex_one() end,				-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 								-- show debug output
);

in_snikch_vortex_one:set_min_advice_level(0);
in_snikch_vortex_one:add_advice_key_precondition("wh2_dlc14.camp.advice.skv.snikch_vortex_mission.001");

in_snikch_vortex_one:add_trigger_condition(
	"MissionSucceeded",
	function(context)
		local faction = context:faction();
		local mission_key = context:mission():mission_record_key();

		return faction:name() == cm:get_local_faction_name() and mission_key == "wh2_dlc14_snikch_vortex_objective_1";		
	end
);

function trigger_in_snikch_vortex_one()
	in_snikch_vortex_one:play_advice_for_intervention(
		-- My lord, the northmen were of little use to us, though they did provide another lead. 
		-- They claimed that rampaging Beastmen they recently encountered have knowledge of the daemonic power. We should hunt them down and find out what they know.
		"wh2_dlc14.camp.advice.skv.snikch_vortex_mission.001"
	);
end;

---------------------------------------------------------------
--
--	Snikch Campaign (Second Mission Completed)
--
---------------------------------------------------------------

-- intervention declaration
in_snikch_vortex_two = intervention:new(
	"in_snikch_vortex_two",										-- string name
	0,	 														-- cost
	function() trigger_in_snikch_vortex_two() end,				-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 								-- show debug output
);

in_snikch_vortex_two:set_min_advice_level(0);
in_snikch_vortex_two:add_advice_key_precondition("wh2_dlc14.camp.advice.skv.snikch_vortex_mission.002");

in_snikch_vortex_two:add_trigger_condition(
	"MissionSucceeded",
	function(context)
		local faction = context:faction();
		local mission_key = context:mission():mission_record_key();

		return faction:name() == cm:get_local_faction_name() and mission_key == "wh2_dlc14_snikch_vortex_objective_2";		
	end
);

function trigger_in_snikch_vortex_two()
	in_snikch_vortex_two:play_advice_for_intervention(
		-- The Dark Tongue of the Beastmen was barely understandable my Lord, but from our interrogations you discovered that it is the disciples of Chaos who know more about what we seek. 
		-- The source of power now feels close, you just need to find this ruinous host.
		"wh2_dlc14.camp.advice.skv.snikch_vortex_mission.002"
	);
end;

---------------------------------------------------------------
--
--	Snikch Campaign (Third Mission Completed)
--
---------------------------------------------------------------

-- intervention declaration
in_snikch_vortex_three = intervention:new(
	"in_snikch_vortex_three",										-- string name
	0,	 														-- cost
	function() trigger_in_snikch_vortex_three() end,				-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 								-- show debug output
);

in_snikch_vortex_three:set_min_advice_level(0);
in_snikch_vortex_three:add_advice_key_precondition("wh2_dlc14.camp.advice.skv.snikch_vortex_mission.003");

in_snikch_vortex_three:add_trigger_condition(
	"MissionSucceeded",
	function(context)
		local faction = context:faction();
		local mission_key = context:mission():mission_record_key();
		
		return faction:name() == cm:get_local_faction_name() and mission_key == "wh2_dlc14_snikch_vortex_objective_3";		
	end
);

function trigger_in_snikch_vortex_three()
	in_snikch_vortex_three:play_advice_for_intervention(
		-- We have finally found it, my lord! The daemonic power is trapped within a Dark Elf by the name of Malus Darkblade. We need only to find him and extract it – by any means necessary!
		"wh2_dlc14.camp.advice.skv.snikch_vortex_mission.003"
	);
end;

---------------------------------------------------------------
--
--	Snikch Campaign (Final Battle Reminder)
--
---------------------------------------------------------------

-- intervention declaration
in_snikch_vortex_reminder = intervention:new(
	"in_snikch_vortex_reminder",									-- string name
	0,	 															-- cost
	function() trigger_in_snikch_vortex_reminder() end,				-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 									-- show debug output
);

in_snikch_vortex_reminder:set_min_advice_level(0);
in_snikch_vortex_reminder:add_advice_key_precondition("wh2_dlc14.camp.advice.skv.snikch_vortex_mission.004");

in_snikch_vortex_reminder:add_trigger_condition(
	"SnikchVortexBattleReminder",
	true
);

function trigger_in_snikch_vortex_reminder()
	in_snikch_vortex_reminder:play_advice_for_intervention(
		-- Take heed, you have been targeted by the deadliest Skaven brood, Clan Eshin, who seek the power of the Daemon Tz’Arkan. And they have sent their greatest assassin, Deathmaster Snikch!
		"wh2_dlc14.camp.advice.skv.snikch_vortex_mission.004"
	);
end;
...
in_possession_start:set_min_advice_level(1);
...
in_possession_elixir:set_min_advice_level(1);
...
in_possession_full_control:set_min_advice_level(1);
...
in_possession_full_possession:set_min_advice_level(1);
...
in_possession_5:set_min_advice_level(1);
...
in_whispers_issued:set_min_advice_level(1);
...
in_whispers_succeeded:set_min_advice_level(1);
...
in_malekith_favour:set_min_advice_level(1);
...
in_malus_scrolls_early:set_min_advice_level(1);
...
in_malus_scrolls_mid:set_min_advice_level(1);
...
in_malus_scrolls_mid:add_trigger_condition(
	...
	function(context)
		if cm:get_campaign_name() == "wh2_main_great_vortex" then
			local faction = context:faction();
			local scrolls = faction:pooled_resource("wh2_main_ritual_currency");
			return scrolls:value() >= 1200 and faction:name() == cm:get_local_faction_name();
		else
			return false;
		end
	end
...
---------------------------------------------------------------
--
--	Malus Vortex (Stage One)
--
---------------------------------------------------------------

-- intervention declaration
in_malus_vortex_one = intervention:new(
	"in_malus_vortex_one",									-- string name
	0,	 													-- cost
	function() trigger_in_malus_vortex_one() end,			-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 							-- show debug output
);

in_malus_vortex_one:set_min_advice_level(1);
in_malus_vortex_one:add_advice_key_precondition("wh2_dlc14.camp.advice.def.malus_vortex_mission.001");

in_malus_vortex_one:add_trigger_condition(
	"ScriptEventMalusVortexOne",
	true
);

function trigger_in_malus_vortex_one()
	in_malus_vortex_one:play_advice_for_intervention(
		-- We must have accidentally stumbled upon a Skaven breeding ground, my lord.
		-- No matter – we will deal with these rodents swiftly before continuing with our task.
		"wh2_dlc14.camp.advice.def.malus_vortex_mission.001"
	);
end;

---------------------------------------------------------------
--
--	Malus Vortex (Stage Two)
--
---------------------------------------------------------------

-- intervention declaration
in_malus_vortex_two = intervention:new(
	"in_malus_vortex_two",									-- string name
	0,	 													-- cost
	function() trigger_in_malus_vortex_two() end,			-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 							-- show debug output
);

in_malus_vortex_two:set_min_advice_level(1);
in_malus_vortex_two:add_advice_key_precondition("wh2_dlc14.camp.advice.def.malus_vortex_mission.002");

in_malus_vortex_two:add_trigger_condition(
	"ScriptEventMalusVortexTwo",
	true
);

function trigger_in_malus_vortex_two()
	in_malus_vortex_two:play_advice_for_intervention(
		-- The ratmen have come for you again, my lord. A coincidence, perhaps, but let us remove this inconvenience and continue on with vigilance.
		"wh2_dlc14.camp.advice.def.malus_vortex_mission.002"
	);
end;

---------------------------------------------------------------
--
--	Malus Vortex (Stage Three)
--
---------------------------------------------------------------

-- intervention declaration
in_malus_vortex_three = intervention:new(
	"in_malus_vortex_three",									-- string name
	0,	 													-- cost
	function() trigger_in_malus_vortex_three() end,			-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 							-- show debug output
);

in_malus_vortex_three:set_min_advice_level(1);
in_malus_vortex_three:add_advice_key_precondition("wh2_dlc14.camp.advice.def.malus_vortex_mission.003");

in_malus_vortex_three:add_trigger_condition(
	"ScriptEventMalusVortexThree",
	true
);

function trigger_in_malus_vortex_three()
	in_malus_vortex_three:play_advice_for_intervention(
		-- There are pestilent machinations afoot, for you have become a target of Skaven aggressions at every turn. Stay alert, my lord, there are unseen schemes stirring in the shadows.
		"wh2_dlc14.camp.advice.def.malus_vortex_mission.003"
	);
end;

---------------------------------------------------------------
--
--	Malus Vortex (Stage Four)
--
---------------------------------------------------------------

-- intervention declaration
in_malus_vortex_four = intervention:new(
	"in_malus_vortex_four",									-- string name
	0,	 													-- cost
	function() trigger_in_malus_vortex_four() end,			-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 							-- show debug output
);

in_malus_vortex_four:set_min_advice_level(1);
in_malus_vortex_four:add_advice_key_precondition("wh2_dlc14.camp.advice.def.malus_vortex_mission.004");

in_malus_vortex_four:add_trigger_condition(
	"ScriptEventMalusVortexFour",
	true
);

function trigger_in_malus_vortex_four()
	in_malus_vortex_four:play_advice_for_intervention(
		-- Take heed, you have been targeted by the deadliest Skaven brood, Clan Eshin, who seek the power of the Daemon Tz’Arkan.
		-- And they have sent their greatest assassin, Deathmaster Snikch!
		"wh2_dlc14.camp.advice.def.malus_vortex_mission.004"
	);
end;

---------------------------------------------------------------
--
--	Malus Vortex (Stage Five)
--
---------------------------------------------------------------

-- intervention declaration
in_malus_vortex_five = intervention:new(
	"in_malus_vortex_five",									-- string name
	0,	 													-- cost
	function() trigger_in_malus_vortex_five() end,			-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 							-- show debug output
);

in_malus_vortex_five:set_min_advice_level(1);
in_malus_vortex_five:add_advice_key_precondition("wh2_dlc14.camp.advice.def.malus_vortex_mission.005");

in_malus_vortex_five:add_trigger_condition(
	"ScriptEventMalusVortexFive",
	true
);

function trigger_in_malus_vortex_five()
	in_malus_vortex_five:play_advice_for_intervention(
		-- Push forth, my lord. Destroy these ratmen and leave these squalid lands, so that you may take the collected scrolls back to the Witch King to claim your long-awaited reward.
		"wh2_dlc14.camp.advice.def.malus_vortex_mission.005"
	);
end;

---------------------------------------------------------------
--
--	Malus Vortex (Final Battle Reminder)
--
---------------------------------------------------------------

-- intervention declaration
in_malus_vortex_reminder = intervention:new(
	"in_malus_vortex_reminder",									-- string name
	20,	 														-- cost
	function() trigger_in_malus_vortex_reminder() end,			-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 								-- show debug output
);

in_malus_vortex_reminder:set_min_advice_level(1);
in_malus_vortex_reminder:add_advice_key_precondition("wh2_dlc14.camp.advice.def.malus_vortex_mission.006");

in_malus_vortex_reminder:add_trigger_condition(
	"ScriptEventMalusVortexReminder",
	true
);

function trigger_in_malus_vortex_reminder()
	in_malus_vortex_reminder:play_advice_for_intervention(
		-- The threat of Clan Eshin cannot be allowed to follow us back to Naggaroth. We must deal with them at once.
		"wh2_dlc14.camp.advice.def.malus_vortex_mission.006"
	);
end;
...
in_imrik_dragon_spawn:set_min_advice_level(0);
...
in_imrik_dragon_enter:set_min_advice_level(0);
...
in_imrik_dragon_quest_trigger:set_min_advice_level(0);
...
in_imrik_dragon_quest_success_one:set_min_advice_level(0);
...
in_imrik_dragon_quest_success_two:set_min_advice_level(0);
...
in_imrik_dragon_quest_success_five:set_min_advice_level(0);
...
in_imrik_dragon_dilemma_non_quest:set_min_advice_level(0);
...
in_imrik_dragon_encounter_generic:set_min_advice_level(0);
...
in_wagh_resource_gained:set_min_advice_level(0);
...
in_wagh_trigger:set_min_advice_level(0);
...
in_wagh_trigger_reminder:set_min_advice_level(0);
...
in_wagh_select:set_min_advice_level(0);
...
in_wagh_god_select:set_min_advice_level(0);
...
in_wagh_success:set_min_advice_level(0);
...
in_wagh_failure:set_min_advice_level(0);
...
in_wagh_transported_army:set_min_advice_level(0);
...
in_yvresse_reminder:set_min_advice_level(0);
...
in_mists_of_yvresse_unlocked:set_min_advice_level(0);
...
in_mists_of_yvresse_activated:set_min_advice_level(0);
...
in_yvresse_defence_introduction:set_min_advice_level(0);
...
in_yvresse_defence_reminder:set_min_advice_level(0);
...
in_yvresse_defence_increased_1:set_min_advice_level(0);
...
in_yvresse_defence_increased_2:set_min_advice_level(0);
...
in_yvresse_defence_increased_3:set_min_advice_level(0);
...
in_athel_tamarha_panel_open:set_min_advice_level(0);
...
in_athel_tamarha_mistwalker:set_min_advice_level(0);
...
in_athel_tamarha_prisoner:set_min_advice_level(0);
...
in_athel_tamarha_prison_action:set_min_advice_level(0);
...
in_athel_tamarha_upgrade:set_min_advice_level(0);
...
in_groms_waaagh:set_min_advice_level(0);
...
in_food_merchant_first_appears:set_min_advice_level(0);
...
in_food_merchant_dilemma:set_min_advice_level(0);
...
in_groms_cauldron_panel_open:set_min_advice_level(0);
...
in_groms_cauldron_ingredient:set_min_advice_level(0);
...
in_groms_cauldron_recipe:set_min_advice_level(0);
...
in_salvage_post_battle:set_min_advice_level(0);
...
in_salvage_acquired:set_min_advice_level(0);
...
in_salvage_upgraded:set_min_advice_level(0);
...
in_forge_of_daith:set_min_advice_level(0);
...
---------------------------------------------------------------
--
--	Throt's vortex Story - Introduction
--
---------------------------------------------------------------

-- intervention declaration
in_throt_story_introduction = intervention:new(
	"in_throt_story_introduction",										-- string name
	0,	 																-- cost
	function() trigger_in_throt_story_introduction() end,				-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 										-- show debug output
);

in_throt_story_introduction:set_min_advice_level(0);
in_throt_story_introduction:add_advice_key_precondition("wh2_dlc16.camp.advice.skv.throt_story.001");

in_throt_story_introduction:add_trigger_condition(
	"ScriptEventPlayerFactionTurnStart",
	function(context)
		local faction = context:faction();
		local turn = cm:turn_number();
		return turn > 2 and faction:name() == cm:get_local_faction_name();
	end
);

function trigger_in_throt_story_introduction()
	local uic_throt_story = find_uicomponent(core:get_ui_root(), "layout", "resources_bar", "topbar_list_parent", "growth_var_bar_holder", "flesh_lab_top_bar", "throt_vortex_holder");
	if not uic_throt_story then
		script_error("ERROR: trigger_in_throt_story_introduction() could not find the throt stort ui, how can this be?");
		return false;
	end;			
	-- pulse the ui wait 5 seconds, stop pulsing UI
	pulse_uicomponent(uic_throt_story, true, 5);
	cm:callback(function() pulse_uicomponent(uic_throt_story, false, 5) end, 5);
			
	in_throt_story_introduction:play_advice_for_intervention(
		-- My lord, there is limited time before Ariel will be laid vulnerable to attack! She channels her power through Waystones nearby – destroying them will grant you more time to gather your strength and will weaken her forces in the final battle.
		"wh2_dlc16.camp.advice.skv.throt_story.001"
	);
end;

---------------------------------------------------------------
--
--	Throt's vortex Story - Province Completed
--
---------------------------------------------------------------

-- intervention declaration
in_throt_story_province_completed = intervention:new(
	"in_throt_story_province_completed",								-- string name
	0,	 																-- cost
	function() trigger_in_throt_story_province_completed() end,				-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 										-- show debug output
);

in_throt_story_province_completed:set_min_advice_level(0);
in_throt_story_province_completed:add_advice_key_precondition("wh2_dlc16.camp.advice.skv.throt_story.002");

in_throt_story_province_completed:add_trigger_condition(
	"MissionSucceeded",
	function(context)
		return context:mission():mission_record_key() == "wh2_dlc16_skv_throt_early_game_capture_province";
	end
);

function trigger_in_throt_story_province_completed()
	in_throt_story_province_completed:scroll_camera_to_settlement_for_intervention(
		"wh2_main_vor_shadow_wood_clar_karond",
		-- You have established a foothold here, but do not rest on your laurels, for Ariel will not reside in Naggaroth forever. You must disrupt her ritual by capturing or razing the settlements containing her Waystones, each of which serves to strengthen her bodyguard.
		"wh2_dlc16.camp.advice.skv.throt_story.002"
	);
end;

---------------------------------------------------------------
--
--	Throt's vortex Story - One Waystone Mission Completed
--
---------------------------------------------------------------

-- intervention declaration
in_throt_story_waystone_one = intervention:new(
	"in_throt_story_waystone_one",										-- string name
	0,	 																-- cost
	function() trigger_in_throt_story_waystone_one() end,				-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 										-- show debug output
);

in_throt_story_waystone_one:set_min_advice_level(0);
in_throt_story_waystone_one:add_advice_key_precondition("wh2_dlc16.camp.advice.skv.throt_story.003");

in_throt_story_waystone_one:add_trigger_condition(
	"ScriptEvent_WaystoneMissionCompleted",
	true
);

function trigger_in_throt_story_waystone_one()
	local uic_throt_story = find_uicomponent(core:get_ui_root(), "layout", "resources_bar", "topbar_list_parent", "growth_var_bar_holder", "flesh_lab_top_bar", "throt_vortex_holder");
	if not uic_throt_story then
		script_error("ERROR: trigger_in_throt_story_waystone_one() could not find the throt stort ui, how can this be?");
		return false;
	end;			
	-- pulse the ui wait 5 seconds, stop pulsing UI
	pulse_uicomponent(uic_throt_story, true, 5);
	...
	in_throt_story_waystone_one:play_advice_for_intervention(
		-- Well done, my lord! You have successfully destroyed one of the Waystones Ariel intended to use for her ritual. This disruption to her plans weakens her forces and has given you more time to prepare yourself for the attack.
		"wh2_dlc16.camp.advice.skv.throt_story.003"
	);
end;

---------------------------------------------------------------
--
--	Throt's vortex Story - Three Waystone Mission Completed
--
---------------------------------------------------------------

-- intervention declaration
in_throt_story_waystone_three = intervention:new(
	"in_throt_story_waystone_three",									-- string name
	0,	 																-- cost
	function() trigger_in_throt_story_waystone_three() end,				-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 										-- show debug output
);

in_throt_story_waystone_three:set_min_advice_level(0);
in_throt_story_waystone_three:add_advice_key_precondition("wh2_dlc16.camp.advice.skv.throt_story.004");

in_throt_story_waystone_three:add_trigger_condition(
	"ScriptEvent_UnlockThrotFinalBattle",
	true
);

function trigger_in_throt_story_waystone_three()
	local uic_throt_story = find_uicomponent(core:get_ui_root(), "layout", "resources_bar", "topbar_list_parent", "growth_var_bar_holder", "flesh_lab_top_bar", "throt_vortex_holder");
	if not uic_throt_story then
		script_error("ERROR: trigger_in_throt_story_waystone_three() could not find the throt stort ui, how can this be?");
		return false;
	end;			
	-- pulse the ui wait 5 seconds, stop pulsing UI
	pulse_uicomponent(uic_throt_story, true, 5);
	...
	in_throt_story_waystone_three:play_advice_for_intervention(
		-- Tireless work, unclean one! The destruction of Ariel’s Waystones has weakened her forces. She can now be provoked into attempting her ritual early, which will leave her open to attack. Should you destroy more Waystones first, however, her bodyguard will be weakened even further.
		"wh2_dlc16.camp.advice.skv.throt_story.004"
	);
end;
...
in_worldroots_pacification:set_min_advice_level(0);
...
in_worldroots_oak_of_ages:set_min_advice_level(0);
...
in_worldroots_markers:set_min_advice_level(0);
...
in_worldroots_markers:add_trigger_condition(
	...
	function(context)
		...
		local is_worldroots_intro_invasion = context.string:find("invasion") and context.string:find("intro")
...
function trigger_in_worldroots_markers()
	--Need to pass th scroll_camera_for_intervention a region to lift the shroud, we dont actaully need to life the shroud so just pass Witch foor Vortex/Oak of Ages for ME
	local region_name = "";
	if cm:get_campaign_name() == "main_warhammer" then
		region_name = "wh_main_yn_edri_eternos_the_oak_of_ages";
	else
		region_name = "wh2_main_vor_naggaroth_glade";
	end
...
in_worldroots_button:set_min_advice_level(0);
...
function trigger_in_worldroots_button()
	local uic_worldroots = find_uicomponent(core:get_ui_root(), "layout", "faction_buttons_docker", "button_group_management", "button_world_roots");
...
in_worldroots_teleport:set_min_advice_level(0);
...
function trigger_in_worldroots_teleport()
	...
	if cm:get_campaign_name() == "main_warhammer" then
		...
		if faction_name == "wh_dlc05_wef_wood_elves" or faction_name == "wh_dlc05_wef_argwylon" then
			--No Oak of Ages
			glade_list = {"wh2_main_heart_of_the_jungle_oreons_camp","wh2_main_badlands_glade","wh2_main_lustria_glade","wh2_main_naggaroth_glade","wh2_main_old_world_glade","wh2_main_avelorn_gaean_vale","wh2_main_laurelorn_forest_laurelorn_forest"}
			region_name = "wh_main_yn_edri_eternos_the_oak_of_ages";
		elseif faction_name == "wh2_dlc16_wef_sisters_of_twilight" then
			--No Witchwood
			glade_list = {"wh_main_yn_edri_eternos_the_oak_of_ages", "wh2_main_heart_of_the_jungle_oreons_camp","wh2_main_badlands_glade","wh2_main_lustria_glade","wh2_main_old_world_glade","wh2_main_avelorn_gaean_vale","wh2_main_laurelorn_forest_laurelorn_forest"}
			region_name = "wh2_main_naggaroth_glade";
		elseif faction_name == "wh2_dlc16_wef_drycha" then
			--No Gryphon Wood
			glade_list = {"wh_main_yn_edri_eternos_the_oak_of_ages", "wh2_main_heart_of_the_jungle_oreons_camp","wh2_main_badlands_glade","wh2_main_lustria_glade","wh2_main_naggaroth_glade","wh2_main_avelorn_gaean_vale","wh2_main_laurelorn_forest_laurelorn_forest"}
			region_name = "wh2_main_old_world_glade";
		else
			out("How did this faction get here??? - " .. faction_name);
		end
	else
		--No Witchwood
		glade_list = {"wh2_main_vor_lustria_glade", "wh2_main_vor_avelorn_gaean_vale", "wh2_main_vor_heart_of_the_jungle_oreons_camp"};
		region_name = "wh2_main_vor_naggaroth_glade";
	end
...
in_worldroots_amber:set_min_advice_level(0);
...
in_worldroots_amber:add_trigger_condition(
	"ScriptEventPlayerFactionTurnStart",
	function(context)
		local pr = context:faction():pooled_resources();
		for i = 0, pr:num_items() - 1 do
			if pr:item_at(i):key() == "wef_amber" then
				if pr:item_at(i):value() > 0 then
					return true;
				end;
			end;
		end;
...
function trigger_in_worldroots_amber()
	...
	local uic_tech = find_uicomponent(core:get_ui_root(), "layout", "faction_buttons_docker", "button_group_management", "button_technology");
	local uic_amber = find_uicomponent(core:get_ui_root(), "layout", "resources_bar", "topbar_list_parent", "canopic_jars_holder");	
...
in_worldroots_rebirth:set_min_advice_level(0);
...
in_worldroots_invasions:set_min_advice_level(0);
...
in_aspects:set_min_advice_level(0);
	...
	function(context)
		...
		if faction:name() == cm:get_local_faction_name() then
			...
			if context:character():has_military_force() then
				...
				for i = 0, unit_list:num_items(i) - 1 do
...
in_wild_spirits:set_min_advice_level(0);
...
function trigger_in_wild_spirits()
	...
	local uic_ws = find_uicomponent(core:get_ui_root(), "layout", "hud_center_docker", "hud_center", "small_bar", "button_group_army", "button_mercenaries");
	...
	cm:callback(function()
		uic_ws = find_uicomponent(core:get_ui_root(), "layout", "hud_center_docker", "hud_center", "small_bar", "button_group_army", "button_mercenaries");
...
in_spites:set_min_advice_level(0);
...
in_flesh_lab_growth_juice:set_min_advice_level(0);
...
in_flesh_lab_monster_pack:set_min_advice_level(0);
...
in_flesh_lab_augment_unlocked:set_min_advice_level(0);
...
in_flesh_lab_upgrades:set_min_advice_level(0);
...
in_rakarth_monster_pen:set_min_advice_level(0);
...
in_rakarth_raiding:set_min_advice_level(0);
...
in_rakarth_settlements:set_min_advice_level(0);
...
in_rakarth_rite:set_min_advice_level(0);
...
in_silent_sanctums_point_gained:set_min_advice_level(0);
...
in_silent_sanctums_constructed:set_min_advice_level(0);
...
in_threat_map_first_mission_issued:set_min_advice_level(0)
...
in_threat_map_panel_open_1:set_min_advice_level(0)
...
in_threat_map_panel_open_2:set_min_advice_level(0)
...
in_threat_map_panel_open_3:set_min_advice_level(0)
...
in_threat_map_first_mission_complete:set_min_advice_level(0)
...
in_taurox_rampage_first_battle:set_min_advice_level(0)
...
in_taurox_rampage_first_threshold:set_min_advice_level(0)
...
in_taurox_rampage_momentum_lost:set_min_advice_level(0)
...
in_taurox_rampage_over:set_min_advice_level(0)
...
in_taurox_rampage_success:set_min_advice_level(0)
...
in_bloodgrounds_herdstone:set_min_advice_level(0)
...
in_bloodgrounds_herdstone:set_wait_for_event_dismissed(false);
...
in_bloodgrounds_regions:set_min_advice_level(0)
...
in_bloodgrounds_settlements:set_min_advice_level(0)
...
in_bloodgrounds_settlements:set_wait_for_event_dismissed(false);
...
in_bloodgrounds_settlements:add_trigger_condition(
	...
	function()
		...
		local herdstone_count = faction:pooled_resource("bst_herdstone_shard"):value();
...
in_bloodgrounds_first_raze:set_min_advice_level(0)
...
in_bloodgrounds_all_razed:set_min_advice_level(0)
...
in_bloodgrounds_ritual_complete:set_min_advice_level(0)
...
in_beastmen_technology_panel_opened:set_min_advice_level(0);
...
in_marks_of_ruination_turn_two:set_min_advice_level(0);
...
in_marks_of_ruination_first_threshold:set_min_advice_level(0);
...
in_marks_of_ruination_completed:set_min_advice_level(0);
...
in_forge_opened:set_min_advice_level(0);
...
in_forge_first_part_recovered:set_min_advice_level(0);
...
---------------------------------------------------------------
--
--	Dwarf Forge - Vortex Final Battle Unlocked
--
---------------------------------------------------------------

-- intervention declaration
in_forge_vortex_final_battle = intervention:new(
	"in_forge_vortex_final_battle",									-- string name
	0,	 															-- cost
	function() trigger_in_forge_vortex_final_battle() end,			-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 									-- show debug output
);

in_forge_vortex_final_battle:set_min_advice_level(0);
in_forge_vortex_final_battle:set_wait_for_fullscreen_panel_dismissed(false);
in_forge_vortex_final_battle:add_advice_key_precondition("wh2_dlc17.camp.advice.dwf.thorek_story.003");

in_forge_vortex_final_battle:add_trigger_condition(
	"MissionIssued",
	function(context)
		return context:mission():mission_record_key() == "wh2_dlc17_dwf_thorek_final_battle_lost_vault";
	end
);

function trigger_in_forge_vortex_final_battle()
	in_forge_vortex_final_battle:play_advice_for_intervention("wh2_dlc17.camp.advice.dwf.thorek_story.003")
end;

---------------------------------------------------------------
--
--	Thorek Vortex - Skaven Attack
--
---------------------------------------------------------------

-- intervention declaration
in_thorek_vortex_skaven_attack = intervention:new(
	"in_forge_vortex_final_bin_thorek_vortex_skaven_attackattle",	-- string name
	0,	 															-- cost
	function() trigger_in_thorek_vortex_skaven_attack() end,		-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 									-- show debug output
);

in_thorek_vortex_skaven_attack:set_min_advice_level(0);
in_thorek_vortex_skaven_attack:set_wait_for_fullscreen_panel_dismissed(false);
in_thorek_vortex_skaven_attack:add_advice_key_precondition("wh2_dlc17.camp.advice.dwf.thorek_skaven_attack.001");

in_thorek_vortex_skaven_attack:add_trigger_condition(
	"MissionSucceeded",
	function(context)
		return context:mission():mission_record_key() == "wh2_dlc17_dwf_thorek_vortex_objective_1";
	end
);

function trigger_in_thorek_vortex_skaven_attack()
	in_thorek_vortex_skaven_attack:play_advice_for_intervention("wh2_dlc17.camp.advice.dwf.thorek_skaven_attack.001")
end;
...
in_artefacts_first_pair_recovered:set_min_advice_level(0);
...
in_artefacts_first_artefact_forged:set_min_advice_level(0);
...
in_artefacts_third_artefact_forged:set_min_advice_level(0);
...
in_artefacts_all_artefact_forged:set_min_advice_level(0);
...
---------------------------------------------------------------
--
--	Thorek Confederate Karak Azul
--
---------------------------------------------------------------

-- intervention declaration
in_eg_thorek_confederate = intervention:new(
	"in_eg_thorek_confederate",								-- string name
	0,	 													-- cost
	function() trigger_in_eg_thorek_confederate() end,		-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 							-- show debug output
);

in_eg_thorek_confederate:set_disregard_cost_when_triggering(true);

in_eg_thorek_confederate:add_trigger_condition(
	"MissionIssued",
	function(context)
		return context:mission():mission_record_key() == "wh2_dlc17_dwf_thorek_early_game_confederate";
	end
);

function trigger_in_eg_thorek_confederate()
	in_eg_thorek_confederate:play_advice_for_intervention("wh2.dlc17.camp.vortex.thorek.ironbrow.early.game.001.confederation");
end;
...
in_oathgold:add_trigger_condition(
	...
	function(context)
		local faction = context:faction();
		local resource = faction:pooled_resource("dwf_oathgold");
...
in_dwarf_runes:set_min_advice_level(0);
...
in_grudges_low_severity:set_min_advice_level(0);
...
in_grudges_high_severity:set_min_advice_level(0);
...
---------------------------------------------------------------
--
--	Ogre Camps - Introduction
--
---------------------------------------------------------------

in_ogre_camps_introduction = intervention:new(
	"in_ogre_camps_introduction",							-- string name
	0,	 													-- cost
	function() trigger_in_ogre_camps_introduction() end,	-- trigger callback
	BOOL_INTERVENTIONS_DEBUG	 							-- show debug output
)

in_ogre_camps_introduction:set_min_advice_level(0)
in_ogre_camps_introduction:add_advice_key_precondition("war.camp.advice.race.ogres.001")

in_ogre_camps_introduction:add_trigger_condition(
	"ScriptEventMarkerSpawned",
	function(context)
		is_ogre_camp = string.find(context.string, Ogre_Camp.marker_key)
		if is_ogre_camp then
			in_ogre_camps_introduction.ogre_camp_position = context:table_data()
		end
		return is_ogre_camp
	end
);

function trigger_in_ogre_camps_introduction()
	local x, y = cm:log_to_dis(in_ogre_camps_introduction.ogre_camp_position[1], in_ogre_camps_introduction.ogre_camp_position[2]);

	in_ogre_camps_introduction:scroll_camera_for_intervention(
	nil,
	x,
	y,
	-- "Ogres, my lord. These guys are tough and stupid so watch out." - Line taken from WH3. Solely for dress-up purposes of introducing the help page.
	"war.camp.advice.ogre_camps.001",
	{
		"wh2.camp.advice.ogre_camps.info_001",
		"wh2.camp.advice.ogre_camps.info_002",
		"wh2.camp.advice.ogre_camps.info_003"
	})
end