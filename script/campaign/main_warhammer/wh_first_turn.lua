...
function set_first_turn_ui_allowed(value, local_faction)
	...
	uim:suppress_end_of_turn_warning("bankrupt", should_suppress);
	uim:suppress_end_of_turn_warning("tech", should_suppress);
	uim:suppress_end_of_turn_warning("edict", should_suppress);
	uim:suppress_end_of_turn_warning("character", should_suppress);
	uim:suppress_end_of_turn_warning("army", should_suppress);
	uim:suppress_end_of_turn_warning("politics", should_suppress);
	uim:suppress_end_of_turn_warning("siege", should_suppress);
	uim:suppress_end_of_turn_warning("morale", should_suppress);
...
function set_first_turn_resources_bar_allowed(value)
	if value == false then
...
function province_panel_tutorial:new(start_advice, objective, region_name, province_name, end_callback)
	...
	pp.settlement_name = settlement_prepend_str .. region_name;
...
function construction_tutorial:new(region_name, province_name, start_advice, start_objective, base_building_card, upgrade_building_card, upgrade_objective, end_advice, end_callback)
	...
	ct.cm = cm;
	ct.settlement_name = settlement_prepend_str .. region_name;
...
function first_attack_enemy_mission:trigger()
	...
	if faction and faction:culture() == "wh_main_dwf_dwarfs" then
		mm:add_payload("faction_pooled_resource_transaction{resource dwf_oathgold;factor wh2_main_resource_factor_missions;amount 15;}"); -- Oathgold Reward
	elseif faction and faction:culture() == "wh_main_emp_empire" then
		mm:add_payload("faction_pooled_resource_transaction{resource emp_prestige;factor wh2_dlc13_resource_factor_events;amount 300;}"); -- Prestige Reward
	end;
	...
	if not effect.get_advice_history_string_seen("prelude_attacking_enemy_advice") then
...
function first_attack_enemy_mission:process_end_turn()
	...
	if cm:character_can_reach_character(enemy_char, closest_player_char) then
		...
		if closest_player_distance < distance_threshold then
			...
			cm:attack(enemy_char_str, player_char_str, true);
			return;
		end;
	end;
...
function first_capture_settlement_mission:new(start_advice, mission_key, region_key, player_faction, enemy_faction, player_starting_area, end_callback)
	...
	if faction and faction:culture() == "wh_main_dwf_dwarfs" then
		mm:add_payload("faction_pooled_resource_transaction{resource dwf_oathgold;factor wh2_main_resource_factor_missions;amount 15;}"); -- Oathgold Reward
	elseif faction and faction:culture() == "wh_main_emp_empire" then
		mm:add_payload("faction_pooled_resource_transaction{resource emp_imperial_authority;factor wh2_main_resource_factor_missions;amount 1;}"); -- Imperial Authority Reward
	end;
	...
	intervention:set_disregard_cost_when_triggering(true);
...
function first_quest_battle_mission:trigger()
	...
	if not effect.get_advice_history_string_seen("quest_battles") then
		effect.set_advice_history_string_seen("quest_battles");
	end;
...
function modify_public_order_if_unseen(region_str, public_order_modifier, advice_key, province_key, mission_key)
	...
	if not effect.get_advice_history_string_seen("prelude_public_order_advice") and cm:model():difficulty_level() >= -1 then
...
function ll_unlock:new(faction, startpos_id, forename, event, condition)
	...
	if not is_function(condition) then
		script_error("ERROR: ll_unlock:new() called but supplied condition [" .. tostring(condition) .."] is not a function");
		return false;
	end;
		...
		core:add_listener(
			...
			function()
				if char_with_forename_has_no_military_force(self.forename) then
					...
					cm:unlock_starting_general_recruitment(self.startpos_id, self.faction);
	...
	for i = 0, faction_list:num_items() - 1 do
		...
		if current_faction:is_human() then
			...
			for i = 0, char_list:num_items() - 1 do
				...
				if not (current_char:character_subtype("dlc03_bst_beastlord") or current_char:character_subtype("wh2_main_def_black_ark")) and cm:char_is_general_with_army(current_char) then
