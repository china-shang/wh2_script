...
local monster_hunts = {
	["wh_dlc08_nor_norsca"] = {
		...
		["monster_hunt_3"] = {
				...
				["tech"] = "tech_dlc08_nor_creatures_06"
		},
		["monster_hunt_4"] = {
		...
		["monster_hunt_6"] = {
				...
				["tech"] = "tech_dlc08_nor_creatures_07"
		},
		["monster_hunt_7"] = {
			...
			for i = 1, #monster_hunts_index do
				if key == monster_hunts[faction][monster_hunts_index[i]]["qb"] then
					for raze_id, god_key in pairs(RAZE_ID_TO_GOD_KEY) do
						if NORSCAN_GODS[faction][god_key].favour == MAX_NORSCAN_GOD_FAVOUR then
							maxed = true;
						end;
					end;
...
core:add_listener(
	...
	function(context)
		...
		if cm:model():world():whose_turn_is_it():name() == faction then
			if UIComponent(context.component):Parent(false, "book_of_monster_hunts") ~= nil then 
				...
				for i = 1, #monster_hunts_index do
					if monster_hunts_index[i] == mission_index then
						activate_a_monster_hunt(mission_index, faction, monster_hunts_other_faction[faction], UIComponent(context.component):Parent(false, "book_of_monster_hunts"));
