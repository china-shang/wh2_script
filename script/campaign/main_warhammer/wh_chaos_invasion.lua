...
function ci_setup_old()
	...
	-- early game event
	---------------------------------------------------------
	if cm:get_local_faction_name(true) and cm:model():turn_number() < 50 and not cm:get_saved_value("ci_early_event_shown") then
		core:add_listener(
			"early_game_event_message",
			"ScriptEventHumanFactionTurnStart",
			function(context) return cm:model():turn_number() >= ci_early_game_event_turn end,
...
function ci_mid_game_start_setup()
	if cm:get_local_faction_name(true) and not cm:is_multiplayer() then
	...
	if is_table(chaos_rises_start_pos) and is_string(chaos_rises_scene) then
		...
		cutscene:set_skippable(
			...
			function()
				...
				cm:make_region_visible_in_shroud(cm:get_local_faction_name(true), ci_chaos_wastes_string);
	...
	elseif override_final_pos then
		...
		cutscene:set_skippable(
			...
			function()
				...
				cm:callback(
					function()
						cm:make_region_visible_in_shroud(cm:get_local_faction_name(true), override_region_to_unshroud);
...
function ci_mid_game_show_event(callback, delay)
	...
	if cm:get_local_faction_name(true) and cm:get_saved_value("ci_stage") == 0 then
...
function ci_mid_game_setup_listeners(exemption_turn)
	...
	if cm:get_local_faction_name(true) then
		...
		core:add_listener(
			...
			function(context) return (not exemption_turn or cm:model():turn_number() ~= exemption_turn) end,
			function()
	...
	if not cm:get_saved_value("ci_start_pre_late_game_to_late_game_listener_mp") then
		core:add_listener(
			...
			"ScriptEventHumanFactionTurnStart",
			function(context) return context:faction():imperium_level() >= ci_late_game_imperium and (not cm:is_multiplayer() or ci_get_num_hordes(ci_chaos_faction_string) > 0) end,
	...
	if cm:is_multiplayer() then
		core:add_listener(
			...
			function(context)
				...
				return turn_number and turn_number < cm:model():turn_number();
			end,
			function()
				ci_late_game_start();
			end,
...
function ci_late_game_start()
	...
	elseif cm:get_local_faction_name(true) then
		-- singleplayer
			...
			function()
				if ci_get_num_hordes() < 4 then
					...
					if num_waves >= desired_waves then
						...
						cm:remove_faction_turn_start_listener_by_name("chaos_army_count_check_late");
						return;
					end;
...
function ci_spawn_horde(stage, callback, instant)
	...
	if is_multiplayer or not cm:get_local_faction_name(true) then
	...
	for i = 1, #ci_spawn_areas do
		...
		if current_num_hordes >= max_num_hordes then
			...
			for j = 1, desired_hordes do
				...
				if j == 1 or not cm:get_local_faction_name(true) or instant then
					spawn_function();
				else
					local timer = j - 1;
	...
	if is_function(callback) then
		if is_multiplayer or not cm:get_local_faction_name(true) or instant then
			callback();
		else
		...
		function()
			...
			for i = 1, #factions_at_war_with do
				...
				if not current_faction:is_dead() then
					...
					if cm:get_local_faction_name(true) then
						if cm:get_faction(cm:get_local_faction_name(true)):is_ally_vassal_or_client_state_of(current_faction) then
							cm:force_declare_war(cm:get_local_faction_name(true), norsca_faction_name, true, true);
							cm:force_diplomacy("faction:" .. cm:get_local_faction_name(true), "faction:" .. norsca_faction_name, "peace", false, false, true);
						end;
					end;
...
function ci_spawn_archaon()
	...
	cm:remove_faction_turn_start_listener_by_name("end_chaos_turn");
end;
