...
local vortex_battle_reminder = false;
local vortex_battle_counter = 0;
...
function add_shadowy_dealings_listeners()
	...
	core:add_listener(
		"dust_MissionSucceeded",
		"MissionSucceeded",
		function(context)
			return context:faction():name() == dust_faction;
		end,
		function(context)
			dust_MissionSucceeded(context);
		end,
		true
	);

	if not effect.get_advice_history_string_seen("vortex_battle_reminder") then
		effect.set_advice_history_string_seen("vortex_battle_reminder")
...
function dust_FactionTurnStart(faction)
	if faction:has_pooled_resource("skv_dust") == true then
		if faction:is_human() == true then
			local dust = faction:pooled_resource("skv_dust");
...
function dust_RitualCompletedEvent(context)
	...
	if ritual_category == "ESHIN_VORTEX_RITUAL" then
		...
		if ritual_key == "wh2_dlc14_eshin_actions_vortex_mission_1" or ritual_key == "wh2_dlc14_eshin_actions_mortal_empires_mission_1" then
			dust_cap = dust_cap + 1;
			dust_UpdateCap();
		elseif ritual_key == "wh2_dlc14_eshin_actions_vortex_mission_2" or ritual_key == "wh2_dlc14_eshin_actions_mortal_empires_mission_2" then
			dust_cap = dust_cap + 1;
			dust_UpdateCap();
		elseif ritual_key == "wh2_dlc14_eshin_actions_vortex_mission_3" or ritual_key == "wh2_dlc14_eshin_actions_mortal_empires_mission_3" then
			dust_cap = dust_cap + 1;
			dust_UpdateCap();

			if cm:get_campaign_name() == "wh2_main_great_vortex" then
				vortex_battle_reminder = true;
			end
			...
			if target_type == "FACTION" then
				...
				if target_faction:is_null_interface() == false and target_faction:is_dead() == false then
					if target_faction:has_home_region() == true then
						...
						cm:add_scripted_composite_scene_to_settlement(comp_scene, scene_type, settlement_key, 0, 0, true, true, false);
						table.insert(dust_actions, comp_scene);
					end
				end
			elseif target_type == "REGION" then
...
function dust_MissionSucceeded(context)
	local mission_key = context:mission():mission_record_key();
	local faction = context:faction();

	if faction:name() == dust_faction then
		if mission_key == "wh2_dlc14_snikch_vortex_objective_3" then
			core:svr_save_registry_bool("shadow_snikch_call_to_arms", true);
			cm:register_instant_movie("warhammer2/shadow/shadow_snikch_call_to_arms");
		elseif mission_key == "wh2_dlc14_qb_skv_final_battle_snikch" then
			core:svr_save_registry_bool("shadow_snikch_win", true);
			cm:register_instant_movie("warhammer2/shadow/shadow_snikch_win");
			vortex_battle_reminder = false;
			dust_cap = dust_cap + 1;
			dust_UpdateCap();			
		end
	end
	...
	cm:callback(function()
		...
		cm:callback(function()
			...
			if region == nil then
				...
				local settlement_key = "settlement:"..region:name();
				cm:add_scripted_composite_scene_to_settlement(key, comp_scene, settlement_key, 0, 0, true, true, false);
			end
