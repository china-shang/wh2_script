...
function Eltharion_UpdateGromTimer()
	...
	effect.set_context_value("grom_timer", timer);
	...
	core:add_listener(
		...
		function(context)
			...
			for i = 1, #final_battle_mission_keys do
				if context:mission():mission_record_key() == final_battle_mission_keys[i] then
					check_res = true;
					cm:complete_scripted_mission_objective("wh_main_long_victory", "complete_eltharion_final_battle", true);
				end
			end
			return check_res;
		end,
...
function Get_Final_Battle_Key()
	...
	if faction ~= false and faction:has_pooled_resource("yvresse_defence") then
