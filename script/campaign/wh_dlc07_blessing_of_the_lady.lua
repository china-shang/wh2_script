...
function Add_Lady_Blessing_Listeners()
	...
	core:add_listener(
		...
		function(context)
			...
			return not character:is_null_interface() and Is_Bretonnian(character:faction():name()) and character:has_military_force() and not (pb:has_attacker() and pb:attacker() == character and character:has_garrison_residence());
		end,
...
function Blessing_BattleCompleted(context)
	...
	if Is_Bretonnian(attacker_name) or Is_Bretonnian(defender_name) then
		...
		if cm:model():pending_battle():has_attacker() and cm:model():pending_battle():attacker():is_null_interface() == false and Is_Bretonnian(attacker_name) and (attacker:is_null_interface() == false and attacker:is_human()) then
		...
		if cm:model():pending_battle():has_defender() and cm:model():pending_battle():defender():is_null_interface() == false and Is_Bretonnian(defender_name) and (defender:is_null_interface() == false and defender:is_human()) then
...
function Blessing_Character_Won(character)
	if character:is_null_interface() == false and Is_Bretonnian(character:faction():name()) then
