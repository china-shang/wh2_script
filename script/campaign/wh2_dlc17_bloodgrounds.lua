Bloodgrounds = {
	...
	ruination_ritual_chapter_objective = "wh2_dlc17_objective_taurox_01",
	ruination_ritual_scripted_objective = "ruination_ritual_performed",
...
function Bloodgrounds:setup_ritual_of_ruin_listener()
	...
	core:add_listener(
		...
		function(context)
			...
			if bloodground.ritual_unlocked and not bloodground.ritual_completed then
				...
				self:update_effect_bundles_for_bloodgrounds(true)
	...
	core:add_listener(
		"ScriptEventPlayerFactionTurnStartRitualofRuin",
		"ScriptEventHumanFactionTurnStart",
		function(context)
			return context:faction():subculture() == self.beastmen_sc_key
		end,
		function(context)
			self:update_effect_bundles_for_bloodgrounds()
		end,
	...
	-- This is only needed once, since only one Chapter Objective (the first one) listens for the ruination ritual.
	core:add_listener(
		"RitualOfRuinScriptedObjective",
		"ScriptEventRitualofRuinPerformed",
		true,
		function(context)
			cm:complete_scripted_mission_objective(self.ruination_ritual_chapter_objective, self.ruination_ritual_scripted_objective, true);
		end,
		false
	);
end
...
function Bloodgrounds:restore_bloodground_ui()
	...
	for bloodground_key, bloodground in pairs(self.active_bloodgrounds) do
		...
		bloodgrounds_component:InterfaceFunction("AddActiveBloodground", bloodground)
	end
end
...
function Bloodgrounds:create_bloodground(region, is_preview)
	...
	if not is_preview then
		...
		bloodgrounds_component:InterfaceFunction("AddActiveBloodground", self.active_bloodgrounds[bloodground_key])
	...
	elseif self:is_bloodground_region(bloodground_key) then
		bloodgrounds_component:InterfaceFunction("SetPreviewRegions", self:is_bloodground_region(bloodground_key).region_list)
	else
		bloodgrounds_component:InterfaceFunction("SetPreviewRegions", temp_region_list)
	end
end
...
function Bloodgrounds:remove_bloodground(herdstone_region_key)
	...
	bloodgrounds_component:InterfaceFunction("RemoveActiveBloodground", bloodground.herdstone_region)
...
function Bloodgrounds:update_effect_bundles_for_bloodgrounds()
	...
	for k, bloodground in pairs(self.active_bloodgrounds) do
		...
		local bloodground_owner_key = herdstone_region_interface:owning_faction():name()
			...
			for i = 1, #bundles_to_apply do
				...
				elseif bundle_key == self.bloodgrounds_plague_bundle_key then
					...
					if cm:random_number() <= self.bloodgrounds_plague_spawn_chance then
						cm:spawn_plague_at_region(region_interface, self.bloodgrounds_plague_key)
					end
...
function Bloodgrounds:grant_ritual_of_ruin_rewards(bloodground)
	...
	local payload_string = "payload{faction_pooled_resource_transaction{resource "..self.ruination_key..";factor "..self.ruination_factor..";amount "..tostring(marks_to_gain)..";}}"
...
function Bloodgrounds:apply_vfx(region_key, opt_vfx_key, opt_is_preview)
	...
	if opt_vfx_key then
		cm:add_scripted_composite_scene_to_settlement(region_key.."_bloodground_fx",opt_vfx_key,"settlement:"..region_key,1,1,true,true,true)
...
function Bloodgrounds:create_confirmation_dialogue(bloodground_key)
	...
	local dialogue_string = effect.get_localised_string("campaign_localised_strings_string_ritual_of_ruin_confirm")
	...
	core:add_listener(
		...
		"ComponentLClickUp",
		function(context)
			if context.string == "button_tick" or context.string == "button_cancel" then
				return true
			end
		end,
		function(context)
...
function Bloodgrounds:create_debug_ui(region_name)
	...
	effect.set_context_value("dragon_taming_turns_until_next", bloodground.devastation);
...
function Bloodgrounds:ui_update_ritual_state(bloodground)
	local bloodgrounds_component = find_uicomponent(core:get_ui_root(), self.bloodgrounds_uic_name)
	bloodgrounds_component:InterfaceFunction("UpdateRitualOfRuinState", bloodground.herdstone_region, bloodground.ritual_unlocked, bloodground.ritual_completed, bloodground.devastation)
end
...
function Bloodgrounds:are_there_any_human_beastmen()
	return cm:are_any_factions_human(self.beastmen_factions, true)
end
