...
function add_tomb_kings_listeners()
	...
	core:add_listener(
		...
		true,
		function(context)
			tomb_king_final_battle_pending(context);
		end,
		true
	);
	core:add_listener(
		...
		true,
		function(context)
			tomb_king_final_battle_completed(context);
		end,
		true
	);
	core:add_listener(
...
function get_tomb_king_army_count(faction)
	...
	local dynasty_techs = {
		"tech_dlc09_tmb_dynasty_1",
		"tech_dlc09_tmb_dynasty_2",
		"tech_dlc09_tmb_dynasty_3",
		"tech_dlc09_tmb_dynasty_4",
		"tech_dlc09_tmb_dynasty_5",
		"tech_dlc09_tmb_dynasty_6"
	};
...
-- Final Battle - Restrict army abilities
function tomb_king_final_battle_pending(context)
	local pb = cm:model():pending_battle();
	local battle_key = pb:set_piece_battle_key();
	
	if battle_key == "wh2_dlc09_qb_tmb_final_battle_settra" or battle_key == "wh2_dlc09_qb_tmb_final_battle_khatep" or battle_key == "wh2_dlc09_qb_tmb_final_battle_khalida" then
		local attacker = pb:attacker();
		local attacker_cqi = attacker:military_force():command_queue_index();
		cm:apply_effect_bundle_to_force("wh2_dlc09_bundle_tmb_final_battle_army_abilities_barrage", attacker_cqi, 0);
		cm:apply_effect_bundle_to_force("wh2_dlc09_bundle_tmb_final_battle_army_abilities_swarm", attacker_cqi, 0);
	end
end

-- Final battle - Remove army ability restriction
function tomb_king_final_battle_completed(context)
	local pb = cm:model():pending_battle();
	local battle_key = pb:set_piece_battle_key();
	
	if battle_key == "wh2_dlc09_qb_tmb_final_battle_settra" or battle_key == "wh2_dlc09_qb_tmb_final_battle_khatep" or battle_key == "wh2_dlc09_qb_tmb_final_battle_khalida" then
		local this_char_cqi, this_mf_cqi, current_faction_name = cm:pending_battle_cache_get_attacker(1);
		cm:remove_effect_bundle_from_force("wh2_dlc09_bundle_tmb_final_battle_army_abilities_barrage", this_mf_cqi);
		cm:remove_effect_bundle_from_force("wh2_dlc09_bundle_tmb_final_battle_army_abilities_swarm", this_mf_cqi);
	end
end