...
function forced_battle:add_new_force(force_key, unit_list, faction_key, destroy_after_battle, opt_effect_bundle,opt_general_subtype,opt_general_level)
	...
	if force_list[force_key] ~= nil then
		script_error("ERROR: Forced Battle Manager: Forced Battle with key [" .. key .. "] already exists!");
		return false;
	end
...
function Forced_Battle_Manager:load_listeners()
	if is_string(self.active_battle) then
...
function forced_battle:forced_battle_stage_2()
		...
		if self.attacker.interface == nil then
			local attacker_invasion_object = invasion_manager:get_invasion(self.attacker.force_key)
			self.attacker.interface = attacker_invasion_object:get_force()
			self.attacker.cqi = self.attacker.interface:command_queue_index()
		end

		if self.target.interface == nil then
			local target_invasion_object = invasion_manager:get_invasion(self.target.force_key)
			self.target.interface = target_invasion_object:get_force()
			self.target.cqi = self.target.interface:command_queue_index()
		end	

		-- can't ambush a garrisoned force
		if self.target.interface:has_garrison_residence() then
		...
		if self.attacker.interface:active_stance() == "MILITARY_FORCE_ACTIVE_STANCE_TYPE_DOUBLE_TIME" or self.attacker.interface:active_stance() ==  "MILITARY_FORCE_ACTIVE_STANCE_TYPE_MARCH" then
			local uim = cm:get_campaign_ui_manager();
		...
		if not self.attacker.faction_interface:at_war_with(self.target.faction_interface) then
			...
			cm:force_declare_war(self.attacker.faction_interface:name(), self.target.faction_interface:name(),false,false, false)
		else
			 cm:force_attack_of_opportunity(self.attacker.cqi, self.target.cqi, self.is_ambush)
		end
...
function forced_battle:clear_up_forced_battle()
	...
	Forced_Battle_Manager.active_battle = false
	self.attacker = {}
	self.target = {}	
end
...
cm:add_loading_game_callback(
	function(context)
		if not cm:is_new_game() then
			...
			Forced_Battle_Manager.active_battle = cm:load_named_value("ACTIVE_FORCED_BATTLE", false, context);
		end
	end
);