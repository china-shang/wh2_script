...
local food_raze_index = {	["wh2_dlc15_boar"] = 		{},
							...
							["wh2_dlc15_puffer_fish"] =	{},
							["wh2_dlc15_tentacle"] = 	{"wh_main_sc_nor_norsca"},
							["wh2_dlc15_glowing"] = 	{},
							["wh2_dlc15_green"] = 		{},
							["wh2_dlc15_indigo"] = 		{},
							["wh2_dlc15_pepper"] = 		{},
	...
	function(context)
		...
		for  i = 1, #ingredients do
			...
			elseif ingredients[i] == food_index[12] then
				--give scrap
				cm:faction_add_pooled_resource(Grom_faction, "grn_salvage", "wh2_dlc12_resource_factor_loot", 10);
			...
			elseif ingredients[i] == food_index[17] then
				--give waaagh
				cm:faction_add_pooled_resource(Grom_faction, "grn_waaagh", "wh2_dlc15_resource_factor_waaagh_other", 5);
...
function get_grom_enemies_from_battle()
	...
	if cm:pending_battle_cache_num_attackers() >= 1 then
		for i = 1, cm:pending_battle_cache_num_attackers() do
			...
			if cm:get_character_by_cqi(this_char_cqi) and not cm:get_character_by_cqi(this_char_cqi):region():is_null_interface() and (cm:get_character_by_cqi(this_char_cqi):region():name() == 
"wh2_main_vor_northern_yvresse_tor_yvresse" or cm:get_character_by_cqi(this_char_cqi):region():name() ==  "wh2_main_yvresse_tor_yvresse") then
				in_yvresse = true;
			end
...
function setup_food_challenge_listerner()
	...
	core:add_listener(
		...
		function(context)
				...
				cm:complete_scripted_mission_objective("wh2_dlc15_food_food_challenge", "food_challenge_"..Food_challenge_info[3], true);
				--upgrade cauldron
				local cooking_interface = cm:model():world():cooking_system():faction_cooking_info(cm:get_faction(Grom_faction));
				local current_slots = cooking_interface:max_secondary_ingredients();
				out(current_slots);
