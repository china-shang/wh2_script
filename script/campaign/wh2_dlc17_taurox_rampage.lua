...
local taurox_forename = "names_name_529446613";
...
local improved_battle_rampage = 12;
...
local momentum_matrix = {
	...
	["battle_won"] 				= 	{factor = "wh2_dlc17_taurox_bst_momentum_gained_battle_won", 		amount = 2},
	...
	["ap_replenish"] 			= 	{factor = "wh2_dlc17_taurox_bst_momentum_lost_ap_replenish", 		amount = -2},
	["ap_replenish_reduced"] 	= 	{factor = "wh2_dlc17_taurox_bst_momentum_lost_ap_replenish", 		amount = -1}
};
...
function add_taurox_rampage_listeners()
	...
	if taurox_interface then		
		...
		core:add_listener(
			...
			function(context) 
				TauroxRampage_PooledResourceEffectChangedEvent(context) 
			end,
			true
		);
...
function TauroxRampage_BattleCompleted(context)
	...
	if battle_won == true then
		...
		if attacker_result:find("victory") then
			...
			if total_rampage_value > 50 then
				total_rampage_value = 50;
			end
...
function TauroxRampage_MomentumUpdate(momentum_entry)
	...
	local taurox_faction = cm:get_faction(taurox_faction_key);
	local current_momentum = taurox_faction:pooled_resource("bst_momentum"):value();
	cm:faction_add_pooled_resource(taurox_faction_key,"bst_momentum", momentum_entry.factor, momentum_entry.amount);
	
	current_momentum = taurox_faction:pooled_resource("bst_momentum"):value();
...
function TauroxRampage_PooledResourceEffectChangedEvent(context)
	local taurox_faction = cm:get_faction(taurox_faction_key);	

	--get current bst_rampage value
	local current_rampage_value = taurox_faction:pooled_resource("bst_rampage"):value();
...
function TauroxRampage_RampageRestart(victory)
	...
	if victory == true then
		local current_momentum_value = taurox_faction:pooled_resource("bst_momentum"):value()
