...
function early_game_decide_content_delivery_mode(advice_mode, advice_key)
	...
	local advice_key_score = effect.get_advice_thread_score(advice_key);
...
function early_game_should_show_infotext(advice_mode, advice_key, event_name)
	...
	elseif core:is_advice_level_low() then
		...
		return effect.get_advice_thread_score(advice_key) < 2;
	end;
	
	script_error("WARNING: early_game_should_show_infotext() couldn't determine whether to show infotext, advice_mode is [" .. tostring(advice_mode) .. "], advice_key is [" .. tostring(advice_key) .. "], advice score is [" .. tostring(effect.get_advice_thread_score(advice_key)) .. "], event_name is [" .. tostring(event_name) .. "] and event experienced is [" .. tostring(get_early_game_event_experienced(event_name)) .. "]");
	return true;
end;
...
function get_early_game_event_experienced(event_name, savegame_only)
	...
	return effect.get_advice_history_string_seen(event_name);
end;
...
function set_early_game_event_experienced(event_name, savegame_only)
	...
	if not savegame_only then
		effect.set_advice_history_string_seen(event_name);
	end;
end;
...
function start_early_game_character_lockdown_listener(lord_cqi, ...)
	...
	do
		...
		if not character then
			if cm:model():turn_number() - cm:get_saved_value("first_turn_count_modifier") == 1 and cm:whose_turn_is_it() == cm:get_local_faction_name() then
	...
	for i = 1, arg.n do
		...
		core:add_listener(
			...
			function(context)
				...
				cm:apply_effect_bundle_to_characters_force(effect_bundle_str, lord_cqi, 1, true);
			end,
			false
		);
	end;
...
function start_early_game_capture_enemy_settlement_mission_listener(advice_key, infotext, mission_key, mission_issuer, target_id, enemy_faction_name, mission_rewards, start_from_how_they_play_event_dismissed, start_from_books_of_nagash_mission_issued, start_from_turn_two, force_advice)
	...
	local function in_eg_capture_enemy_settlement_mission_trigger()	
		...
		if delivery_mode == EARLY_GAME_SHOW_MISSION_ONLY and force_advice then
			out.design("\tforcing advice to play");
			delivery_mode = EARLY_GAME_SHOW_ADVICE_WITH_MISSION;
		else
...
function start_early_game_generic_mission_listener(listener_name, trigger_event, mission_key, advice_key, infotext, mission_conditions_defined_in_db, event_on_success, event_on_issue)
	...
	if mission_conditions_defined_in_db and n then
	...
	in_eg_generic_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_capture_province_mission_listener(advice_key, infotext, mission_key, mission_issuer, mission_rewards, optional_province_number, force_advice)
	...
	in_eg_capture_province_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_enact_commandment_mission_listener(advice_key, infotext, mission_key, mission_issuer, mission_rewards)
	...
	in_eg_enact_commandment_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_search_ruins_mission_listener(advice_key, infotext, region_key, trigger_turn, mission_key, mission_issuer, mission_rewards, factions_personalities_mapping, force_advice)
	...
	if (not cm:is_new_game() and not region:is_abandoned()) and not cm:get_saved_value("early_game_search_ruins_mission_issued") then
	...
	in_eg_search_ruins_mission:set_disregard_cost_when_triggering(true);
	...
	-- triggers if any of the player's armies can get to the specified ruin this turn
			...
			for i = 0, character_list:num_items() - 1 do
				...
				if cm:char_is_general_with_army(current_char) and current_char:has_region() then
					...
					for j = 0, adjacent_region_list:num_items() - 1 do
						...
						if current_region:is_abandoned() and cm:character_can_reach_settlement(current_char, current_region:settlement()) and current_region:name() == region_key then
							return true;
						end;
					end;
				end;
...
function start_early_game_rogue_army_mission_listener(advice_key, infotext, mission_key, mission_issuer, rogue_army_faction_name, rogue_army_position_list, rogue_army_unit_list, rogue_army_home_region, mission_rewards)
	...
	local function in_eg_rogue_armies_mission_trigger()
		...
		local random_position_list = cm:random_sort(rogue_army_position_list);
	...
	in_eg_rogue_armies_mission:set_disregard_cost_when_triggering(true);
	...
	-- SV DISABLED: 10/3/20 Uniquely, this mission won't trigger if it's present in the player's advice history, so they will only see one rogue army early-game mission until the advice history is reset
...
function start_early_game_upgrade_settlement_mission_listener(mission_key, mission_issuer, building_keys, target_region_key, mission_rewards, ignore_how_they_play, use_construct_building_in_provinces_mission_type)
	...
	in_eg_upgrade_settlement_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_tech_building_mission_listener(advice_key, infotext, mission_key, mission_issuer, buildings, mission_rewards, start_from_how_they_play_event_dismissed, building_override_list)
	...
	in_eg_construct_tech_building_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_research_tech_mission_listener(advice_key, infotext, mission_key, mission_issuer, mission_rewards, trigger_on_capture_settlement_mission_issued, should_delay_initial_firing, fire_on_turn, number_of_techs)
	...
	if not is_number(number_of_techs) then
		number_of_techs = 1;
	end;
	...
	mm:add_condition("total " .. number_of_techs);
	...
	in_eg_research_tech_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_public_order_mission_listener(advice_key, infotext, mission_key, mission_issuer, region_key, mission_rewards, disregard_first_turn_flag)
	...
	in_eg_public_order_mission:set_disregard_cost_when_triggering(true);
	...
	in_eg_public_order_mission:add_precondition(
		function()
			return (cm:get_saved_value("bool_prelude_first_turn_was_loaded") or disregard_first_turn_flag) and not get_early_game_event_experienced(event_name, true) and cm:model():turn_number() < turn_threshold + 5;
		end
	);
...
function start_early_game_eliminate_faction_mission_listener(advice_key, infotext, mission_key, mission_issuer, faction_key, number_settlements_threshold, max_turn_threshold, mission_rewards, force_advice)
	...
	in_eg_eliminate_faction_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_growth_point_mission_listener(advice_key, infotext, mission_key, mission_issuer, region_key, required_building_keys, upgrade_building_keys, mission_rewards, force_advice)
	...
	in_eg_growth_point_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_hero_building_mission_listener(advice_key, infotext, mission_key, mission_issuer, trigger_building_list, turn_delay, target_building_list, mission_rewards, start_from_growth_mission_succeeded)
	...
	in_eg_hero_building_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_hero_technology_mission_listener(advice_key, infotext, mission_key, mission_issuer, mission_rewards)
	...
	local eligible_technologies = {
		"tech_dlc09_tmb_liche_priest_1",
		"tech_dlc09_tmb_liche_priest_2",
		"tech_dlc09_tmb_liche_priest_3",
		"tech_dlc09_tmb_liche_priest_4",
		"tech_dlc09_tmb_liche_priest_5",
		"tech_dlc09_tmb_liche_priest_6",
		"tech_dlc09_tmb_necrotect_1",
		"tech_dlc09_tmb_necrotect_2",
		"tech_dlc09_tmb_necrotect_3",
		"tech_dlc09_tmb_necrotect_4",
		"tech_dlc09_tmb_necrotect_5",
		"tech_dlc09_tmb_necrotect_6",
		"tech_dlc09_tmb_tomb_prince_1",
		"tech_dlc09_tmb_tomb_prince_2",
		"tech_dlc09_tmb_tomb_prince_3",
		"tech_dlc09_tmb_tomb_prince_4",
		"tech_dlc09_tmb_tomb_prince_5",
		"tech_dlc09_tmb_tomb_prince_6"
	};
	...
	in_eg_hero_technology_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_hero_recruitment_mission_listener(advice_key, infotext, mission_key, mission_issuer, mission_rewards)
	...
	in_eg_hero_recruitment_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_hero_action_mission_listener(advice_key, infotext, mission_key, mission_issuer, mission_rewards)
	...
	in_eg_hero_action_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_raise_army_mission_listener(advice_key, infotext, mission_key, mission_issuer, mission_rewards)
	...
	in_eg_raise_army_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_non_aggression_pact_mission_listener(advice_key, infotext, mission_key, mission_issuer, target_faction_key, turn_threshold, mission_rewards)
	...
	in_eg_non_aggression_pact_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_trade_agreement_mission_listener(advice_key, infotext, mission_key, mission_issuer, target_faction_key, min_turn_threshold, max_turn_threshold, mission_rewards)
	...
	in_eg_trade_agreement_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_ritual_settlement_capture_mission_listener(advice_key, cutscene, infotext, infotext_delay, mission_key, mission_issuer, region_key, turn_threshold, mission_rewards)
	...
	in_eg_ritual_settlement_capture_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_ritual_building_mission_listener(advice_key, infotext, mission_key, mission_issuer, region_key, building_key, mission_rewards)
	...
	in_eg_construct_ritual_building_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_ritual_currency_mission_listener(advice_key, infotext, mission_trigger_currency_threshold, earn_currency_mission_key, earn_currency_mission_issuer, ritual_trigger_currency_threshold, enact_ritual_mission_key, enact_ritual_mission_issuer, ritual_key, mission_rewards, force_advice)
	...
	if not cm:get_faction(local_faction):has_pooled_resource("wh2_main_ritual_currency") then
	...
	if effect.get_advice_history_string_seen("early_game_ritual_currency_mission_completed") then
		...
		mm = mission_manager:new(
			...
			function()
				effect.set_advice_history_string_seen("early_game_ritual_currency_mission_completed");
	...
	in_eg_ritual_currency_mission:set_disregard_cost_when_triggering(true);
	...
	in_eg_ritual_currency_mission:add_trigger_condition(
		"ScriptEventPlayerFactionTurnStart",
		function()
			local faction_currency = cm:get_faction(local_faction):pooled_resource("wh2_main_ritual_currency"):value();
...
function start_early_game_ritual_mission_listener(advice_key, infotext, ritual_trigger_currency_threshold, enact_ritual_mission_key, enact_ritual_mission_issuer, ritual_key, mission_rewards, force_advice)
	...
	in_eg_ritual_mission:set_disregard_cost_when_triggering(true);
	...
	if effect.get_advice_history_string_seen("early_game_ritual_currency_mission_completed") then
		...
		in_eg_ritual_mission:add_trigger_condition(
			"ScriptEventPlayerFactionTurnStart",
			function()
				return cm:get_faction(cm:get_local_faction_name()):pooled_resource("wh2_main_ritual_currency"):value() >= ritual_trigger_currency_threshold;
			end
		);
	else
...
function start_early_game_resources_mission_listener(advice_key, infotext, region_key, turn_threshold, mission_key, resource_key, mission_issuer, mission_rewards)
	...
	in_eg_resources_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_pooled_resource_mission_listener(advice_key, infotext, mission_key, resource_name, resource_min_threshold, resource_mission_threshold, mission_issuer, mission_rewards)
	...
	in_eg_pooled_resource_mission:set_disregard_cost_when_triggering(true);
	...
	in_eg_pooled_resource_mission:add_trigger_condition(
		...
		function(context)
			local faction = cm:get_faction(local_faction);
			local pooled_resources = faction:pooled_resources();
...
function start_early_game_mortuary_cult_mission_listener(advice_key, infotext, mission_key, mission_issuer, mission_rewards)
	...
	in_eg_mortuary_cult_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_tmb_recruitment_building_mission_listener(advice_key, infotext, mission_key, trigger_turn, upgrade_buildings_list, mission_issuer, mission_rewards)
	...
	in_eg_tmb_recruitment_building_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_tmb_recruitment_building_guide_listener(advice_key, infotext, mission_key, num_units, mission_issuer, mission_rewards)
											...
											function(context)
												...
												cm:callback(
													function()
														...
														core:show_fullscreen_highlight_around_components(25, false, unpack(list_of_unit_cards));
														...
														local tp_unit_cap = text_pointer:new("unit_cap", uic_cap_pos_x + uic_cap_size_x * 0.75, uic_cap_pos_y + (uic_cap_size_y / 2), 100, "left");
														...
														tp_unit_cap:set_style("semitransparent_2_sec_highlight");
														...
														local tp_unit_capless = text_pointer:new("unit_capless", uic_capless_pos_x + (uic_capless_size_x / 2), uic_capless_pos_y + (uic_capless_size_y / 2), 100, "right");
														...
														tp_unit_capless:set_style("semitransparent_2_sec_highlight");
	...
	local function in_eg_tmb_recruitment_building_guide_trigger()
		...
		if (core:is_advice_level_minimal() or effect.get_advice_thread_score(advice_key) > 0) and not core:is_tweaker_set("SKIP_SCRIPTED_CONTENT_5") then
	...
	in_eg_tmb_recruitment_building_guide:set_disregard_cost_when_triggering(true);
	
	in_eg_tmb_recruitment_building_guide:set_wait_for_event_dismissed(false);
...
function start_early_game_books_of_nagash_mission_listener(mission_key, mission_issuer, mission_rewards, building_override_list)
	...
	local function start_books_of_nagash_scripted_tour(completion_callback)
		...
		effect.set_advice_history_string_seen("books_of_nagash_panel_tour");
	
		local uic_books_of_nagash_button = find_uicomponent(core:get_ui_root(), "layout", "resources_bar", "right_spacer_tomb_kings", "button_books_of_nagash");
		...
		core:show_fullscreen_highlight_around_components(25, false, uic_books_of_nagash_button);
		...
		local tp_books_of_nagash_button = text_pointer:new("books_of_nagash_button", uic_button_pos_x + uic_button_size_x / 2, uic_button_pos_y + (uic_button_size_y / 2), 100, "top");
		...
		tp_books_of_nagash_button:set_style("semitransparent_2_sec_highlight");
							...
							function(context)
								...
								core:progress_on_uicomponent_animation(
									...
									function()
										...
										core:show_fullscreen_highlight_around_components(25, false, uic_map);
										...
										local tp_map = text_pointer:new("map", uic_map_pos_x + (uic_map_size_x / 4), uic_map_pos_y + (uic_map_size_y / 2), 100, "right");
										...
										tp_map:set_style("semitransparent_2_sec_highlight");
										...
										tp_map:set_close_button_callback(
											function()
												...
												local uic_close_button = find_uicomponent(core:get_ui_root(), "layout", "hud_center", "small_bar", "button_close_holder", "button_close");
	...
	local function in_eg_books_of_nagash_mission_trigger()
		...
		mm:trigger(
			function()
				...
				if core:is_tweaker_set("SKIP_SCRIPTED_CONTENT_6") or not effect.get_advice_history_string_seen("books_of_nagash_panel_tour") then
	...
	in_eg_books_of_nagash_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_cst_pirate_cove_mission_listener(advice_key, infotext, mission_key, mission_issuer, buildings, mission_rewards)
	...
	in_eg_construct_pirate_cove_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_cst_growth_point_ship_mission_listener(advice_key, infotext, mission_key, mission_issuer, faction_key, upgrade_building_keys, mission_rewards)
	...
	in_eg_growth_point_ship_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_cst_loyalty_mission_listener(advice_key, infotext, mission_key, mission_issuer, faction_key, mission_rewards)
	...
	in_eg_cst_loyalty_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_cst_treasure_hunting_mission_listener(advice_key, infotext, mission_key)
	...
	in_eg_cst_treasure_hunting_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_cst_search_for_the_vengeance_mission_listener(advice_key, infotext, mission_key, target_faction_key, target_region_key)
	...
	in_eg_cst_search_for_the_vengeance_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_recruit_hunter_mission_listener(advice_key, infotext, mission_key, turn, mission_issuer, mission_rewards)
	...
	in_eg_recruit_hunter_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_nakai_gift_region_mission_listener(advice_key, infotext, region_key, mission_key, mission_issuer, mission_rewards)
	...
	mm:add_new_scripted_objective(
		...
		function(context)
			...
			if faction:name() == "wh2_dlc13_lzd_spirits_of_the_jungle" and faction:is_human() == true then
				...
				if context:occupation_decision() == "1132333820" then			--Quetzl
					return true;
				elseif context:occupation_decision() == "1485462950" then 	--Itzl
					return true;
				elseif context:occupation_decision() == "1999603937" then		--Xholankha
					return true;
				else
					return false;
				end;
	...
	in_eg_nakai_gift_region_mission:set_disregard_cost_when_triggering(true);
...
function start_early_game_shadowy_dealings_mission_listener(mission_key, mission_issuer, mission_rewards)

	if not is_string(mission_key) then
		script_error("ERROR: start_early_game_shadowy_dealings_mission_listener() called but supplied mission key [" .. tostring(mission_key) .. "] is not a string");
		return false;
	end;
	
	
	if mission_issuer and not is_string(mission_issuer) then
		script_error("ERROR: start_early_game_shadowy_dealings_mission_listener() called but supplied mission issuer [" .. tostring(mission_issuer) .. "] is not a string or nil");
		return false;
	end;
	...
	if not is_table(mission_rewards) then
		script_error("ERROR: start_early_game_shadowy_dealings_mission_listener() called but supplied mission rewards [" .. mission_rewards .. "] is not a table");
		return false;
	end;
	
	if #mission_rewards == 0 then
		script_error("ERROR: start_early_game_shadowy_dealings_mission_listener() called but supplied mission rewards table is empty");
		return false;
	end;
	
	for i = 1, #mission_rewards do
		if not is_string(mission_rewards[i]) then
			script_error("ERROR: start_early_game_shadowy_dealings_mission_listener() called but supplied mission reward [" .. i .. "] is [" .. tostring(mission_rewards[i]) .. "] and not a string");
			return false;
		end;
	end;
	
	local local_faction = cm:get_local_faction_name();
	local event_name = "early_game_shadowy_dealings_mission_listener";
	...
	mm:add_new_scripted_objective(
		"mission_text_text_wh2_dlc14_objective_override_eg_mission_shadowy_dealings",
		"ScriptEventShadowyDealingsEGMission",
		true
	);
	...
	--
	-- Shadowy Dealings scripted tour
	--
	
	local function shadowy_dealings_scripted_tour_disable_ui(should_disable)
		local uim = cm:get_campaign_ui_manager();
	...
	local function start_shadowy_dealings_scripted_tour(completion_callback)
	
		out("%%%%% start_shadowy_dealings_scripted_tour() called");
		
		effect.set_advice_history_string_seen("shadowy_dealings_panel_tour");
	
		local uic_shadowy_dealings_button = find_uicomponent(core:get_ui_root(), "layout", "faction_buttons_docker", "button_group_management", "button_shadowy_dealings");
		if not uic_shadowy_dealings_button then
			script_error("ERROR: start_shadowy_dealings_scripted_tour() could not find the shadowy dealings button, how can this be?");
			return false;
		end;
		
		-- lock ui
		shadowy_dealings_scripted_tour_disable_ui(true);
		
		-- show fullscreen highlight around the shadowy dealings button
		core:show_fullscreen_highlight_around_components(25, false, uic_shadowy_dealings_button);
		
		-- pulse the button
		pulse_uicomponent(uic_shadowy_dealings_button, true, 5);
		
		-- set up button text pointer
		local uic_button_size_x, uic_button_size_y = uic_shadowy_dealings_button:Dimensions();
		local uic_button_pos_x, uic_button_pos_y = uic_shadowy_dealings_button:Position();
		
		local tp_shadowy_dealings_button = text_pointer:new("button_shadowy_dealings", uic_button_pos_x + uic_button_size_x / 5, uic_button_pos_y + (uic_button_size_y / 2), 100, "right");
		tp_shadowy_dealings_button:set_layout("text_pointer_text_only");
		tp_shadowy_dealings_button:add_component_text("text", "ui_text_replacements_localised_text_dlc14_text_pointer_shadowy_dealings_button");
		tp_shadowy_dealings_button:set_style("semitransparent_2_sec_highlight");
		tp_shadowy_dealings_button:set_panel_width(300);
		
		tp_shadowy_dealings_button:set_close_button_callback(
			function()
				
				-- stop pulsing button
				pulse_uicomponent(uic_shadowy_dealings_button, false, 5);
				
				cm:callback(
					function() 
						-- hide text pointer
						tp_shadowy_dealings_button:hide();
					
						-- lift fullscreen highlight
						core:hide_fullscreen_highlight();

						-- establish listener for the shadowy dealings panel opening						
						core:add_listener(
							event_name,
							"PanelOpenedCampaign",
							function(context)
							...
							function(context)
								out("%%%%% Shadowy Dealings panel opened");
							
								cm:remove_callback(event_name);
								
								local uic_panel = find_child_uicomponent(core:get_ui_root(), "shadowy_dealings_panel");
								if not uic_panel then
									script_error("ERROR: start_shadowy_dealings_scripted_tour() could not find the shadowy dealings panel, how can this be?");
									shadowy_dealings_scripted_tour_disable_ui(false);
									completion_callback();
									return false;
								end;
								...
								core:progress_on_uicomponent_animation(
									...
									function()
										cm:callback(
											function()	
												-- get a handle to the map on the panel
												local uic_missions = find_uicomponent(uic_panel, "mission_section");
												
												if not uic_missions then
													script_error("WARNING: start_shadowy_dealings_scripted_tour() could not find the mission_section uicomponent on the shadowy dealings, how can this be?");
													shadowy_dealings_scripted_tour_disable_ui(false);
													completion_callback();
													return false;
												end;
												...
												-- show fullscreen highlight over the map
												core:show_fullscreen_highlight_around_components(25, false, uic_missions);
												
												-- pulse each of the book icons
												pulse_uicomponent(uic_missions, true, 5, true);
												
												-- set up text pointer
												local uic_missions_size_x, uic_missions_size_y = uic_missions:Dimensions();
												local uic_missions_pos_x, uic_missions_pos_y = uic_missions:Position();
												
												local tp_missions = text_pointer:new("mission_section", uic_missions_pos_x + uic_missions_size_x, uic_missions_pos_y + (uic_missions_size_y / 2), 100, "left");
												tp_missions:set_layout("text_pointer_text_only");
												tp_missions:add_component_text("text", "ui_text_replacements_localised_text_dlc14_text_pointer_shadowy_dealings_panel_missions");
												tp_missions:set_style("semitransparent_2_sec_highlight");
												tp_missions:set_panel_width(250);
												
												tp_missions:set_close_button_callback(
													function()
														-- hide text pointer
														tp_missions:hide();
														
														-- stop pulsing highlight of book icons
														pulse_uicomponent(uic_missions, false, 5, true);
														
														-- lift fullscreen highlight
														core:hide_fullscreen_highlight();
														
														local uic_middle = find_uicomponent(uic_panel, "collective_info");
												
														if not uic_middle then
															script_error("WARNING: start_shadowy_dealings_scripted_tour() could not find the collective_info uicomponent on the shadowy dealings, how can this be?");
															shadowy_dealings_scripted_tour_disable_ui(false);
															completion_callback();
															return false;
														end;
														...
														-- show fullscreen highlight over the map
														core:show_fullscreen_highlight_around_components(25, false, uic_middle);
														
														-- pulse each of the book icons
														pulse_uicomponent(uic_middle, true, 5, true);
														
														-- set up text pointer
														local uic_middle_size_x, uic_middle_size_y = uic_middle:Dimensions();
														local uic_middle_pos_x, uic_middle_pos_y = uic_middle:Position();
														
														local tp_middle = text_pointer:new("collective_info", uic_middle_pos_x + uic_middle_size_x, uic_middle_pos_y + (uic_middle_size_y / 2), 100, "left");
														tp_middle:set_layout("text_pointer_text_only");
														tp_middle:add_component_text("text", "ui_text_replacements_localised_text_dlc14_text_pointer_shadowy_dealings_panel_middle");
														tp_middle:set_style("semitransparent_2_sec_highlight");
														tp_middle:set_panel_width(250);

														tp_middle:set_close_button_callback(
															function()
																-- hide text pointer
																tp_middle:hide();
																
																-- stop pulsing highlight of book icons
																pulse_uicomponent(uic_middle, false, 5, true);
																
																-- lift fullscreen highlight
																core:hide_fullscreen_highlight();
																
																local uic_character = find_uicomponent(uic_panel, "character_section");
												
																if not uic_character then
																	script_error("WARNING: start_shadowy_dealings_scripted_tour() could not find the character_section uicomponent on the shadowy dealings, how can this be?");
																	shadowy_dealings_scripted_tour_disable_ui(false);
																	completion_callback();
																	return false;
																end;
																...
																-- show fullscreen highlight over the map
																core:show_fullscreen_highlight_around_components(25, false, uic_character);
																
																-- pulse each of the book icons
																pulse_uicomponent(uic_character, true, 5, true);
																
																-- set up text pointer
																local uic_character_size_x, uic_character_size_y = uic_character:Dimensions();
																local uic_character_pos_x, uic_character_pos_y = uic_character:Position();
																
																local tp_character = text_pointer:new("character_section", uic_character_pos_x, uic_character_pos_y + (uic_character_size_y / 2), 100, "right");
																tp_character:set_layout("text_pointer_text_only");
																tp_character:add_component_text("text", "ui_text_replacements_localised_text_dlc14_text_pointer_shadowy_dealings_panel_character");
																tp_character:set_style("semitransparent_2_sec_highlight");
																tp_character:set_panel_width(250);

																tp_character:set_close_button_callback(
																	function()
																		-- hide text pointer
																		tp_character:hide();
																		
																		-- stop pulsing highlight of book icons
																		pulse_uicomponent(uic_character, false, 5, true);
																		
																		-- lift fullscreen highlight
																		core:hide_fullscreen_highlight();
																		
																		local uic_top = find_uicomponent(core:get_ui_root(), "topbar_list_parent", "warpstone_dust_holder");
														
																		if not uic_top then
																			script_error("WARNING: start_shadowy_dealings_scripted_tour() could not find the warpstone_dust_holder uicomponent on the topbar_list_parent, how can this be?");
																			shadowy_dealings_scripted_tour_disable_ui(false);
																			completion_callback();
																			return false;
																		end;
																		...
																		-- show fullscreen highlight over the map
																		core:show_fullscreen_highlight_around_components(25, false, uic_top);
																		
																		-- pulse each of the book icons
																		pulse_uicomponent(uic_top, true, 5, true);
																		
																		-- set up text pointer
																		local uic_top_size_x, uic_top_size_y = uic_top:Dimensions();
																		local uic_top_pos_x, uic_top_pos_y = uic_top:Position();
																		
																		local tp_top = text_pointer:new("warpstone_dust_holder", uic_top_pos_x + (uic_top_size_x/5), uic_top_pos_y + uic_top_size_y, 100, "top");
																		tp_top:set_layout("text_pointer_text_only");
																		tp_top:add_component_text("text", "ui_text_replacements_localised_text_dlc14_text_pointer_shadowy_dealings_top_bar_resource");
																		tp_top:set_style("semitransparent_2_sec_highlight");
																		tp_top:set_panel_width(400);

																		tp_top:set_close_button_callback(
																			function()																		
																				-- hide text pointer
																				tp_top:hide();
																				
																				-- stop pulsing highlight of book icons
																				pulse_uicomponent(uic_top, false, 5, true);
																				
																				-- lift fullscreen highlight
																				core:hide_fullscreen_highlight();
																				
																				-- highlight panel close button
																				local uic_close_button = find_uicomponent(core:get_ui_root(), "shadowy_dealings_panel", "small_bar", "buttons_holder", "button_close");
																				if not uic_close_button then
																					script_error("WARNING: start_shadowy_dealings_scripted_tour() could not find the shadowy dealings close button, how can this be?");
																					shadowy_dealings_scripted_tour_disable_ui(false);
																					completion_callback();
																					return false;
																				end;
						...
						cm:callback(
							function()
								script_error("WARNING: start_shadowy_dealings_scripted_tour() attempted to open the Shadowy Dealings panel but it didn't open, exiting");
								core:remove_listener(event_name);
								shadowy_dealings_scripted_tour_disable_ui(false);
								completion_callback();
								return false;
						...
						-- simulate click on shadowy dealings button to open the shadowy dealings panel
						uic_shadowy_dealings_button:SimulateLClick();
					end, 
					0.5
				);
			end
		...
		tp_shadowy_dealings_button:show();
	end;
	
	
	-- pre-declare the intervention variable, so that it exists as a local for the subsequent function
	local in_eg_shadowy_dealings_mission = false;
	
	-- intervention trigger function
	local function in_eg_shadowy_dealings_mission_trigger()
		out.design("[EG] start_early_game_shadowy_dealings_mission_listener() is triggering mission, advice level is " .. core:get_advice_level());
		out.design("\tjust issuing mission");
		set_early_game_event_experienced(event_name, true);
		mm:trigger(
			function()
				local function completion_callback()
					--this is intentionally saying books of nagash so that the early game caprute enemy settlement mission doesnt need to be changed
					core:trigger_event("ScriptEventBooksOfNagashMissionIssued");
					in_eg_shadowy_dealings_mission:complete();
				end;
			
				-- play books of nagash scripted tour if it's not been seen already
				if core:is_tweaker_set("SKIP_SCRIPTED_CONTENT_8") or not effect.get_advice_history_string_seen("shadowy_dealings_panel_tour") then
					start_shadowy_dealings_scripted_tour(completion_callback);
				else
					completion_callback();
				end;
			end
	...
	-- set up intervention	
	in_eg_shadowy_dealings_mission = intervention:new(
		"in_eg_shadowy_dealings_mission", 												-- string name
		0,	 																		-- cost
		function() in_eg_shadowy_dealings_mission_trigger() end,					-- trigger callback
		BOOL_INTERVENTIONS_DEBUG	 												-- show debug output
	);
	
	in_eg_shadowy_dealings_mission:set_disregard_cost_when_triggering(true);
	
	in_eg_shadowy_dealings_mission:add_precondition(
		function()
			return not get_early_game_event_experienced(event_name, true);
		end
	);
	...
	-- trigger after how-they-play event is received
	in_eg_shadowy_dealings_mission:add_trigger_condition(
		"ScriptEventHowTheyPlayEventDismissed",
		true
	);
		
	in_eg_shadowy_dealings_mission:start();
end;
...
function start_early_game_possession_mission_listener(mission_key, mission_issuer, mission_rewards)

	if not is_string(mission_key) then
		script_error("ERROR: start_early_game_possession_mission_listener() called but supplied mission key [" .. tostring(mission_key) .. "] is not a string");
		return false;
	end;
	
	
	if mission_issuer and not is_string(mission_issuer) then
		script_error("ERROR: start_early_game_possession_mission_listener() called but supplied mission issuer [" .. tostring(mission_issuer) .. "] is not a string or nil");
		return false;
	end;
	...
	if not is_table(mission_rewards) then
		script_error("ERROR: start_early_game_possession_mission_listener() called but supplied mission rewards [" .. mission_rewards .. "] is not a table");
		return false;
	end;
	
	if #mission_rewards == 0 then
		script_error("ERROR: start_early_game_possession_mission_listener() called but supplied mission rewards table is empty");
		return false;
	end;
	
	for i = 1, #mission_rewards do
		if not is_string(mission_rewards[i]) then
			script_error("ERROR: start_early_game_possession_mission_listener() called but supplied mission reward [" .. i .. "] is [" .. tostring(mission_rewards[i]) .. "] and not a string");
			return false;
		end;
	end;
	
	local local_faction = cm:get_local_faction_name();
	local event_name = "early_game_possession_mission_listener";
	...
	mm:add_new_scripted_objective(
		"mission_text_text_wh2_dlc14_objective_override_eg_mission_possession",
	...
	--
	-- Possession scripted tour
	--
	
	local function possession_scripted_tour_disable_ui(should_disable)
		local uim = cm:get_campaign_ui_manager();
	...
	local function start_possession_scripted_tour(completion_callback)
	
		out("%%%%% start_possession_scripted_tour() called");
		
		effect.set_advice_history_string_seen("possession_tour");
		
		local uic_topbar = find_uicomponent(core:get_ui_root(), "topbar_list_parent", "malus_holder", "malus_top_bar");
		if not uic_topbar then
			script_error("ERROR: start_possession_scripted_tour() could not find the malus_top_bar, how can this be?");
			possession_scripted_tour_disable_ui(false);
			completion_callback();
			return false;
		end;
		...
		local uic_possession_orb = find_uicomponent(uic_topbar, "posession_orb");
		if not uic_possession_orb then
			script_error("ERROR: start_possession_scripted_tour() could not find the possession_orb, how can this be?");
			return false;
		end;
		
		-- lock ui
		possession_scripted_tour_disable_ui(true);
		
		-- show fullscreen highlight around the posession orb
		core:show_fullscreen_highlight_around_components(25, false, uic_possession_orb);
		
		-- pulse the button
		pulse_uicomponent(uic_possession_orb, true, 5);
		
		-- set up button text pointer
		local uic_possession_orb_size_x, uic_possession_orb_size_y = uic_possession_orb:Dimensions();
		local uic_possession_orb_pos_x, uic_possession_orb_pos_y = uic_possession_orb:Position();
		
		local tp_possession_orb = text_pointer:new("posession_orb", uic_possession_orb_pos_x + 67*(uic_possession_orb_size_x/120), uic_possession_orb_pos_y + uic_possession_orb_size_y, 80, "top");
		tp_possession_orb:set_layout("text_pointer_text_only");
		tp_possession_orb:add_component_text("text", "ui_text_replacements_localised_text_dlc14_text_pointer_possession_orb");
		tp_possession_orb:set_style("semitransparent_2_sec_highlight");
		tp_possession_orb:set_panel_width(400);
		
		tp_possession_orb:set_close_button_callback(
			function()
				
				-- stop pulsing button
				pulse_uicomponent(uic_possession_orb, false, 5);
				-- hide text pointer
				tp_possession_orb:hide();
			
				-- lift fullscreen highlight
				core:hide_fullscreen_highlight();

				-- get a handle to the map on the panel
				local uic_potion = find_uicomponent(uic_topbar, "posession_orb","malus_potion", "round_small_button");
				
				if not uic_potion then
					script_error("WARNING: start_possession_scripted_tour() could not find the malus_potion uicomponent on the top_bar, how can this be?");
					possession_scripted_tour_disable_ui(false);
					completion_callback();
					return false;
				end;
				...
				-- show fullscreen highlight over the map
				core:show_fullscreen_highlight_around_components(25, false, uic_potion);
				
				-- pulse each of the book icons
				pulse_uicomponent(uic_potion, true, 5, true);
				
				-- set up text pointer
				local uic_potion_size_x, uic_potion_size_y = uic_potion:Dimensions();
				local uic_potion_pos_x, uic_potion_pos_y = uic_potion:Position();
				
				local tp_potion = text_pointer:new("malus_potion", uic_possession_orb_pos_x + uic_possession_orb_size_x / 5, uic_potion_pos_y + uic_potion_size_y, 120, "top");
				tp_potion:set_layout("text_pointer_text_only");
				tp_potion:add_component_text("text", "ui_text_replacements_localised_text_dlc14_text_pointer_possession_elixir");
				tp_potion:set_style("semitransparent_2_sec_highlight");
				tp_potion:set_panel_width(300);
				
				tp_potion:set_close_button_callback(
					function()
						-- hide text pointer
						tp_potion:hide();
						
						-- stop pulsing highlight of book icons
						pulse_uicomponent(uic_potion, false, 5, true);
						
						-- lift fullscreen highlight
						core:hide_fullscreen_highlight();
						
						local uic_whispers = find_uicomponent(uic_topbar, "tzarkans_whispers");
				
						if not uic_whispers then
							script_error("WARNING: start_possession_scripted_tour() could not find the tzarkans_whispers uicomponent on the stop bar, how can this be?");
							possession_scripted_tour_disable_ui(false);
							completion_callback();
							return false;
						end;
						...
						-- show fullscreen highlight over the map
						core:show_fullscreen_highlight_around_components(25, false, uic_whispers);
						
						-- pulse each of the book icons
						pulse_uicomponent(uic_whispers, true, 5, true);
						
						-- set up text pointer
						local uic_whispers_size_x, uic_whispers_size_y = uic_whispers:Dimensions();
						local uic_whispers_pos_x, uic_whispers_pos_y = uic_whispers:Position();
						
						local tp_whispers = text_pointer:new("tzarkans_whispers", uic_whispers_pos_x + 8*(uic_whispers_size_x/15), uic_whispers_pos_y + 19*(uic_whispers_size_y/16), 80, "top");
						tp_whispers:set_layout("text_pointer_text_only");
						tp_whispers:add_component_text("text", "ui_text_replacements_localised_text_dlc14_text_pointer_whispers");
						tp_whispers:set_style("semitransparent_2_sec_highlight");
						tp_whispers:set_panel_width(300);

						tp_whispers:set_close_button_callback(
							function()
								-- hide text pointer
								tp_whispers:hide();
								
								-- stop pulsing highlight of book icons
								pulse_uicomponent(uic_whispers, false, 5, true);
		...
		tp_possession_orb:show();
	end;
	
	
	-- pre-declare the intervention variable, so that it exists as a local for the subsequent function
	local in_eg_possession_mission = false;
	
	-- intervention trigger function
	local function in_eg_possession_mission_trigger()
		out.design("[EG] start_early_game_possession_mission_listener() is triggering mission, advice level is " .. core:get_advice_level());
		out.design("\tjust issuing mission");
		set_early_game_event_experienced(event_name, true);
		mm:trigger(
			function()
				local function completion_callback()
					--this is intentionally saying books of nagash so that the early game capture enemy settlement mission doesnt need to be changed
					core:trigger_event("ScriptEventBooksOfNagashMissionIssued");
					in_eg_possession_mission:complete();
				end;
				-- play possession scripted tour if it's not been seen already
				if core:is_tweaker_set("SKIP_SCRIPTED_CONTENT_9") or not effect.get_advice_history_string_seen("possession_tour") then
					start_possession_scripted_tour(completion_callback);
				else
					completion_callback();
				end;
			end
	...
	-- set up intervention	
	in_eg_possession_mission = intervention:new(
		"in_eg_possession_mission", 												-- string name
		0,	 																		-- cost
		function() in_eg_possession_mission_trigger() end,							-- trigger callback
		BOOL_INTERVENTIONS_DEBUG	 												-- show debug output
	);
	
	in_eg_possession_mission:set_disregard_cost_when_triggering(true);
	
	in_eg_possession_mission:add_precondition(
		function()
			return not get_early_game_event_experienced(event_name, true);
		end
	);
	...
	-- trigger after how-they-play event is received
	in_eg_possession_mission:add_trigger_condition(
		"ScriptEventFireElixirEGMission",
		true
	);
		
	in_eg_possession_mission:start();
end;


function start_early_game_ritual_category_mission_listener(advice_key, infotext, mission_key, ritual_category, resource_key, resource_threshold, mission_issuer, mission_rewards, text_override)

	if not is_string(advice_key) then
		script_error("ERROR: start_early_game_ritual_category_mission_listener() called but supplied advice key [" .. tostring(advice_key) .. "] is not a string");
		return false;
	end;
	...
	if not is_table(infotext) then
		script_error("ERROR: start_early_game_ritual_category_mission_listener() called but supplied infotext [" .. tostring(infotext) .. "] is not a table or nil");
		return false;
	end;
	
	if not is_string(mission_key) then
		script_error("ERROR: start_early_game_ritual_category_mission_listener() called but supplied mission key [" .. tostring(mission_key) .. "] is not a string");
		return false;
	end;

	if not is_string(mission_key) then
		script_error("ERROR: start_early_game_ritual_category_mission_listener() called but supplied ritual category stirng [" .. tostring(mission_key) .. "] is not a string");
		return false;
	end;
		
	if mission_issuer and not is_string(mission_issuer) then
		script_error("ERROR: start_early_game_ritual_category_mission_listener() called but supplied mission issuer [" .. tostring(ritual_category) .. "] is not a string or nil");
		return false;
	end;

	if text_override and not is_string(text_override) then
		script_error("ERROR: start_early_game_ritual_category_mission_listener() called but supplied text override [" .. tostring(text_override) .. "] is not a string");
		return false;
	end
	...
	if not is_table(mission_rewards) then
		script_error("ERROR: start_early_game_ritual_category_mission_listener() called but supplied mission rewards [" .. mission_rewards .. "] is not a table");
		return false;
	end;
	
	if #mission_rewards == 0 then
		script_error("ERROR: start_early_game_ritual_category_mission_listener() called but supplied mission rewards table is empty");
		return false;
	end;
	
	for i = 1, #mission_rewards do
		if not is_string(mission_rewards[i]) then
			script_error("ERROR: start_early_game_ritual_category_mission_listener() called but supplied mission reward [" .. i .. "] is [" .. tostring(mission_rewards[i]) .. "] and not a string");
			return false;
		end;
	end;
	...
	local local_faction = cm:get_local_faction_name();
	local event_name = "ritual_category_mission_issued";
	...
	mm:add_new_objective("PERFORM_RITUAL");
	mm:add_condition("ritual_category "..ritual_category)
	mm:add_condition("total 1")
	if(text_override) then
		mm:add_condition("override_text "..text_override)
	end
	...
	-- pre-declare the intervention variable, so that it exists as a local for the subsequent function
	local in_eg_ritual_category_mission = false;
	
	-- intervention trigger function
	local function in_ritual_category_mission_trigger()
		out.design("[EG] start_early_game_ritual_category_mission_listener() is triggering mission, advice level is " .. core:get_advice_level());
		...
		if delivery_mode == EARLY_GAME_SHOW_MISSION_ONLY then
			...
			--mm:trigger(function() in_eg_mortuary_cult_mission:complete() end);
			mm:trigger();
			cm:callback(function() in_eg_ritual_category_mission:complete() end, 1)
		else
			if not early_game_should_show_infotext(advice_mode, advice_key, event_name) then
				out.design("\tnot showing infotext");
				infotext = {};
			...
			out.design("\tplaying advice with mission");
			in_eg_ritual_category_mission:play_advice_for_intervention(
				advice_key,
				infotext,
				mm
			);
	...
	-- set up intervention	
	in_eg_ritual_category_mission = intervention:new(
		"eg_ritual_category_mission", 												-- string name
		0,	 																		-- cost
		function() in_ritual_category_mission_trigger() end,						-- trigger callback
		BOOL_INTERVENTIONS_DEBUG	 												-- show debug output
	);

	in_eg_ritual_category_mission:set_disregard_cost_when_triggering(true);
	
	in_eg_ritual_category_mission:add_precondition(
		function()
			return not get_early_game_event_experienced(event_name, true);
		end
	);
	...
	local function check_has_resource()		
		local faction_pooled_resource = cm:get_faction(local_faction):pooled_resource(resource_key);
					
		if faction_pooled_resource:value() >= resource_threshold then			
			return true;
		end;
	...
	-- trigger if the value of the pooled resource exceeds the minimum threshold
	in_eg_ritual_category_mission:add_trigger_condition(
		"ScriptEventPlayerFactionTurnStart",
		function(context)
			return check_has_resource();
		end
	);
	...
	in_eg_ritual_category_mission:start();
end;

-----------------------------------------------------------------------------------
-----------------------------------------------------------------------------------
--
--	Generic scripted mission listener
-- 	Trigger a mission on specified event complete it on another
--
-----------------------------------------------------------------------------------
-----------------------------------------------------------------------------------

function start_generic_scripted_mission_listener(mission_key, mission_issuer, mission_rewards, mission_text, start_mission_event, complete_mission_event)

	if not is_string(mission_key) then
		script_error("ERROR: start_generic_scripted_mission_listener() called but supplied mission key [" .. tostring(mission_key) .. "] is not a string");
		return false;
	end;

	if not is_string(start_mission_event) then
		script_error("ERROR: start_generic_scripted_mission_listener() called but supplied mission key [" .. tostring(start_mission_event) .. "] is not a string");
		return false;
	end;
	
	if not is_string(complete_mission_event) then
		script_error("ERROR: start_generic_scripted_mission_listener() called but supplied mission key [" .. tostring(complete_mission_event) .. "] is not a string");
		return false;
	end;
	
	
	if mission_issuer and not is_string(mission_issuer) then
		script_error("ERROR: start_early_game_books_of_start_generic_scripted_mission_listeneragash_mission_listener() called but supplied mission issuer [" .. tostring(mission_issuer) .. "] is not a string or nil");
		return false;
	end;
	...
	if not is_table(mission_rewards) then
		script_error("ERROR: start_generic_scripted_mission_listener() called but supplied mission rewards [" .. mission_rewards .. "] is not a table");
		return false;
	end;
	
	if #mission_rewards == 0 then
		script_error("ERROR: start_generic_scripted_mission_listener() called but supplied mission rewards table is empty");
		return false;
	end;
	
	for i = 1, #mission_rewards do
		if not is_string(mission_rewards[i]) then
			script_error("ERROR: start_generic_scripted_mission_listener() called but supplied mission reward [" .. i .. "] is [" .. tostring(mission_rewards[i]) .. "] and not a string");
			return false;
		end;
	end;
	...
	-- pre-declare the intervention variable, so that it exists as a local for the subsequent function
	local event_name = "eg_mission_listener_"..mission_key;
	
	-- intervention trigger function
	local function in_eg_generic_scripted_mission_trigger()
		out.design("[EG] in_eg_generic_scripted_mission_trigger() is triggering mission, advice level is " .. core:get_advice_level());
		out.design("\tjust issuing mission");
		set_early_game_event_experienced(event_name, true);
		mm:trigger(
			function()
				local function completion_callback()
					in_eg_generic_scripted_mission:complete();
				end
				completion_callback();
			end
		)
	...
	-- set up intervention	
	in_eg_generic_scripted_mission = intervention:new(
		"in_eg_generic_scripted_mission", 											-- string name
		0,	 																		-- cost
		function() in_eg_generic_scripted_mission_trigger() end,					-- trigger callback
		BOOL_INTERVENTIONS_DEBUG	 												-- show debug output
	);
	
	in_eg_generic_scripted_mission:set_disregard_cost_when_triggering(true);
	
	in_eg_generic_scripted_mission:add_precondition(
		function()
			return not get_early_game_event_experienced(event_name, true);
		end
	);
	...
	-- trigger after how-they-play event is received
	in_eg_generic_scripted_mission:add_trigger_condition(
		start_mission_event,
		true
	);
		
	in_eg_generic_scripted_mission:start();
end;
