...
function cutscene_intro_play()
	...
	advice_to_play[1] = "wh2.camp.vortex.malekith.intro.001";
	advice_to_play[2] = "wh2.camp.vortex.malekith.intro.002";
	advice_to_play[3] = "wh2.camp.vortex.malekith.intro.003";
	advice_to_play[4] = "wh2.camp.vortex.malekith.intro.004";
	
	local cutscene_intro = campaign_cutscene:new(
		cm:get_local_faction_name(true) .. "_intro",							-- string name for this cutscene
		45.5,															-- length of cutscene in seconds
	...
	cutscene_intro:action(
		function()
			...
			cutscene_intro:cindy_playback("script/campaign/wh2_main_great_vortex/factions/wh2_main_def_naggarond/scenes/naggarond_flyover_s01.CindyScene", 0, 5);
		end,
		0
	);
...
if cm:is_multiplayer() then
	...
	-- the lord the player fights in the second battle
	enemy_secondary_lord_char_cqi = cm:get_cached_value(
		"enemy_secondary_lord_char_cqi",
		function()
			local character = cm:get_closest_general_to_position_from_faction(cm:get_faction(clan_septik_faction_str), 199, 654, false);
			if character then
				return character:cqi();
			end;
		end
...
initial_enemy_force_x = 189;
initial_enemy_force_y = 633;
...
function setup_second_enemy_force()
	...
	cm:grant_unit_to_character(enemy_char_str, "wh2_main_skv_art_plagueclaw_catapult");
	cm:grant_unit_to_character(enemy_char_str, "wh2_main_skv_inf_clanrats_0");
end;
...
cm:add_first_tick_callback_sp_new(
	function() 
		...
		if cm:get_saved_value("bool_prelude_first_turn_was_loaded") then
			...
			cm:set_public_order_of_province_for_region(naggarond_region_str, -34);
		end;
		
		-- set enemy faction personality
		cm:force_change_cai_faction_personality(clan_septik_faction_str, "wh_script_foolishly_brave");
