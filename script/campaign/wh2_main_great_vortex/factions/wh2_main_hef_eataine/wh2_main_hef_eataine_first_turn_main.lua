...
if cm:get_saved_value("bool_first_turn_main_end_reached") then
	...
	-- debug only - called when skipping to this section
	...
	cm:grant_unit_to_character(player_char_str, "wh2_main_hef_inf_archers_0");
	cm:grant_unit_to_character(player_char_str, "wh2_main_hef_inf_archers_0");
	cm:grant_unit_to_character(player_char_str, "wh2_main_hef_cav_silver_helms_0");
	cm:grant_unit_to_character(player_char_str, "wh2_main_hef_cav_silver_helms_0");
...
function cutscene_intro_play()			-- called from <faction_name>_start.lua
	...
	cm:set_camera_position(336.9, 332.0, 5.6, 0.0, 4.0);
	...
	cm:scroll_camera_with_cutscene(
		...
		{336.9, 332.0, 16.7, 0, 14}
	);
	
	cm:callback(
		function()
			-- The Phoenix King sends his congratulations, Lord Tyrion, for the The Druchii invaders have been driven from the field! Yet the fight to secure Lothern has only just begun.
			cm:show_advice("wh2.camp.prelude.ft.101.hef", true);
			cm:add_infotext(
				1,
...
function play_intro_post_battle_advice()
	...
	cm:callback(
		function()
			-- The Phoenix King sends his congratulations, Lord Tyrion, for the The Druchii invaders have been driven from the field! Yet the fight to secure Lothern has only just begun.
			cm:show_advice("wh2.camp.prelude.ft.101.hef");
			cm:add_infotext(
				1,
				"war.camp.advice.post_battle_options.info_001",
				"war.camp.advice.post_battle_options.info_002"
...
first_turn_main_movement_destination_x = 515;
first_turn_main_movement_destination_y = 426;

function start_movement_advice()
	
	local ma = intro_campaign_movement_advice:new(
		player_legendary_lord_char_cqi,
		-- The enemy still threaten your capital, Lord Tyrion. Move now to block the route from the south and close off the possibility of attack.
		"wh2.camp.prelude.ft.130.hef",
		"wh2.camp.army_selection_advice.001",
		"wh2.camp.movement_advice.001",				-- Move your army to defend the approach to Lothern.
...
function start_recruitment_advice()
	...
	local ra = intro_campaign_recruitment_advice:new(
		player_legendary_lord_char_cqi,
		-- More elves stand ready to take up arms in the name of the Phoenix King, Lord Tyrion. Recruit them now, and they will be ready to fight in the battles that are to come.
		"wh2.camp.prelude.ft.150.hef",
	...
	ra:add_initial_infotext(
		1,
		"wh2.camp.intro.info_060",
		"wh2.camp.intro.info_061"
	);
		
	ra:add_unit_card_infotext(
		"wh2.camp.intro.info_062",
		"wh2.camp.intro.info_063"
	);
...
function start_settlement_advice()
	...
	local sa = intro_campaign_select_settlement_advice:new(
		lothern_region_str,
		lothern_province_str,
		-- It's time to review what destruction the Druchii have wrought on your territory, my Lord. Select Lothern to inspect your province.
		"wh2.camp.prelude.ft.170.hef",
		"wh2.camp.select_settlement_advice.001",
...
function start_province_advice()
	...
	cm:callback(
		function()
			...
			cm:make_region_visible_in_shroud(local_faction, lothern_region_str);
			cm:make_region_visible_in_shroud(local_faction, glittering_tower_region_str);
			cm:make_region_visible_in_shroud(local_faction, tower_of_lysean_region_str);
...
function start_building_construction_advice()
	...
	local ba = intro_campaign_building_construction_advice:new(
		...
		"wh2.camp.select_settlement_advice.001", 				-- Select Lothern
		"wh2_main_special_settlement_lothern_1",
...
function play_end_turn_advice()
	...
	ea:add_infotext(
		1,
		"wh2.camp.intro.info_120",
		"wh2.camp.intro.info_121",
		"wh2.camp.intro.info_122"
	);
...
function player_ends_turn()
	...
	local enemy_faction_str = cult_of_excess_faction_str;
...
function enemy_faction_starts_turn(enemy_faction_str)
	...
	local char_enemy = cm:get_closest_general_to_position_from_faction(enemy_faction_str, second_enemy_force_start_x, second_enemy_force_start_y, false);
...
function enemy_attack_launched()
	...
	cm:add_custom_battlefield(
		...
		"",																		-- loading override
		"script/battle/intro_battles/high_elves/second/battle_start.lua",		-- script override
	...
	cm:pending_battle_add_scripted_tile_upgrade_tag("force_valid_location:north");
