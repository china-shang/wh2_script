...
if cm:get_saved_value("bool_first_turn_main_end_reached") then
	...
	cm:grant_unit_to_character(player_char_str, "wh2_main_lzd_inf_skink_cohort_1");
	cm:grant_unit_to_character(player_char_str, "wh2_main_lzd_inf_skink_cohort_1");
	cm:grant_unit_to_character(player_char_str, "wh2_main_lzd_cav_cold_ones_1");
	cm:grant_unit_to_character(player_char_str, "wh2_main_lzd_cav_cold_ones_1");
...
function cutscene_intro_play()			-- called from <faction_name>_start.lua
	...
	cm:set_camera_position(74.2, 298.7, 7.3, 0.0, 5.0);
	...
	cm:scroll_camera_with_cutscene(
		...
		{73.9, 299.5, 16.3, 0.0, 14.0}
	);
	
	cm:callback(
		function()
			-- The Skeggi flee before your awesome bulk, Lord Mazdamundi! It only remains to decide the fate of those captured in battle.
			cm:show_advice("wh2.camp.prelude.ft.101.lzd", true);
...
function play_intro_post_battle_advice()
	...
	cm:callback(
		function()
			-- The Skeggi flee before your awesome bulk, Lord Mazdamundi! It only remains to decide the fate of those captured in battle.
			cm:show_advice("wh2.camp.prelude.ft.101.lzd");
			cm:add_infotext(
				1,
...
function start_movement_extents_advice()
	...
	cm:scroll_camera_with_cutscene(
		2,
		nil,
		{74.880363, 296.050018, 10.000244, -0.000008, 10.486972}
	);
...
function movement_extents_advice_army_selected()
	...
	-- set up text pointer
	local tp_movement_extents = text_pointer:new("movement_extents", player_char:display_position_x() + 6, player_char:display_position_y() + 0.5, 50, "worldspace");
...
first_turn_main_movement_destination_x = 110;
first_turn_main_movement_destination_y = 378;

function start_movement_advice()
	
	local ma = intro_campaign_movement_advice:new(
		player_legendary_lord_char_cqi,
		-- As one Skeggi force is defeated, so may others be sent to replace it. Move now to block the route to your capital, lest the warmbloods risk another attack.
		"wh2.camp.prelude.ft.130.lzd",
		"wh2.camp.army_selection_advice.001",
		"wh2.camp.movement_advice.003",				-- Move your army to defend the approach to Hexoatl.
	...
	ma:set_camera_position_override(nil, nil, nil, -0.34, nil);		-- x, y, d, b, h
...
function start_recruitment_advice()
	...
	local ra = intro_campaign_recruitment_advice:new(
		player_legendary_lord_char_cqi,
		-- Life proliferates in the spawning pools, revered Lord. More reptilian servants may be spawned into your service. Breed them now, and they will be ready to fight in the battles to come.
		"wh2.camp.prelude.ft.150.lzd",
...
function start_settlement_advice()
	...
	local sa = intro_campaign_select_settlement_advice:new(
		hexoatl_region_str,
		hexoatl_province_str,
		-- Let us review what destruction the warmblooded trespassers have wrought on your territory, enlightened Lord. Select the temple-city to inspect your province.
		"wh2.camp.prelude.ft.170.lzd",
		"wh2.camp.select_settlement_advice.003",
...
function start_province_advice()
	...
	cm:callback(
		function()
			...
			cm:make_region_visible_in_shroud(local_faction, hexoatl_region_str);
			cm:make_region_visible_in_shroud(local_faction, monolith_of_fallen_gods_region_str);
			cm:make_region_visible_in_shroud(local_faction, ziggurat_of_dawn_region_str);
			
			-- highlight province
			cm:callback(
				function()
					bool_regions_higlighted = true;
					CampaignUI.SetOverlayMode(20, hexoatl_region_str, monolith_of_fallen_gods_region_str, ziggurat_of_dawn_region_str);
...
function start_building_construction_advice()
	...
	local ba = intro_campaign_building_construction_advice:new(
		...
		"wh2.camp.select_settlement_advice.003", 				-- Select Hexoatl
		"wh2_main_special_settlement_hexoatl_hexoatl_1wh2_main_lzd_hexoatl",
...
function player_ends_turn()
	...
	local enemy_faction_str = skeggi_faction_str;
		
	-- look up enemy army at this time, so that we can apply an effect bundle to it in time for the start of its turn	
	local settlement_monoliths_of_fallen_gods = cm:get_region(monolith_of_fallen_gods_region_str):settlement();
	local char_enemy = cm:get_closest_general_to_position_from_faction(enemy_faction_str, settlement_monoliths_of_fallen_gods:logical_position_x(), settlement_monoliths_of_fallen_gods:logical_position_y(), false);
	local char_enemy_cqi = false;
	
	if not char_enemy then
		script_error("ERROR: player_ends_turn() couldn't find the enemy lord");
		return false;
	else
		char_enemy_cqi = char_enemy:cqi();
		cm:apply_effect_bundle_to_characters_force("wh_main_increased_movement_range_25", char_enemy_cqi, 1, false);
	end;
	...
	core:add_listener(
		...
		function() enemy_faction_starts_turn(enemy_faction_str, char_enemy_cqi) end,
		false
	);
end;
...
function enemy_faction_starts_turn(enemy_faction_str, char_enemy_cqi)
	...
	local char_enemy = cm:get_character_by_cqi(char_enemy_cqi);
	...
	-- buff its movement range a bit also, as it won't quite make it in one turn
...
function enemy_attack_launched()
	...
	cm:add_custom_battlefield(
		...
		"",																		-- loading override
		"script/battle/intro_battles/lizardmen/second/battle_start.lua",		-- script override
