...
function cutscene_intro_play()
	...
	advice_to_play[1] = "wh2.camp.vortex.queek.intro.001";
	advice_to_play[2] = "wh2.camp.vortex.queek.intro.002";
	advice_to_play[3] = "wh2.camp.vortex.queek.intro.003";
	advice_to_play[4] = "wh2.camp.vortex.queek.intro.004";
	
	local cutscene_intro = campaign_cutscene:new(
		cm:get_local_faction_name(true) .. "_intro",							-- string name for this cutscene
		61.5,															-- length of cutscene in seconds
	...
	cutscene_intro:action(
		function()
			...
			cutscene_intro:cindy_playback("script/campaign/wh2_main_great_vortex/factions/wh2_main_skv_clan_mors/scenes/clan_mors_flyover_s01.CindyScene", 0, 5);
		end,
		0
	);
...
if cm:is_multiplayer() then
	...
	enemy_secondary_lord_char_cqi = cm:get_cached_value(
		...
		function()
			local character = cm:get_closest_general_to_position_from_faction(cm:get_faction(fortress_of_dawn_faction_str), 528, 76, false);
			if character then
				return character:cqi();
			end;
		end
...
cm:add_first_tick_callback_sp_new(
	function() 
		...
		if cm:get_saved_value("bool_prelude_first_turn_was_loaded") then
			cm:set_public_order_of_province_for_region(yuatek_region_str, -34);
		end;
		
		-- set enemy faction personality
		cm:force_change_cai_faction_personality(fortress_of_dawn_faction_str, "wh_script_foolishly_brave");
