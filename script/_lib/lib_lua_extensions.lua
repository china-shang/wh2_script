...
--- @class boolean boolean Booleans
--- @page lua
...
--- @return ... @number character value(s)
...
--- @desc Returns whether the subject string starts with the search string. The comparison is case-sensitive by default.
--- @p @string subject string
--- @p @string search string
--- @p [opt=false] @boolean ignore case
--- @return boolean start matched
function string.starts_with(input, start_str, ignore_case)
	if ignore_case then
		return string.sub(string.lower(input), 1, string.len(start_str)) == string.lower(start_str);
	end;
...
--- @function end_with
--- @desc Returns whether the subject string end with the search string. The comparison is case-sensitive by default.
--- @p @string subject string
--- @p @string search string
--- @p [opt=false] @boolean ignore case
--- @return boolean start matched
 function string.ends_with(input, end_str, ignore_case)
	if ignore_case then
		return end_str == "" or string.sub(string.lower(input), -string.len(end_str)) == string.lower(end_str);
	end;
...
--- @class function function Functions
--- @page lua
...
function table.tostring(t, for_campaign_savegame, tab_level)
	...
	local function prepend_tabs(str_table, tab_level)
		for i = 1, tab_level do
			table.insert(str_table, "\t");
		end;
	end;

	local function add_line_break(str_table)
		table.insert(str_table, "\n");
	end;

	-- overwrite these formatting functions if we're writing into a savegame
	...
	for key, value in pairs(t) do
		if not for_campaign_savegame or is_table(value) or is_string(value) or is_number(value) or is_boolean(value) then
			...
			if is_table(value) then
				table.insert(str_table, table.tostring(value, for_campaign_savegame, tab_level));
...
--- @class userdata userdata Userdata
--- @page lua
...
--- @function ceilTo
--- @desc Returns the small multiple of the supplied multiplier that is larger than or equal to the supplied value.
--- @p @number value
--- @p [opt=10] @number multiplier
--- @return @number ceil value
--- @example print(math.ceilTo(18.6))
--- @result 20
--- @example print(math.ceilTo(18.6), 15)
--- @result 30
function math.ceilTo(value, multiplier)
	multiplier = multiplier or 10;
	return math.ceil(value * (1 / multiplier) ) * multiplier;
end


--- @function clamp
--- @desc Clamps a supplied subject number value to between supplied minimum and maximum values. If the subject is greater than the maximum then the maximum is returned. If the subject is less than the minimum then the minimum is returned. Otherwise, the subject value is returned unchanged.
--- @p @number subject value
--- @p @number minimum
--- @p @number maximum
--- @return @number clamped value
--- @example print(math.clamp(100, 10, 20))
--- @result 20
...
--- @function floorTo
--- @desc Returns the largest multiple of the supplied multiplier that is smaller than or equal to the supplied value.
--- @p @number value
--- @p [opt=10] @number multiplier
--- @return @number floor value
--- @example print(math.floorTo(14.1))
--- @result 10
--- @example print(math.floorTo(14.1, 3))
--- @result 12
