...
--- @loaded_in_campaign
--- @loaded_in_frontend
--- @class text_pointer text_pointer Text Pointers
...
text_pointer = {
	...
	valid_subtitle_positions = {			-- add documentation to set_subtitle_position if adding entries here
		{
			name = "bottom_subtitle_bar",
			generate_pos = function(label_size_x, label_size_y)
				local screen_x, screen_y = core:get_screen_resolution();
				
				local label_pos_x = (screen_x - label_size_x) / 2;
				local label_pos_y = (screen_y - 80) - label_size_y / 2;
				
				return label_pos_x, label_pos_y;
			end;
		},
		{
			name = "top_subtitle_bar",
			generate_pos = function(label_size_x, label_size_y)
				local screen_x, screen_y = core:get_screen_resolution();
				
				local label_pos_x = (screen_x - label_size_x) / 2;
				local label_pos_y = label_size_y / 2;
		...
		},
		{
			...
			generate_pos = function(label_size_x, label_size_y)
				...
				local label_pos_x = (screen_x - label_size_x) / 2;
				local label_pos_y = 140;
				
				return label_pos_x, label_pos_y;
			end;
		}
	},
	subtitle_position = "bottom_subtitle_bar"
};
...
--- @p number x offset, x offset in pixels.
--- @p number y offset, y offset in pixels.
...
--- @p number stream duration, Stream duration in seconds
...
function text_pointer:set_subtitle()
	self.pointer_display_mode = "subtitle";
end;


--- @function set_subtitle_position
--- @desc Sets a subtitle position string, which determines where the subtitle is displayed. This also sets the text pointer into subtitle mode. Valid subtitle modes are as follows:<ul>
--- @desc <li>bottom_subtitle_bar</li>
--- @desc <li>top_subtitle_bar</li>
--- @desc <li>top_centre</li></ul>
--- @desc By default, subtitles show on the bottom subtitle bar unless set otherwise with this function.
--- @p @string position
function text_pointer:set_subtitle_position(subtitle_position)
	
	if not is_string(subtitle_position) then
		script_error("ERROR: set_subtitle_position() called but supplied subtitle position [" .. tostring(subtitle_position) .. "] is not a string");
		return false;
	end;

	local valid_positions_str = "";
	
	for i = 1, #self.valid_subtitle_positions do
		if self.valid_subtitle_positions[i].name == subtitle_position then
			self.pointer_display_mode = "subtitle";
			self.subtitle_position = subtitle_position;
			return;
		else
			if i == 1 then
				valid_positions_str = self.valid_subtitle_positions[i].name;
			else
				valid_positions_str = valid_positions_str .. ", " .. self.valid_subtitle_positions[i].name;
			end;
		end;
	end;
	
	script_error("ERROR: set_subtitle_position() called but supplied subtitle position [" .. subtitle_position .. "] is not a valid position. Valid positions are: [" .. valid_positions_str .. "]");
end;
...
--- @desc Sets the text of the specified component on the text pointer to that looked up by a supplied db text key. Use this method to show customised text on the text pointer.
--- @p string component name, Name of text component on the text pointer.
--- @p string override text key, Name of text component on the text pointer.
...
--- @p [opt=false] force display, Forces the text pointer to display. This flag is only considered if the text pointer has been set to behave as a subtitle, in which case the text is always displayed and the player's subtitles preferences are ignored.
function text_pointer:show(force_display)
	...
	if pointer_display_mode == "subtitle" and not effect.subtitles_enabled() and not force_display then
		return;
	end;
		...
		if uic_close_button then
			...
			local close_callback = function()
				...
				if self.close_button_callback then
					if self.close_button_callback_delay == 0 then
						...
						if core:is_campaign() then
							cm:callback(function() self.close_button_callback() end, self.close_button_callback_delay);
						else
			...
			if highlight_close_button_wait_offset then	
				if highlight_close_button_wait_offset == 0 then
					...
					if core:is_campaign() then
						cm:callback(function() self:highlight_close_button(true) end, highlight_close_button_wait_offset, self.full_name);
					else
	...
	if pointer_display_mode == "subtitle" then
		for i = 1, #self.valid_subtitle_positions do
			if self.valid_subtitle_positions[i].name == self.subtitle_position then
				label_pos_x, label_pos_y = self.valid_subtitle_positions[i].generate_pos(label_size_x, label_size_y);
				break;
			end;
		end;
		
		out.design("\tsetting position of label at [" .. label_pos_x .. ", " .. label_pos_y .. "], line is subtitle with position " .. self.subtitle_position);
...
function text_pointer:start_streaming_text()
	...
	local update_time_measure = "ms";
	
	if core:is_campaign() then
		update_time = 0.1;
		update_time_measure = "s"
	end;
	...
	out(self.name .. ":start_streaming_text() called, will display " .. chars_per_update .. " characters every " .. update_time .. update_time_measure);
	...
	for i = 1, #self.component_overrides do
		...
		if override_record.is_streaming then
			...
			if bookmark + chars_per_update > override_record.localised_text_len then
				...
				if core:is_campaign() then
					cm:callback(function() self:update_streaming_text_by_char(chars_per_update, update_time) end, update_time, self.full_name);
				else
...
function text_pointer:hide(immediately)
	...
	if self.should_pulse then
		if core:is_campaign() then
			cm:callback(function() pulse_uicomponent(uic_text_label, false, self.panel_pulse_strength, true) end, 0.5);
		else
...
function text_pointer:set_style(style, param1)
	...
	if style == "standard" then										--- @desc <ul><li>standard</li>
		self:set_priority(1500);
		self:set_topmost();
	elseif style == "semitransparent_worldspace" then				--- @desc <li>semitransparent_worldspace</li>
		self:set_panel_state_override("semitransparent");
	elseif style == "semitransparent" then							--- @desc <li>semitransparent</li>
		self:set_priority(1500);
		self:set_topmost();
		self:set_panel_state_override("semitransparent");
	elseif style == "semitransparent_large" then					--- @desc <li>semitransparent_large</li>
		self:set_priority(1500);
		self:set_topmost();
		self:set_panel_state_override("semitransparent_large");
	elseif style == "semitransparent_2_sec_highlight" then			--- @desc <li>semitransparent_2_sec_highlight</li>
		self:set_priority(1500);
		self:set_topmost();
		self:set_panel_state_override("semitransparent");
		self:set_highlight_close_button(2);
		self:set_hide_on_close_button_clicked(false);
	elseif style == "e3_subtitle_with_frame" then					--- @desc <li>e3_subtitle_with_frame</li>
		self:set_priority(1500);
		self:set_topmost();
	...
	elseif style == "subtitle_with_frame" then						--- @desc <li>subtitle_with_frame</li></ul>
		self:set_priority(1500);
		self:set_topmost();
		self:set_panel_state_override("framed");
		...
		self:set_panel_width_to_screen(300, true);
		self:set_stream_by_char(true);
	elseif style == "top_centre_subtitle" then						--- @desc <li>top_centre_subtitle</li></ul>
		self:set_priority(1500);
		self:set_topmost();
		self:set_subtitle_position("top_centre");
		self:set_panel_state_override("mostly_transparent");
		self:set_text_state_override("top_centre_subtitle");
		self:set_panel_show_animation("show_500ms");
		self:set_panel_hide_animation("hide_500ms");
		self:set_text_hide_animation("slide_out_br_500ms");
		self:set_subtitle();
		self:set_panel_width(450);
		self:set_stream_by_char(true);
	end;
end;
