...
function campaign_ui_manager:new()
	...
	cm:add_game_destroyed_callback(
		function(context) 
			ui:reset_all_overrides();
		end
	);
...
function campaign_ui_manager:start_campaign_ui_listeners()
	...
	core:add_listener(
		...
		function(context)
			...
			out.ui("Character selected, forename " .. tostring(character:get_forename()) .. ", surname " .. tostring(character:get_surname()) .. ", position [" .. character:logical_position_x() .. ", " .. character:logical_position_y() .. "], faction " .. character:faction():name() .. ", cqi " .. tostring(character:cqi()));
	...
	core:add_listener(
		...
		true,
		function(context) 
			local settlement = "settlement:" .. context:garrison_residence():region():name();
...
--- @desc Returns the name of the first fullscreen panel that's open, or <code>false</code> if no panels are open. Usually only one fullscreen panel is open at a time but in certain circumstances (such as diplomacy) more than one may be open at a time.
...
--- @desc Returns whether a settlement in a region with the supplied name is selected.
--- @p string region name
--- @return boolean is settlement selected
function campaign_ui_manager:is_settlement_selected(region_name)
	return self.settlement_selected == region_name;
end;
...
----------------------------------------------------------------------------
--- @section End of Turn Warnings
--- @desc The campaign ui manager automatically listens for the end-of-turn warning panel, works out what warnings are present and fires custom script events to notify any listeners. The following table shows all events that this script can trigger:
--- @desc <table class="simple"><tr><td><b>Warning Name</b></td><td><b>Script Event Issued</b></td></tr><tr><td><code>bankrupt</code></td><td><code>ScriptEventBankruptcyWarningIssued</code></td></tr><tr><td><code>tech</code></td><td><code>ScriptEventTechnologyWarningIssued</code></td></tr><tr><td><code>edict</code></td><td><code>ScriptEventCommandmentWarningIssued</code></td></tr><tr><td><code>character</code></td><td><code>ScriptEventCharacterSkillPointWarningIssued</code></td></tr><tr><td><code>army</code></td><td><code>ScriptEventMilitaryForceSkillPointWarningIssued</code></td></tr><tr><td><code>politics</code></td><td><code>ScriptEventOverseerWarningIssued</code></td></tr><tr><td><code>siege</code></td><td><code>ScriptEventSiegeNoEquipmentWarningIssued</code></td></tr><tr><td><code>morale</code></td><td><code>ScriptEventMoraleWarningIssued</code></td></tr></table>
----------------------------------------------------------------------------
...
--- @function suppress_end_of_turn_warning
--- @desc Suppresses or un-suppresses an end turn warning from appearing. End turn warning names can be found in the table above.
--- @p string warning name
--- @p [opt=false] boolean should suppress
...
function campaign_ui_manager:load_override_state(context)
	...
	if self.should_save_override_state then
		...
		for override_name in string.gmatch(load_str, "%w+") do 		
...
function campaign_ui_manager:highlight_character(character, marker_type, altitude)
...
function campaign_ui_manager:unhighlight_character(character, use_marker)
	local marker_name = cm:char_lookup_str(character);
...
function campaign_ui_manager:highlight_blessed_spawnings_button(value, pulse_strength, force_highlight)
	...
	local uic_renown_button = find_uicomponent(ui_root, "layout", "hud_center", "small_bar", "button_group_army", "button_renown");
...
function campaign_ui_manager:highlight_bloodlines_button(value, pulse_strength, force_highlight)
	...
	local uic = find_uicomponent(core:get_ui_root(), "layout", "faction_buttons_docker", "button_bloodlines");
...
function campaign_ui_manager:highlight_gods_bar(value, pulse_strength, force_highlight)
	...
	local uic = find_uicomponent(core:get_ui_root(), "layout", "resources_bar", "norsca_gods_frame");
...
function campaign_ui_manager:highlight_infamy(value, pulse_strength, force_highlight)
	...
	return highlight_visible_component(value, true, "layout", "resources_bar", "infamy_holder");
end;
...
function campaign_ui_manager:highlight_monstrous_arcanum_button(value, pulse_strength, force_highlight)
	...
	local uic = find_uicomponent(core:get_ui_root(), "layout", "faction_buttons_docker", "button_monsters");
...
function campaign_ui_manager:highlight_pieces_of_eight_button(value, pulse_strength, force_highlight)
	...
	local uic = find_uicomponent(core:get_ui_root(), "layout", "faction_buttons_docker", "button_treasure_hunts");
...
function campaign_ui_manager:highlight_pieces_of_eight_panel(value, pulse_strength, force_highlight, do_not_highlight_upstream)
	...
	if uic_panel and uic_panel:Visible(true) then
		...
		if uic_button_pieces_of_eight and (uic_button_pieces_of_eight:CurrentState() == "selected"  or uic_button_pieces_of_eight:CurrentState() == "selected_down") then
			...
			--print_all_uicomponent_children(uic_panel)
		else
	...
	if uic_panel and uic_panel:Visible(true) then
		...
		for j = 0, regions:num_items() - 1 do
			...
			if regions:item_at(j):foreign_slot_manager_for_faction(cm:get_local_faction_name(true)):is_null_interface() == false then
				local region = regions:item_at(j):name();
			
				highlight_action(self, settlement_prepend_str .. region);
			end;
...
function campaign_ui_manager:highlight_raise_dead_button(value, pulse_strength, force_highlight)
	...
	local cm = self.cm;
	local uic = find_uicomponent(core:get_ui_root(), "layout", "button_group_army", "button_mercenaries");
...
function campaign_ui_manager:highlight_recruit_black_ark_button(value, pulse_strength, force_highlight)
	...
	local uic = find_uicomponent(core:get_ui_root(), "layout", "hud_center", "button_group_settlement", "button_sea_locked_general");
