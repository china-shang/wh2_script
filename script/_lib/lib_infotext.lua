...
--- @loaded_in_battle
--- @loaded_in_campaign
--- @class infotext_manager infotext_manager Infotext Manager
...
--- @example local uic_infotext = im:get_uicomponent()			-- calling a function on the object once created
...
function infotext_manager:show_infotext(infotext_record, end_callback)
	...
	for i = 1, #infotext_record do
		...
		elseif string.sub(current_infotext_record, 1, 1) == "-" then
			...
			do
				local function add_infotext_callback()
				...
				local interval = (delay + ((i - 1) * 0.1 * time_speed)) * time_multiplier * time_speed;

				if interval == 0 then
					add_infotext_callback();
				else
					callback(add_infotext_callback, interval, "add_infotext");
				end;
			end;
