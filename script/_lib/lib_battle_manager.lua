...
--
--- @loaded_in_battle
--- @class battle_manager battle_manager Battle Manager
--- @index_pos 1
--- @alias bm
...
function battle_manager:new(b)
	...
	bm.__tostring = function() return TYPE_BATTLE_MANAGER end;
	bm.__type = function() return TYPE_BATTLE_MANAGER end;
	
	-- overwrite out() with a custom output function for battle
	getmetatable(out).__call = function(t, input) 		-- t is the 'this' object
		local output_str = get_timestamp() .. "<" .. tostring(timestamp_tick) .. "ms> ";
	...
	bm.tm = timer_manager:new(b:model_tick_time_ms());
	...
	core:set_ui_root(UIComponent(UIComponent(b:ui_component("layout")):Parent())); 
...
--- @function out
--- @desc Prints a string to the debug output spool. The string is prepended with a timestamp.
--- @p string output
...
function battle_manager_phase_change(event)
	local bm = get_bm();
	bm:out("\t\tEvent triggered :: " .. event:get_name());

	for i = 1, #bm.phase_change_callback_list do	
		if bm.phase_change_callback_list[i].event == event:get_name() then
...
function battle_manager_unit_selection_handler(unit, selected)	
	for i = 1, #get_bm().unit_selection_callback_list do
		local current_unit_selection_callback_entry = get_bm().unit_selection_callback_list[i];
...
function Battle_Manager_Battle_Results(event)
	if event:get_name() == "Battle Results" then
		get_bm():process_results(event:get_bool1());
	end;
end;
...
function battle_manager:play_next_advice()
	...
	if current_advice.is_debug then
		...
		if advice_offset > 0 then
			self:callback(function() effect.advance_scripted_advice_thread(current_advice.advisor_string, 1) end, advice_offset, "battle_manager_pending_advice");
		else
...
function battle_manager:set_locatable_objective_action(obj_key, cam_pos_generator, duration)
	...
	if not uic_panel then
		script_error("ERROR: set_locatable_objective_action() called but couldn't find uic_panel");
		return false;
	end;
	
	local uic_button_zoom = find_uicomponent(uic_panel, obj_key, "button_zoom");
	if not uic_button_zoom then
		script_error("ERROR: set_locatable_objective_action() called but couldn't find uic_button_zoom");
		return false;
	end;
	...
	core:add_listener(
		...
		function()
			...
			self:out("* set_locatable_objective_action() is scrolling the camera to pos: " .. v_to_s(cam_pos) .. ", targ: " .. v_to_s(cam_targ) .. " over duration " .. duration .. "s as zoom button has been clicked");
...
--- @p @number zoom duration, Duration of camera movement in seconds.
function battle_manager:set_locatable_objective_callback(obj_key, cam_pos_generator, duration)
	if not is_string(obj_key) then
...
function battle_manager:enable_spell_browser_button(value)
	...
	if value == false then
		...
		if self.spell_browser_button_text and self.spell_browser_button_text_src then
			uic:SetTooltipText(self.spell_browser_button_text, self.spell_browser_button_text_src, true);
		end;
	end;
end;
