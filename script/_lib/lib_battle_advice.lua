...
--
--- @loaded_in_battle
--- @class advice_manager advice_manager Advice Manager
...
function advice_manager:attempt_to_trigger(advice_monitor)
	...
	if self.advice_is_playing then
		if advice_monitor.can_interrupt_other_advice then
			...
			if self.is_debug then
				self:out("Attempting to trigger advice monitor " .. advice_monitor.name .. " - advice is currently playing and it cannot interrupt, doing nothing");
			end;
			return;
		end;
	end;
...
--	ADVICE MONITOR
--
--- @class advice_monitor advice_monitor Advice Monitor
--- @page advice_manager
