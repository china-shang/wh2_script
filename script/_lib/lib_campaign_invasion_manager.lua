...
function invasion_manager:new_invasion(key, faction_key, force_list, spawn_location)
	out("Invasion Manager: New Invasion '"..tostring(key).."'...");
	out("\tFaction: "..tostring(faction_key));
	out("\tForce: "..tostring(force_list));
	out("\tSpawn: "..tostring(spawn_location));
...
function invasion_manager:new_invasion_from_existing_force(key, force)
	out("Invasion Manager: New Invasion from existing force'"..tostring(key).."'...");
	out("\tForce: "..tostring(force));
...
function invasion_manager:parse_spawn_location(spawn_location)
	out("Invasion Manager: Parsing a Spawn Location of '"..tostring(spawn_location).."'");
...
function invasion_manager:remove_invasion(key)
	if key ~= nil then
		out("Invasion Manager: Removing Invasion '"..key.."'");
...
function invasion_manager:kill_invasion_by_key(key)	
	...
	if invasion then
		out("Invasion Manager: Killing Invasion [" .. invasion.key .. "]");
		invasion:kill();
	end
end

---	@class invasion invasion Invasion
--- @page invasion_manager
--- @section Setup
---- Internal - Creates a new invasion object
function invasion:new(o)
	out.savegame("Invasion: New Invasion object created... ["..tostring(o).."]");
	o = o or {};
	setmetatable(o, self);
	self.__index = self;
	return o;
...
function invasion:set_target(target_type, target, target_faction_key)
	out("Invasion: Set Target for '"..self.key.."'...");
	target_type = target_type or "NONE";
	
	if target_type == "REGION" then
		out("\tTarget: "..target_type.." - "..tostring(target));
	...
	elseif target_type == "LOCATION" then
		out("\tTarget: "..target_type.." - X:"..tostring(target.x).." / Y:"..tostring(target.y));
		self.target_type = target_type;
		self.target = target;
...
function invasion:remove_target()
	out("Invasion: Removing Target for '"..self.key.."'");
	self.target_type = "NONE";
	self.target = nil;
...
--- @p table target_list, A table containing faction keys of all the factions that will trigger an attack
...
function invasion:add_aggro_radius(radius, target_list, cooldown_timer, abort_timer)
	...
	if radius > 0 and #target_list > 0 then
		out("Invasion: Adding Aggro Radius of "..tostring(radius).." for '"..self.key.."' with "..tostring(#target_list).." targets, a cooldown of "..tostring(cooldown_timer).." and an abort time of "..tostring(abort_timer).." turns");
...
function invasion:abort_on_target_owner_change(abort)
	out("Invasion: Invasion '"..self.key.."' will abort on target owner change: "..tostring(abort));
...
function invasion:should_stop_at_end(should_stop)
	if should_stop == true then
		out("Invasion: Invasion will stop after target");
	else
		out("Invasion: Invasion will NOT stop after target");
	end
...
function invasion:apply_effect(effect_key, turns)
	if effect_key == nil and turns == nil then
		out("Invasion: Applying stored effect '"..self.effect.."' ("..self.effect_turns..") to force "..self.force_cqi);
		cm:apply_effect_bundle_to_force(self.effect, self.force_cqi, self.effect_turns);
	else
		if self.started == true then
			out("Invasion: Applying effect '"..effect_key.."' ("..turns..") to force "..self.force_cqi);
			cm:apply_effect_bundle_to_force(effect_key, self.force_cqi, turns);
		else
			out("Invasion: Preparing effect '"..effect_key.."' ("..turns..")");
...
function invasion:add_character_experience(experience_amount, by_level)
	if experience_amount == nil then
		if self.experience_by_level == true then
			...
			out("Invasion: Applying stored character experience amount (by level) of " .. xp .. " to general " .. self.general_cqi);
			cm:add_agent_experience(cm:char_lookup_str(self.general_cqi), xp);
		else
			out("Invasion: Applying stored character experience amount of " .. self.experience_amount .. " to general " .. self.general_cqi);
		...
		if self.started == true then
			if by_level == true then
				...
				out("Invasion: Applying character experience amount (by level) of " .. xp .. " to general " .. self.general_cqi);
				cm:add_agent_experience(cm:char_lookup_str(self.general_cqi), experience_amount);
			else
				out("Invasion: Applying character experience amount of " .. experience_amount .. " to general " .. self.general_cqi);
				cm:add_agent_experience(cm:char_lookup_str(self.general_cqi), experience_amount);
			end
		else
			out("Invasion: Preparing character experience amount of " .. experience_amount);
...
function invasion:add_unit_experience(unit_experience_amount)
	if unit_experience_amount == nil then
		...
		if self.started == true then
			...
			out("Invasion: Preparing unit experience amount of " .. unit_experience_amount .. " - Total: " .. self.unit_experience_amount);
		end
	end
end
...
function invasion:start_invasion(callback_function, declare_war, invite_attacker_allies, invite_defender_allies)
	out("Invasion: Starting Invasion for '"..self.key.."'...");
	...
	if self.started == false then
		...
		if self.prespawned == true then
			out("\tPre-Spawned force: Ignoring force spawning...");
			self:force_created(self.general_cqi, declare_war, invite_attacker_allies, invite_defender_allies);
		else
			out("\tSpawning Force '"..tostring(self.unit_list).."'...");
			local temp_region = cm:model():world():region_manager():region_list():item_at(0):name();
			
			if self.new_general ~= nil then
				out("\t\tCreating new general!");
				cm:create_force_with_general(self.faction, self.unit_list, temp_region, self.spawn.x, self.spawn.y, self.new_general.agent_type, self.new_general.agent_subtype, self.new_general.forename, self.new_general.clan_name, self.new_general.family_name, self.new_general.other_name, self.new_general.make_faction_leader,
				...
				if self.general_cqi == nil then
					...
					cm:create_force_with_existing_general("character_cqi:"..self.general_cqi, self.faction, self.unit_list, temp_region, self.spawn.x, self.spawn.y,
					function(cqi) self:force_created(cqi, declare_war, invite_attacker_allies, invite_defender_allies) end);
				end
			end
		end
		
		out("\tAdding Listener 'INVASION_"..self.key.."'");
		core:add_listener(
			"INVASION_"..self.key,
...
function invasion:force_created(general_cqi, declare_war, invite_attacker_allies, invite_defender_allies, was_respawn)
	...
	if self.immortal_general ~= nil then
		out("\t\tMaking Character Immortal: "..tostring(self.immortal_general));
	...
	out("\t\tForce Spawned (General CQI: "..tostring(general_cqi)..", Force CQI: "..tostring(self.force_cqi)..", Invasion: "..tostring(self.key)..")");
		...
		if this_faction and enemy_faction then
			if this_faction:at_war_with(enemy_faction) == false then
				if declare_war == true then
					...
					out("\t\t\tDeclared war on "..tostring(self.target_faction));
				end
			end
		end
	end
	...
	if was_respawn == true then
		...
		core:trigger_event("ScriptEventInvasionManagerRespawn", self);
	end
end

---- Internal - Respawn the invasion at its start location
function invasion:attempt_respawn()
	out("Invasion: Attempting Respawn for '"..self.key.."'...");
	out("\tRespawns Remaining: "..tostring(self.respawn_count));
	...
	if self.respawn_turn >= self.respawn_delay then
		...
		elseif self.rogue_army == true then
			...
			out("\tCOULDN'T FIND CHARACTER TO RESPAWN?!");
			self.respawn = false;
			
			local faction = cm:model():world():faction_by_key(self.faction);
			out("\tFaction: "..faction:command_queue_index());
			local char_list = faction:character_list();
			
			for i = 0, char_list:num_items() - 1 do
				local character = char_list:item_at(i);
				out("\t\t"..character:command_queue_index());
				out("\t\t"..character:get_forename().." / "..character:get_surname());
				out("\t\t"..tostring(character:is_wounded()));
			end
		end
	else
		out("\tWaiting another turn...");
		self.respawn_turn = self.respawn_turn + 1;
		out("\tTurns: "..tostring(self.respawn_turn).." / Delay: "..tostring(self.respawn_delay));
	end
end

---- Internal - Advances the invasion, moving or attacking their target if there is one
function invasion:advance()
	out("Invasion: Advancing Invasion for '"..self.key.."'...");
	...
	if force:is_null_interface() == false then
		...
		if general:is_null_interface() == false then
			...
			out("\tGeneral Lookup: "..general_lookup);
			out("\tTarget: "..tostring(self.target_type).." ["..tostring(self.target).."]");
			
			if self.maintain_army == true then
				out("\tMaintaining army...");
				if force:unit_list():num_items() < #self.unit_list_table then
					out("\t\tArmy is missing units: "..force:unit_list():num_items().."/"..#self.unit_list_table);
					if cm:model():random_percent(self.maintain_army_chance) then
						out("\t\tArmy succeeded its maintain chance of "..self.maintain_army_chance.."%");
							...
							for current_unit, current_number in pairs(multiple_units["current"]) do
								if initial_unit == current_unit then
									if current_number < initial_number then
										...
										out("\t\tAdding unit to army: "..initial_unit);
									end
								end
							end
						end
					else
						out("\t\tArmy failed its maintain chance of "..self.maintain_army_chance.."%");
					end
				end
			end
			
			if self.target_type ~= "NONE" then
				out("\tDisabling movement for invasion general");
				cm:cai_disable_movement_for_character(general_lookup);
				
				if self.target_faction ~= nil then
					out("\tOn advance, "..self.faction.." declares war on "..self.target_faction);
				...
				if self.aggro_enabled == true then
					if self.aggro_target_cqi ~= nil then
						out("\tAggro target already in progress");
					...
					elseif self.aggro_target_cqi == nil then
						...
						if self.aggro_cooldown_value > 0 then
							...
							out("\t\tAggro cooldown is in effect ("..tostring(self.aggro_cooldown_value)..")");
						else
							out("\t\tAttempting to find target in aggro radius ("..tostring(self.aggro_radius)..")");
							local aggro_target = self:find_aggro_target();
							
							if aggro_target ~= nil then
								out("\t\t\tFound target: "..tostring(aggro_target));
								self.aggro_target_cqi = aggro_target;
							else
								out("\t\t\tCouldn't find target");
							end
						end
					end
				end
				...
				if self.aggro_enabled == true and self.aggro_target_cqi ~= nil then
					...
					if char_obj:is_null_interface() == false then
						...
						out("\tAttacking aggro target: "..tostring(self.aggro_target_cqi));
						cm:cai_disable_movement_for_character(general_lookup);
						...
						cm:attack(general_lookup, target_character_lookup, false);
						self.aggro_turn_abort_value = self.aggro_turn_abort_value + 1;
						out("\t\tTurns spent in aggro: "..tostring(self.aggro_turn_abort_value));
					else
				...
				elseif self.target_type == "LOCATION" then
					...
					--------------------------------------------------------------------------------
					out("\tMoving to Location... ["..self.target.x..", "..self.target.y.."]");
					...
					out("\tDistance from target = "..distance_from_target);
				
					if distance_from_target < 2 then
						out("\tArrived at Location!");
						self.target_type = "NONE";
					else
						out("\tMoving...");
						cm:move_to(general_lookup, self.target.x, self.target.y, false);
					end
				elseif self.target_type == "CHARACTER" then
					...
					if tagert_character_obj:is_null_interface() == false and tagert_character_obj:has_military_force() then
						out("\tAttacking Character...");
						cm:cai_disable_movement_for_character(general_lookup);
						cm:enable_movement_for_character(general_lookup);
						cm:replenish_action_points(general_lookup);
						cm:attack(general_lookup, target_character_lookup, false);
					else
						out("\tCouldn't find target... releasing force!");
						self.target_type = "NONE";
					end
				elseif self.target_type == "REGION" then
					...
					if target_region_obj:is_null_interface() == false and target_region_obj:is_abandoned() == false then
						...
						if region_owner == self.target_faction then
							out("\tAttacking Region...");
							cm:cai_disable_movement_for_character(general_lookup);
							cm:enable_movement_for_character(general_lookup);
							cm:replenish_action_points(general_lookup);
							cm:attack_region(general_lookup, target_region_key, false);
						else
							out("\tTarget Region owner is no longer initial faction target:");
							out("\t\tInitial: "..tostring(self.target_faction));
							out("\t\tCurrent: "..region_owner);
							
							if self.target_owner_abort == false then
								out("\ttarget_owner_abort is "..tostring(self.target_owner_abort).." - Switching Target Faction!");
								self.target_faction = region_owner;
								cm:force_declare_war(self.faction, self.target_faction, true, true);
							else
								out("\ttarget_owner_abort is "..tostring(self.target_owner_abort).." - Aborting Invasion!");
								self.target_type = "NONE";
							end
						end
					else
				...
				elseif self.target_type == "FORCE" then
					...
					if target_force_obj:is_null_interface() == false then
						if target_force_obj:has_general() == true then
							...
							cm:enable_movement_for_character(general_lookup);
							cm:replenish_action_points(general_lookup);
							cm:attack(general_lookup, enemy_general_lookup, false);
						end
					else
						out("\tCouldn't find target... releasing force!");
						self.target_type = "NONE";
				...
				elseif self.target_type == "PATROL" then
					...
					------------------------------------------------------------------------------------
					out("\tFollowing patrol route...");
					out("\tNext patrol point: #"..self.patrol_position.." ["..self.target[self.patrol_position].x..", "..self.target[self.patrol_position].y.."]");
					...
					out("\tDistance from next patrol point = "..distance_from_target);
				
					if distance_from_target < 2 then
						out("\tArrived at patrol location #"..self.patrol_position);
						
						if self.patrol_position == #self.target then
							out("\t\tLast patrol position reached...");
							...
							if self.stop_at_end == true then
								...
								out("\t\t\tRestarting patrol and moving... #"..self.patrol_position.." ["..self.target[self.patrol_position].x..", "..self.target[self.patrol_position].y.."]");
								self.patrol_position = 1;
								cm:move_to(general_lookup, self.target[self.patrol_position].x, self.target[self.patrol_position].y, false);
							end
						elseif self.target[self.patrol_position + 1] ~= nil then
							out("\t\tMoving to next patrol point... #"..self.patrol_position.." ["..self.target[self.patrol_position].x..", "..self.target[self.patrol_position].y.."]");
						...
						out("\tMoving... #"..self.patrol_position.." ["..self.target[self.patrol_position].x..", "..self.target[self.patrol_position].y.."]");
						cm:move_to(general_lookup, self.target[self.patrol_position].x, self.target[self.patrol_position].y, false);
					end
				end
			end
		...
		out("\tForce is a null interface, assuming it has died...");
	...
	if self.target_type == "NONE" then
		...
		if general:is_null_interface() == false then
			...
			if self.stop_at_end == true then
				out("\tInvasion has been told to stop after completion");
				out("\tDisabling movement for invasion general");
				cm:cai_disable_movement_for_character(general_lookup);
			else
...
function invasion:find_aggro_target()
	local general = self:get_general();
				...
				for j = 0, target_faction:military_force_list():num_items() - 1 do
					...
					if force:is_armed_citizenry() == false and force:has_general() == true then
						...
						if enemy_general:is_at_sea() == at_sea then
							if at_sea == false or (at_sea == true and enemy_general:in_settlement() == false) then
								...
								if distance <= self.aggro_radius then
									return enemy_general:command_queue_index();
								end
							end
						end
					end
...
function invasion:release()
	out("Invasion: Releasing Invasion with key '"..self.key.."'...");
	local general = self:get_general();
...
function invasion:kill(general_only)
	...
	if commander:is_null_interface() == false then
		out("Invasion: Killing Invasion with key '"..self.key.."'...");
		local commander_cqi = commander:command_queue_index();
		local killed = cm:kill_character(commander_cqi, not general_only, false);
		out("\tKilled: "..tostring(killed));
	end
	self:remove_target();
	invasion_manager:remove_invasion(self.key);
end
