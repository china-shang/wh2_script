...
--
--- @loaded_in_battle
---	@class generated_battle generated_battle Generated Battle
--- @alias gb
		...
		for j = 1, #current_alliance do
			...
			for k = 1, #current_armies do
				...
				for m = 1, current_army.sunits:count() do
					self.bm:out("\t\t\t\t" .. m .. ": " .. current_army.sunits:item(m).unit:type() .. " at position " .. v_to_s(current_army.sunits:item(m).unit:position()));
				end;
...
function generated_battle:start_deployment()
	...
	if self.intro_cutscene then
		if self.cutscene_during_deployment then
			...
			local uic_deployment = self.bm:ui_component("finish_deployment");
			
			--self.bm:out("Disabling finish_deployment UI for Cutscene.");
			uic_deployment:SetVisible(false);
			self.sm:add_listener(
				"cutscene_ended",
				function()
					--self.bm:out("Re-enabling finish_deployment UI.");
			...
			self.bm:callback(
				function()
					if group_name then
						...
						self.bm:out("generated_battle:start_terrain_composite_scene_on_message() is responding to message " .. message .. " and is now playing composite scene with key " .. comp_scene_key .. wait_offset_str);
					end;
					self.bm:start_terrain_composite_scene(comp_scene_key, group_name, delay_if_queued);
				end,
				wait_offset
			);
		end
...
function generated_battle:stop_terrain_composite_scene_on_message(message, comp_scene_key, wait_offset)
	...
	self.sm:add_listener(
		...
		function()
			...
			self.bm:callback(
				function()
					self.bm:out("generated_battle:stop_terrain_composite_scene_on_message() is responding to message " .. message .. " and is now stopping composite scene with key " .. comp_scene_key .. wait_offset_str);
...
--- @p @number marker type, Marker type. See the documentation for @battle:add_ping_icon for a list of valid values.
...
function generated_battle:battle_ending()
	...
	if not winning_alliance_num then
		self.bm:out("\twarning: could not determine the winning alliance");
		return;
	end;
...
--	GENERATED ARMY
--
---	@class generated_army generated_army Generated Army
--- @alias ga
...
function generated_army:new(script_name, sub_army_num, sunits, generated_battle, is_debug)
	...
	if not (alliance_number == bm:local_alliance() and army_number == bm:local_army()) and generated_battle:should_not_deploy_ai() and has_deployed(first_sunit.unit) then
	...
	self.sm:add_listener(
		...
		function()
			if value then
				...
				self.bm:out(self.id .. " responding to message " .. message .. ", making all currently-standing units not-invincible and setting their morale to default");
...
function generated_army:refresh_on_message(message)
	...
	self.sm:add_listener(
		message,
		function()
			self.bm:out(self.id .. " responding to message " .. message .. ", refreshing units");
...
function generated_army:deploy_at_random_intervals_on_message(message, min_units, max_units, min_period, max_period, cancel_message, spawn_first_wave_immediately)
	if not is_string(message) then
	...
	self.sm:add_listener(
		...
		function()
			if not cancel_message_received then
				...
				self.sunits:deploy_at_random_intervals(min_units, max_units, min_period, max_period, true, spawn_first_wave_immediately);
			end;
		end,
		0
	);
...
--	GENERATED CUTSCENE
--
-- @class generated_cutscene generated_cutscene Generated Cutscene
-- @page generated_battle
